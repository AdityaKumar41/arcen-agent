# OpenAI-Compatible API Server for Arcen Agent
...
SSE streaming. By exposing this endpoint, arcen-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  arcen-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.arcen/.env`
2. Run `arcen gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with arcen-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns arcen-agent as a model) |
...
Authorization: Bearer arcen-api-key-here
...
But arcen-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "arcen-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured arcen-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of arcen-agent's core prompt
...
Once implemented, arcen-agent works as a drop-in backend for:
