...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/arcen/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when arcen
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# arcen process and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main arcen process and per-profile
...
RUN set -eu; \
    ...
    # 'Arcen WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via ARCEN_UID at runtime
RUN useradd -u 10000 -m -d /opt/data arcen
...
#
# ui-tui/packages/arcen-ink/ is copied IN FULL (not just its manifests)
# because it is referenced as a `file:` workspace dependency from
# ui-tui/package.json.  Copying the tree up front lets npm resolve the
# workspace to real content instead of stopping at a bare package.json.
COPY package.json package-lock.json ./
COPY ui-tui/package.json ui-tui/
COPY ui-tui/packages/arcen-ink/ ui-tui/packages/arcen-ink/
...
# lock on the @arcen/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/arcen/.venv at first
...
COPY --chown=arcen:arcen . .

# Build terminal UI assets.
RUN cd ui-tui && npm run build

# ---------- Permissions ----------
# Make install dir world-readable so any ARCEN_UID can read it at runtime.
# The venv needs to be traversable too.
# node_modules trees additionally need to be writable by the arcen user
# so the runtime `npm install` triggered by _tui_need_npm_install() in
# arcen_cli/main.py succeeds (see #18800).
# /opt/arcen/gateway is runtime-writable: Python may create __pycache__ and
# gateway state artifacts beneath the package after services drop privileges,
# especially when the arcen UID is remapped at boot (#27221).
# The .venv MUST remain arcen-writable so lazy_deps.py can install
# remaining optional platform packages and future pin bumps at first use.
# Without this, `uv pip install` fails with EACCES and adapters silently
# fail to load.  See tools/lazy_deps.py.
USER root
RUN chmod -R a+rX /opt/arcen && \
    chown -R arcen:arcen /opt/arcen/.venv /opt/arcen/ui-tui /opt/arcen/gateway /opt/arcen/node_modules
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the arcen user via
# `s6-setuidgid arcen` in its run script. If ARCEN_UID is unset, services
# run as the default arcen user (UID 10000).

# ---------- Link arcen-agent itself (editable) ----------
...
# container always returns nothing — meaning `arcen dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the ARCEN_GIT_SHA build-arg to
# /opt/arcen/.arcen_build_sha at build time, and have
# arcen_cli/build_info.py read it at runtime.  Both `arcen dump` and
...
ARG ARCEN_GIT_SHA=
RUN if [ -n "${ARCEN_GIT_SHA}" ]; then \
        printf '%s\n' "${ARCEN_GIT_SHA}" > /opt/arcen/.arcen_build_sha && \
        chown arcen:arcen /opt/arcen/.arcen_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static service declared at build time: main-arcen.
...
# `exec arcen`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $ARCEN_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/arcen/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-arcen-setup && \
    chmod +x /etc/cont-init.d/01-arcen-setup
...
# fast path (`node --expose-gc /opt/arcen/ui-tui/dist/entry.js`) and skip the
...
# describes the monorepo workspace set, but the image only needs root/ui-tui.
# So the actualized node_modules can permanently
...
ENV ARCEN_TUI_DIR=/opt/arcen/ui-tui
ENV ARCEN_HOME=/opt/data

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> arcen ...` they default to root, and any file the
# command writes under $ARCEN_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/arcen/bin/arcen, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid arcen`
# when invoked as root. Non-root callers (supervised processes,
# `--user arcen`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/arcen/.venv/bin/arcen). See the shim source for
# the opt-out env var (ARCEN_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/arcen-exec-shim.sh /opt/arcen/bin/arcen

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> arcen ...` and any
# subprocess that doesn't activate the venv first still find arcen.
#
# /opt/arcen/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/arcen/bin:/opt/arcen/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. arcen
# subcommand vs. no-args), drops to the arcen user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/arcen/docker/main-wrapper.sh" ]
CMD [ ]
