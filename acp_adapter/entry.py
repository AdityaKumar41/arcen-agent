"""CLI entry point for the arcen-agent ACP adapter.

Loads environment variables from ``~/.arcen/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from ARCEN_HOME (default ``~/.arcen``)."""
    from arcen_cli.env_loader import load_arcen_dotenv

    arcen_home = get_arcen_home()
    loaded = load_arcen_dotenv(arcen_home=arcen_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", arcen_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="arcen-acp",
        description="Run Arcen Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Arcen version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import ArcenACPAgent  # noqa: F401

    print("Arcen ACP check OK")


def _run_setup() -> None:
    from arcen_cli.main import main as arcen_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "arcen", "model"]
        arcen_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``arcen postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting arcen-agent ACP adapter")
    ...
    import acp
    from .server import ArcenACPAgent
