...
``auth.json``.  They intentionally have no dependency on ``arcen_cli.auth`` so
...
# Sources Arcen owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Arcen-owned OAuth/device-code state)
