...
def _state_file() -> Path:
    return get_arcen_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.arcen/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `arcen update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``arcen update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``arcen curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `arcen curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Arcen' background skill CURATOR. This is an "
    ...
    "into ~/.arcen/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: arcen-config-*, arcen-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.arcen/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.arcen/logs/curator/``)
    ...
    authored skill data in ``~/.arcen/skills/``.

    ``ensure_arcen_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_arcen_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every arcen install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: arcen curator status
        keep an umbrella stable: arcen curator pin document-tools
    ...
    lines.append("full report: arcen curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: arcen curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.arcen/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.arcen/skills/.archive/` for "
            "safety and can be restored via `arcen curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.arcen/skills/.archive/`. "
            "Restore any via `arcen curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `arcen curator restore <name>`")
    lines.append("- All archives live under `~/.arcen/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `arcen curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `arcen update` only if "
        ...
        # recorded in state so `arcen curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``arcen model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `arcen model` → auxiliary
