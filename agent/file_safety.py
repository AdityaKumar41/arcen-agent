...
def _arcen_home_path() -> Path:
    """Resolve the active ARCEN_HOME (profile-aware) without circular imports."""
    try:
        from arcen_constants import get_arcen_home  # local import to avoid cycles
        return get_arcen_home()
    except Exception:
        return Path(os.path.expanduser("~/.arcen"))


def _arcen_root_path() -> Path:
    """Resolve the Arcen root dir (always the parent of any profile, never per-profile)."""
    try:
        from arcen_constants import get_default_arcen_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    arcen_home = _arcen_home_path()
    arcen_root = _arcen_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(arcen_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(arcen_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved ARCEN_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("ARCEN_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Arcen control-plane files: block both the ACTIVE profile's view
    # (arcen_home) AND the global root view. Without the root pass, a
    ...
    arcen_dirs = []
    for base in (_arcen_home_path(), _arcen_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in arcen_dirs:
                arcen_dirs.append(real)
        except Exception:
            continue

    for base_real in arcen_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Arcen path.

    Three categories are blocked:

      * Internal Arcen cache files under ``ARCEN_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under ARCEN_HOME and the global Arcen
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.arcen/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active ARCEN_HOME (profile-aware) AND the global
    # Arcen root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (ARCEN_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    arcen_dirs: list[Path] = []
    for base in (_arcen_home_path(), _arcen_root_path()):
        try:
            real = base.resolve()
            if real not in arcen_dirs:
    ...
    for hd in arcen_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Arcen cache file "
    ...
    for hd in arcen_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Arcen credential store "
    ...
    for hd in arcen_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Arcen MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Arcen MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Arcen profiles are separate ARCEN_HOME dirs under
...
#
# Reference: May 2026 incident where a arcen-security profile session
# edited skills under both ``~/.arcen/profiles/arcen-security/skills/``
# AND ``~/.arcen/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under ARCEN_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from ARCEN_HOME.
    ...
    """
    try:
        home_real = _arcen_home_path().resolve()
        root_real = _arcen_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Arcen scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Arcen entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Arcen profile {info['target_profile']!r}, but the "
...
#
#   <ARCEN_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.arcen/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.arcen/…`` segment and warns
# regardless of which Arcen profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.arcen/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.arcen`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.arcen/…`` and returns the
    index where the inner Arcen-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .arcen / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".arcen":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Arcen state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.arcen``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.arcen`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Arcen resolver to
    succeed, so it works correctly even when called from contexts where
    ARCEN_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Arcen process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real ARCEN_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.arcen/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.arcen/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host ARCEN_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Arcen state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Arcen "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real ARCEN_HOME. Use the "
