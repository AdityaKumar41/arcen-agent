...
as ``kind: backend``) or ``~/.arcen/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``arcen tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``arcen tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``arcen tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$ARCEN_HOME/cache/images/``, creating parents as needed."""
    from arcen_constants import get_arcen_home

    path = get_arcen_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$ARCEN_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$ARCEN_HOME/cache/images/``.
