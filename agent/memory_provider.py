...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - arcen_home (str): The active ARCEN_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.arcen``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "arcen").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'arcen memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], arcen_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'arcen memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``arcen_home`` is the active ARCEN_HOME directory path.
