...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Arcen starts and finds ``~/.openclaw/``.

    Points users at ``arcen claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``arcen claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Arcen, run "
        "`arcen claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `arcen claw cleanup` (renames it to ~/.openclaw.pre-migration — "
