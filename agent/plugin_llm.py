...
Plugins built on Arcen Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Arcen
...
:class:`~arcen_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Arcen' main config
...
handles every provider, fallback chain, and per-task override Arcen
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`arcen_cli.plugins.PluginContext`
