...
Every Arcen request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Arcen Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=arcen-agent",
        "client=arcen-client-v<__version__>",
    ]

The version is sourced live from ``arcen_cli.__version__`` so it auto-aligns
...
``arcen_cli.__version__`` is the canonical source of truth.
"""
...
def _arcen_version() -> str:
    """Return the current Arcen release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``arcen_cli`` cannot be imported (should
...
def arcen_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=arcen-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=arcen-client-v{_arcen_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=arcen-agent", arcen_client_tag()]
