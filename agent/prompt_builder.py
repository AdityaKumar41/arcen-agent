...
from arcen_constants import get_arcen_home, get_skills_dir, is_wsl
...
_ARCEN_MD_NAMES = (".arcen.md", "ARCEN.md")


def _find_arcen_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.arcen.md`` or ``ARCEN.md``.
...
ARCEN_AGENT_HELP_GUIDANCE = (
    ...
    "the documentation at https://arcen-cli.arcenpay.com/docs is your "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.arcen/kanban.db`. Your task id is in "
    "`$ARCEN_KANBAN_TASK`; your workspace is `$ARCEN_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ARCEN_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `arcen kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Arcen WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Arcen itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Arcen runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Arcen itself is running. The host OS, home, and cwd "
                f"of the Arcen process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Arcen "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Arcen
    ...
    extra = (os.getenv("ARCEN_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_arcen_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.arcen/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ARCEN_PLATFORM")
        or get_session_env("ARCEN_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Arcen Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `arcen-agent` skill "
            "first. It has the actual commands (e.g. `arcen config set …`, `arcen tools`, "
            "`arcen setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: arcen setup, arcen setup tools, arcen setup terminal, arcen status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from ARCEN_HOME and return its content, or None.
    ...
    """
    try:
        from arcen_cli.config import ensure_arcen_home
        ensure_arcen_home()
    except Exception as e:
        logger.debug("Could not ensure ARCEN_HOME before loading SOUL.md: %s", e)

    soul_path = get_arcen_home() / "SOUL.md"
...
def _load_arcen_md(cwd_path: Path) -> str:
    """.arcen.md / ARCEN.md — walk to git root."""
    arcen_md_path = _find_arcen_md(cwd_path)
    if not arcen_md_path:
        return ""
    try:
        content = arcen_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = arcen_md_path.name
        try:
            rel = str(arcen_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".arcen.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", arcen_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .arcen.md / ARCEN.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ARCEN_HOME is independent and always included when present.
    ...
    # SOUL.md from ARCEN_HOME only — skip when already loaded as identity
    if not skip_soul:
