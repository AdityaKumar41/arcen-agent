...
Storage
-------
Bundles live in ``~/.arcen/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ARCEN_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``arcen bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ARCEN_HOME.

    Honors ``ARCEN_BUNDLES_DIR`` for tests; falls back to
    ``<ARCEN_HOME>/skill-bundles``.
    """
    override = os.environ.get("ARCEN_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_arcen_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `arcen bundles` CLI subcommand.
# ---------------------------------------------------------------------------
