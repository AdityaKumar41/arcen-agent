...
# Matches ${ARCEN_SKILL_DIR} / ${ARCEN_SESSION_ID} tokens in SKILL.md.
# Tokens that don't resolve (e.g. ${ARCEN_SESSION_ID} with no session) are
# left as-is so the user can debug them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(ARCEN_SKILL_DIR|ARCEN_SESSION_ID)\}")
...
def substitute_template_vars(
    ...
    """Replace ${ARCEN_SKILL_DIR} / ${ARCEN_SESSION_ID} in skill content.
    ...
    def _replace(match: re.Match) -> str:
        token = match.group(1)
        if token == "ARCEN_SKILL_DIR" and skill_dir_str:
            return skill_dir_str
        if token == "ARCEN_SESSION_ID" and session_id:
