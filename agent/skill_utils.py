...
from arcen_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ARCEN_KANBAN_TASK`` / ``ARCEN_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("ARCEN_KANBAN_TASK") or os.getenv("ARCEN_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Arcen Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ARCEN_PLATFORM`` or
            ``ARCEN_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``arcen`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.arcen/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``arcen`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ARCEN_HOME, not cwd
        if not p.is_absolute():
            p = (arcen_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.arcen/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    arcen = metadata.get("arcen") or {}
    if not isinstance(arcen, dict):
        arcen = {}
    return {
        "fallback_for_toolsets": arcen.get("fallback_for_toolsets", []),
        "requires_toolsets": arcen.get("requires_toolsets", []),
        "fallback_for_tools": arcen.get("fallback_for_tools", []),
        "requires_tools": arcen.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    arcen = metadata.get("arcen")
    if not isinstance(arcen, dict):
        return []
    raw = arcen.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Arcen metadata, VCS, virtualenv/dependency, and cache
