...
Owns one Codex thread per Arcen session. Drives `turn/start`, consumes
...
# Arcen' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ARCEN_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Arcen/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Arcen session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ARCEN_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ARCEN_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="arcen",
            client_title="Arcen Agent",
            client_version=_get_arcen_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Arcen' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Arcen' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own arcen-tools callback we
            # auto-accept — the user already approved Arcen' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Arcen approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Arcen returns 'once', 'session', 'always', or 'deny'.
...
def _get_arcen_version() -> str:
    """Best-effort Arcen version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("arcen-agent")
