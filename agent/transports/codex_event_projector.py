"""Projects codex app-server events into Arcen' messages list.

The translator that lets Arcen' memory/skill review keep working under the
...
Arcen' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Arcen
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Arcen' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Arcen stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Arcen already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Arcen' messages store text only.
        text_parts: list[str] = []
