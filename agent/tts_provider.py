...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Arcen with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.arcen/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Arcen
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``arcen tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``arcen setup`` for
