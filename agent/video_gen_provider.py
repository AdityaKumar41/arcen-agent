...
as ``kind: backend``) or ``~/.arcen/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``arcen tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ARCEN_HOME/cache/videos/``, creating parents as needed."""
    from arcen_constants import get_arcen_home

    path = get_arcen_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ARCEN_HOME/cache/videos/``.
