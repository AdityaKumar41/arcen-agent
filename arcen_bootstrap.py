"""Windows UTF-8 bootstrap for Arcen entry points.
...
should be imported at the very top of every Arcen entry point
(``arcen``, ``arcen-agent``, ``arcen-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import arcen_bootstrap``
# (or ``from arcen_bootstrap import apply_windows_utf8_bootstrap``) at
