"""
Backup and import commands for arcen CLI.

`arcen backup` creates a zip archive of the entire ~/.arcen/ directory
(excluding the arcen-agent repo and transient files).

`arcen import` restores from a backup zip, overlaying onto the current
ARCEN_HOME root.
"""
...
from arcen_constants import get_default_arcen_root, get_arcen_home, display_arcen_home
...
_EXCLUDED_DIRS = {
    "arcen-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to arcen root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ARCEN_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Arcen home directory."""
    arcen_root = get_default_arcen_root()

    if not arcen_root.is_dir():
        print(f"Error: Arcen home directory not found at {arcen_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"arcen-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"arcen-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_arcen_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(arcen_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(arcen_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(arcen_root)
    ...
    print(f"\nRestore with: arcen import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like an Arcen backup.
    ...
    # Look for telltale files that a arcen home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be an Arcen backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.arcen/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a arcen dir name
        if prefix in {".arcen", "arcen"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_arcen_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (arcen_root / "config.yaml").exists()
        has_env = (arcen_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Arcen configuration.")
        ...
        for member in members:
            ...
            target = arcen_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(arcen_root.resolve())
            except ValueError:
        ...
        profiles_dir = arcen_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # arcen_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: arcen profile list  (after installing arcen)")

        # Guidance
        print()
        if not (arcen_root / "arcen-agent").is_dir():
            print("Note: The arcen-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: arcen update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  arcen -p {pname} gateway install")

        print("Done. Your Arcen configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and arcen backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ARCEN_HOME).
...
# — `arcen update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(arcen_home: Optional[Path] = None) -> Path:
    home = arcen_home or get_arcen_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    arcen_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(arcen_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ARCEN_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``arcen update``.
    ...
    Args:
        ...
        arcen_home: Override for the Arcen home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(arcen_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for arcen backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_arcen_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, arcen_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``arcen_root`` to ``out_path``.
...
def _pre_update_backup_dir(arcen_home: Optional[Path] = None) -> Path:
    home = arcen_home or get_arcen_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ARCEN_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ARCEN_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``arcen update``) should continue even if the backup fails.
    """
    arcen_root = arcen_home or get_default_arcen_root()
    if not arcen_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(arcen_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, arcen_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `arcen claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ARCEN_HOME under ``backups/`` before a
    ``arcen claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``arcen import <archive>``.  Writes to
    ``<ARCEN_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `arcen import` and the
