"""`arcen checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.arcen/checkpoints/``.  Actions:

    arcen checkpoints               # same as `status`
    arcen checkpoints status        # total size, project count, breakdown
    arcen checkpoints list          # per-project checkpoint counts + workdir
    arcen checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    arcen checkpoints clear [-f]    # nuke the entire base (asks first)
    arcen checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    arcen checkpoints
    arcen checkpoints prune --retention-days 3 --max-size-mb 200
    arcen checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: arcen checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``arcen checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `arcen checkpoints` → status
