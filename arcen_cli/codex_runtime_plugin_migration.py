"""Migrate Arcen' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Arcen' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Arcen mcp_servers.<n>.command/args/env  → codex stdio transport
  Arcen mcp_servers.<n>.url/headers       → codex streamable_http transport
  Arcen mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Arcen mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Arcen-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Arcen' own
...
MIGRATION_MARKER = (
    "# managed by arcen-agent — `arcen codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end arcen-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Arcen config.")
...
# Arcen keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Arcen' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, arcen_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Arcen MCP server config to the codex inline-table dict
    ...
    if not isinstance(arcen_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(arcen_cfg.get("command"))
    has_url = bool(arcen_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ARCEN_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Arcen)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Arcen' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Arcen' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Arcen
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="arcen-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ARCEN_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed arcen-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.arcen`` symlink that happens to live under ``/private/var/folders``
...
def _build_arcen_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Arcen' own
    ...
    so a arcen installed under /opt/, /usr/local/, or a venv all work.
    ARCEN_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ARCEN_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_arcen_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ARCEN_HOME its launcher sets
    ...
    # — that would override the launcher's ARCEN_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ARCEN_HOME", tmp_path) would
    ...
    arcen_home = os.environ.get("ARCEN_HOME") or ""
    if arcen_home and _looks_like_test_tempdir(arcen_home):
        arcen_home = ""
    if arcen_home:
        env["ARCEN_HOME"] = arcen_home
    # PYTHONPATH passes through so a worktree-launched arcen finds the
    ...
    env["ARCEN_QUIET"] = "1"
    env["ARCEN_REDACT_SECRETS"] = env.get("ARCEN_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.arcen_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    expose_arcen_tools: bool = True,
) -> MigrationReport:
    """Translate Arcen mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        arcen_config: full ~/.arcen/config.yaml dict
        ...
        expose_arcen_tools: when True (default), register Arcen' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Arcen for tools
    ...
    arcen_servers = (arcen_config or {}).get("mcp_servers") or {}
    if not isinstance(arcen_servers, dict):
        report.errors.append(
            "mcp_servers in Arcen config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in arcen_servers.items():
    ...
    # Inject Arcen' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Arcen for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/arcen_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_arcen_tools:
        translated["arcen-tools"] = _build_arcen_tools_mcp_entry()
        if "arcen-tools" not in report.migrated:
            report.migrated.append("arcen-tools")
