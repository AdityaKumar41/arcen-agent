"""Slash command definitions and autocomplete for the Arcen CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Arcen config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Arcen works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Arcen is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.arcen/skills/ for newly installed or removed skills",
    ...
    CommandDef("plugins", "List installed plugins and their status",
               "Tools & Skills", cli_only=True),
    CommandDef("reach", "Agent-Reach internet capability suite (doctor, setup, update, status)",
               "Tools & Skills", args_hint="[doctor|setup|update|status]",
               subcommands=("doctor", "setup", "update", "status")),
    ...
    CommandDef("learn", "Reflect on current session trajectory and save as a custom skill",
               "Tools & Skills", aliases=("reflect",), args_hint="[skill-name]"),
    CommandDef("evolve", "Analyze skill failure patterns and apply Hermes-style prompt optimizations",
               "Tools & Skills", args_hint="<skill-name>"),
    CommandDef("mcp", "Manage Model Context Protocol (MCP) servers (list, status, add, reload)",
               "Tools & Skills", args_hint="[list|status|add <name> <cmd>|reload]",
               subcommands=("list", "status", "add", "reload")),
    ...
    CommandDef("update", "Update Arcen Agent to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`arcen_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/arcen`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Arcen
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``arcen skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``arcen skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``arcen skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/arcen <verb>`` subcommand.
    ...
    can still reach them via ``/arcen <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/arcen`` is always reserved as the first entry so the
    legacy ``/arcen <subcommand>`` form keeps working for anything that
    ...
    # Reserve /arcen as the catch-all top-level command.
    entries.append(("arcen", "Talk to Arcen or run a subcommand", "[subcommand] [args]"))
    seen.add("arcen")
...
def slack_app_manifest(request_url: str = "https://arcen-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /arcen handler.

    Maps both canonical names and aliases so /arcen bg do stuff works
    the same as /arcen background do stuff.

    Plugin-registered slash commands are included so ``/arcen <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from arcen_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
