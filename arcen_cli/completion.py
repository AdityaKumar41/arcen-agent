"""Shell completion script generation for arcen CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_arcen_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Arcen Agent bash completion
# Add to ~/.bashrc:
#   eval "$(arcen completion bash)"

_arcen_profiles() {{
    local profiles_dir="$HOME/.arcen/profiles"
...
_arcen_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_arcen_profiles)" -- "$cur"))
        return
    fi
...
complete -F _arcen_completion arcen
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef arcen
# Arcen Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(arcen completion zsh)"

_arcen_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.arcen/profiles" ]]; then
        profiles+=($HOME/.arcen/profiles/*(N/:t))
    fi
...
_arcen() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_arcen_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'arcen command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Arcen Agent fish completion",
        "# Add to your config:",
        "#   arcen completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __arcen_profiles",
        "    echo default",
        "    if test -d $HOME/.arcen/profiles",
        "        for d in $HOME/.arcen/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c arcen -f -s p -l profile"
        " -d 'Profile name' -xa '(__arcen_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__arcen_profiles)' -d 'Profile name'"
                )
