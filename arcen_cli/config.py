"""
Configuration management for Arcen Agent.

Config files are stored in ~/.arcen/ for easy access:
- ~/.arcen/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.arcen/.env         - API keys and secrets

This module provides:
- arcen config          - Show current configuration
- arcen config edit     - Open config in editor
- arcen config set      - Set a specific value
- arcen config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``arcen config set`` (which rewrites
    ...
    ``config.yaml`` in place: arcen never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.arcen/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``arcen logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  arcen config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Arcen makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Arcen itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``arcen update``, the TUI build.
...
#   on every plugin install / ``arcen update``.
...
# * ``ARCEN_HOME`` / ``ARCEN_PROFILE`` / ``ARCEN_CONFIG`` /
#   ``ARCEN_ENV`` — Arcen runtime location flags. Writing these into
...
#
# IMPORTANT: ``ARCEN_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ARCEN_GEMINI_CLIENT_ID,
# ARCEN_LANGFUSE_PUBLIC_KEY, ARCEN_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Arcen runtime location — never via dashboard env writer.
    # NOT a ARCEN_* blanket: integration credentials (ARCEN_GEMINI_*,
    # ARCEN_LANGFUSE_*, ARCEN_SPOTIFY_*, ...) ARE allowed.
    "ARCEN_HOME", "ARCEN_PROFILE", "ARCEN_CONFIG", "ARCEN_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Arcen runtime location "
            "(ARCEN_HOME, ARCEN_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `arcen plugins enable
    # observability/langfuse` or `arcen tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ARCEN_LANGFUSE_ENV",
    "ARCEN_LANGFUSE_RELEASE",
    "ARCEN_LANGFUSE_SAMPLE_RATE",
    "ARCEN_LANGFUSE_MAX_CHARS",
    "ARCEN_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from arcen_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ARCEN_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_arcen_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Arcen is running in package-manager-managed mode.

    Two signals: the ARCEN_MANAGED env var (set by the systemd service),
    or a .managed marker file in ARCEN_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade arcen-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Arcen was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.arcen/.install_method`` file (written by installers)
    2. ARCEN_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``nousresearch/arcen-agent`` image stamps ``docker``
    ...
    so ``arcen update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_arcen_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.arcen/.install_method."""
    stamp = get_arcen_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Arcen lives in a ``uv tool`` layout.

    ``uv tool install arcen-agent`` places the install at
    ``.../uv/tools/arcen-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``arcen-agent`` happens to be uv-tool-installed on the machine while
    the *active* Arcen is a regular pip/venv install, causing
    ``arcen update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/arcen-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade arcen-agent"
    if method == "docker":
        return "docker pull nousresearch/arcen-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade arcen-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade arcen-agent"
        return "pip install --upgrade arcen-agent"
    return "arcen update"
...
# Long-form text for ``arcen update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# arcen_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Arcen, it doesn't update
...
✗ ``arcen update`` doesn't apply inside the Docker container.

Arcen Agent runs as a published image (nousresearch/arcen-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/arcen-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate arcen-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/arcen-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/arcen-agent/tags
  • Your config and session history live under ``$ARCEN_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``arcen update`` inside Docker.
...
def format_managed_message(action: str = "modify this Arcen installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ARCEN_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Arcen installation is managed by NixOS "
            f"(ARCEN_MANAGED={env_hint}).\n"
            "Edit services.arcen-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Arcen installation is managed by Homebrew "
            f"(ARCEN_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade arcen-agent"
        )

    return (
        f"Cannot {action}: this Arcen installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Arcen."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ARCEN_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, arcen_bin
    or None if container mode is not active, we're already inside the
    container, or ARCEN_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ARCEN_DEV") == "1":
        return None
    ...
    container_mode_file = get_arcen_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "arcen-agent")
    exec_user = info.get("exec_user", "arcen")
    arcen_bin = info.get("arcen_bin", "/data/current-package/bin/arcen")
...
# Re-export from arcen_constants — canonical definition lives there.
from arcen_constants import get_arcen_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_arcen_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_arcen_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_arcen_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ARCEN_UID / ARCEN_GID env vars set by Docker deployments.

    Docker containers running Arcen commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ARCEN_HOME once, but
    subdirectories created at runtime by ``ensure_arcen_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ARCEN_UID", "").strip()
    gid_str = os.environ.get("ARCEN_GID", "").strip()
    try:
...
def _chown_to_arcen_uid(path) -> None:
    """Chown ``path`` to ``ARCEN_UID:ARCEN_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_arcen_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_arcen_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the arcen group can
    share state with the gateway service.

    The mode can be overridden via the ARCEN_HOME_MODE environment variable
    (e.g. ARCEN_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ARCEN_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ARCEN_UID``/``ARCEN_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ARCEN_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_arcen_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Arcen runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ARCEN_CONTAINER") or os.environ.get("ARCEN_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ARCEN_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ARCEN_HOME if the user doesn't have one yet."""
...
def ensure_arcen_home():
    """Ensure ~/.arcen directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_arcen_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_arcen_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ARCEN_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Arcen-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Arcen
        ...
        # identity slot (SOUL.md). Empty by default. The ARCEN_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # skipped. When empty, Arcen auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Arcen sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Arcen
        ...
        #  "/home/user/.arcen/cache/documents:/output"]
        ...
        # bundled Arcen image, which drops to the `arcen` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Arcen sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Arcen should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``arcen chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.arcen/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: arcen sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Arcen truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``arcen kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``arcen kanban decompose`` and the kanban
        ...
        # ``arcen profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `arcen model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `arcen` (and `arcen chat`) launches by default:
        ...
        # REPL and `--tui` (or ARCEN_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `arcen --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `arcen -c` muscle memory.  Default off so existing
        # users aren't surprised.  ARCEN_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `arcen --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/arcen`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.arcen/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.arcen/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Arcen feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Arcen auto-pauses the goal and
    ...
    # always goes to ~/.arcen/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ARCEN_SKILL_DIR} and ${ARCEN_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `arcen curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `arcen update`, so pruning them only
        ...
        # skipped), snapshot ~/.arcen/skills/ into
        # ~/.arcen/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `arcen curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for arcen-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Arcen Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ARCEN_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.arcen/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ARCEN_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `arcen doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``arcen_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Arcen to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via ARCEN_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `arcen -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `arcen` launches with
        ...
        # decomposition is manual via `arcen kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     arcen-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Arcen tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.arcen/logs/.
    ...
    # Optional remotely-hosted model catalog manifest.  When a URL is set,
    # the CLI fetches curated model lists for OpenRouter and Nous Portal,
    # falling back to the in-repo snapshot on network failure.
    "model_catalog": {
        "enabled": True,
        "url": "",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `arcen model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.arcen/.env,
        ...
        # behavior: files must live under the Arcen cache, under
        ...
        # user. Bridged to ARCEN_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Arcen
        # cache (~/.arcen/cache/{documents,images,audio,video,screenshots})
        ...
        # to ARCEN_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ARCEN_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ARCEN_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.arcen/state.db.
    ...
    "sessions": {
        ...
        # default of ``arcen sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.arcen/sessions/session_{sid}.json`` on every turn
    ...
    # ``arcen update`` behaviour.
    "updates": {
        # Run a full ``arcen backup``-style zip of ARCEN_HOME before every
        # ``arcen update``.  Backups land in ``<ARCEN_HOME>/backups/`` and
        # can be restored with ``arcen import <path>``.  Off by default —
        # on large ARCEN_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `arcen update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<ARCEN_HOME>/lsp/bin/`` on first use.
    ...
    # `arcen tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.arcen/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.arcen/.env (or your shell) and never in config.yaml.
            ...
            # ~/.arcen/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'arcen model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Arcen auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @arcen:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ARCEN_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #arcen)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: arcen-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'arcen-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Arcen API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Arcen API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Arcen API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ARCEN_TOOL_PROGRESS and ARCEN_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.arcen.config`` entries, then checks
    ...
    try:
        ...
        # should never break `arcen update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Arcen versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``arcen_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'arcen setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Arcen won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'arcen doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ARCEN_HOME", "~/.arcen")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ARCEN_TOOL_PROGRESS")
            old_mode = get_env_value("ARCEN_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ARCEN_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ARCEN_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ARCEN_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ARCEN_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ARCEN_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_arcen_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `arcen plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `arcen curator status` has no
    ...
    #   3. Creates `~/.arcen/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_arcen_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_arcen_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: arcen config set <key> <value>")
    ...
    # metadata.arcen.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _arcen_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.arcen/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.arcen/config.yaml.
    ...
    (which change ``ARCEN_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — arcen auth) — OpenAI Codex
#   nous         (OAuth — arcen auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.arcen/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.arcen/.env.
    ...
    interactive menu render (`arcen tools`, `arcen setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `arcen tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Arcen env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.arcen/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.arcen/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.arcen/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.arcen/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Arcen — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Arcen vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.arcen/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Arcen Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Warn on stale ARCEN_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ARCEN_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ARCEN_MAX_ITERATIONS={_env_ghost} "
                f"(run 'arcen doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  arcen config edit     # Edit config file", Colors.DIM))
    print(color("  arcen config set <key> <value>", Colors.DIM))
    print(color("  arcen setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: arcen config set <key> <value>")
            print()
            print("Examples:")
            print("  arcen config set model anthropic/claude-sonnet-4")
            print("  arcen config set terminal.backend docker")
            print("  arcen config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'arcen config migrate' to add them")
        ...
        print("  arcen config           Show current configuration")
        print("  arcen config edit      Open config in editor")
        print("  arcen config set <key> <value>   Set a config value")
        print("  arcen config check     Check for missing/outdated config")
        print("  arcen config migrate   Update config with new options")
        print("  arcen config path      Show config file path")
        print("  arcen config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``arcen config`` UI so users
