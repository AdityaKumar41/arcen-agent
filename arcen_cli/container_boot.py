...
``$ARCEN_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-arcen-setup
(the stage2 hook) has chowned the volume and seeded $ARCEN_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$ARCEN_HOME``,
    not under ``profiles/``). The dispatcher in ``arcen_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``arcen gateway start`` (no ``-p``) targets. Without it,
    bare ``arcen gateway start`` inside the container would land on
    ...
    ``$ARCEN_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        arcen_home: The container's ARCEN_HOME (typically /opt/data).
            Profiles live under ``<arcen_home>/profiles/<name>/``;
            the default profile lives at ``<arcen_home>`` itself.
    ...
    # ``arcen gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_prior_state(arcen_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(arcen_home)
    ...
    profiles_root = arcen_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `arcen profile create` (config.yaml
            # is not — that comes later via `arcen setup`). Use it as the
            ...
            # via ``arcen -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(arcen_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = arcen_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("ARCEN_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "arcen":
        args = args[1:]
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `arcen -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit arcen
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the arcen user) won't hit EACCES. See
...
def _write_reconcile_log(
    arcen_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $ARCEN_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    arcen_home = Path(os.environ.get("ARCEN_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        arcen_home=arcen_home, scandir=scandir,
    )
    for a in actions:
        print(
