...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(arcen\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*arcen)"
    r"|(systemctl\s+(restart|stop|start)\s+.*arcen)"
    r"|(p?kill\s+.*arcen.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'arcen cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: arcen gateway install", Colors.DIM))
        print(color("                    sudo arcen gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    arcen gateway install    # Install as a user service")
        print("    sudo arcen gateway install --system  # Linux servers: boot-time system service")
        print("    arcen gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `arcen gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: arcen cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
