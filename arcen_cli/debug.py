"""``arcen debug`` debug tools for Arcen Agent.

Currently supports:
    arcen debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.arcen/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[arcen debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.arcen/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``arcen debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``arcen debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Arcen version, provider, which API keys
    are configured — NOT the actual keys)
  • Recent log lines (agent.log, errors.log, gateway.log — may
    contain conversation fragments and file paths)
  • Full agent.log and gateway.log (up to 512 KB each — likely
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `arcen debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "arcen-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``arcen debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.arcen/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``arcen debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "arcen-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----ArcenDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from arcen_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_arcen_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``arcen dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``arcen dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``arcen debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "arcen debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    # 2-3. Full logs (optional — failures are collected, not raised)
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `arcen debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  arcen debug delete <url>")

    print(f"\nShare these links with the Arcen team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: arcen debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'arcen debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``arcen debug`` call.
    ...
    # best-effort — any failure is swallowed so ``arcen debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: arcen debug <command>")
        print()
        print("Commands:")
