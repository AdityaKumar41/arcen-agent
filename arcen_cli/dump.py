"""
Dump command for arcen CLI.

Outputs a compact, plain-text summary of the user's Arcen setup
...
from arcen_cli.config import get_arcen_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.arcen_build_sha`` by the Dockerfile's
    ``ARCEN_GIT_SHA`` build-arg (see ``arcen_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``arcen dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from arcen_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(arcen_home: Path) -> int:
...
def _cron_summary(arcen_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = arcen_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"arcen_home:      {display_arcen_home()}")
    ...
    toolsets = config.get("toolsets", ["arcen-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(arcen_home)}")
    lines.append(f"  skills:             {_count_skills(arcen_home)}")
