"""
arcen fallback — manage the fallback provider chain.
...
Subcommands:
  arcen fallback [list]   Show the current fallback chain (default when no subcommand)
  arcen fallback add      Pick provider + model via the same picker as `arcen model`,
                           then append the selection to the chain
  arcen fallback remove   Pick an entry to delete from the chain
  arcen fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.arcen/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from arcen_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `arcen model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  arcen fallback add")
        print()
        return
    ...
    print("  Docs: https://arcen-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `arcen model`, then append the selection to the chain."""
    from arcen_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `arcen model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `arcen fallback list` to view, or `arcen fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``arcen fallback [subcommand]``."""
