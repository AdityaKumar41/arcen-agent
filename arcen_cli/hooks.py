"""arcen hooks — inspect and manage shell-script hooks.

Usage::

    arcen hooks list
    arcen hooks test <event> [--for-tool X] [--payload-file F]
    arcen hooks revoke <command>
    arcen hooks doctor

Consent records live under ``~/.arcen/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.arcen/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``arcen hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: arcen hooks {list|test|revoke|doctor}")
        print("Run 'arcen hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.arcen/config.yaml.")
        print("See `arcen hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `arcen hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `arcen hooks test` and `arcen hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Arcen wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `arcen hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `arcen hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `arcen hooks doctor`.")
