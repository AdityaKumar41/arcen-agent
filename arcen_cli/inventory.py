...
def load_picker_context() -> ConfigContext:
    ...
    Replaces inline config slices used by ``tui_gateway/server.py``.
    """
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      ...
      mirroring the ``arcen model`` CLI picker. Adds network calls
...
def _append_unconfigured_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    from arcen_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `arcen model` to configure ({auth_type})"
        )
