...
Used by ``arcen kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``arcen_cli/goals.py`` — same aux
...
ARCEN_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    int(os.getenv("ARCEN_KANBAN_SPECIFY_MAX_TOKENS", "6000")),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Arcen Agent board.
...
def _profile_author() -> str:
    """Mirror of ``arcen_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("ARCEN_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=ARCEN_KANBAN_SPECIFY_MAX_TOKENS,
