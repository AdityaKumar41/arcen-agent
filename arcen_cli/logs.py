"""``arcen logs`` — view and filter Arcen log files.
...
Usage examples::
    ...
    arcen logs                    # last 50 lines of agent.log
    arcen logs -f                 # follow agent.log in real time
    arcen logs errors             # last 50 lines of errors.log
    arcen logs gateway -n 100    # last 100 lines of gateway.log
    arcen logs --level WARNING    # only WARNING+ lines
    arcen logs --session abc123   # filter by session ID substring
    arcen logs --component tools  # only tool-related lines
    arcen logs --since 1h         # lines from the last hour
    arcen logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_arcen_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Arcen runs — try 'arcen chat' first)")
        sys.exit(1)
    ...
    if component:
        from arcen_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_arcen_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_arcen_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_arcen_home()}/logs/")
        return

    print(f"Log files in {display_arcen_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'arcen chat' to generate logs)")
