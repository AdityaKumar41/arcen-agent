#!/usr/bin/env python3
"""
Arcen CLI - Main entry point.

Usage:
    arcen                     # Interactive chat (default)
    arcen chat                # Interactive chat
    arcen gateway             # Run gateway in foreground
    arcen gateway start       # Start gateway as service
    arcen gateway stop        # Stop gateway service
    arcen gateway status      # Show gateway status
    arcen gateway install     # Install gateway service
    arcen gateway uninstall   # Uninstall gateway service
    arcen setup               # Interactive setup wizard
    arcen logout              # Clear stored authentication
    arcen status              # Show status of all components
    arcen cron                # Manage cron jobs
    arcen cron list           # List cron jobs
    arcen cron status         # Check if cron scheduler is running
    arcen doctor              # Check configuration and dependencies
    arcen honcho setup                    # Configure Honcho AI memory integration
    arcen honcho status                   # Show Honcho config and connection status
    arcen honcho sessions                 # List directory → session name mappings
    arcen honcho map <name>               # Map current directory to a session name
    arcen honcho peer                     # Show peer names and dialectic settings
    arcen honcho peer --user NAME         # Set user peer name
    arcen honcho peer --ai NAME           # Set AI peer name
    arcen honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    arcen honcho mode                     # Show current memory mode
    arcen honcho mode [hybrid|honcho|local]  # Set memory mode
    arcen honcho tokens                   # Show token budget settings
    arcen honcho tokens --context N       # Set session.context() token cap
    arcen honcho tokens --dialectic N     # Set dialectic result char cap
    arcen honcho identity                 # Show AI peer identity representation
    arcen honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    arcen honcho migrate                  # Step-by-step migration guide: OpenClaw native → Arcen + Honcho
    arcen version             Show version
    arcen update              Update to latest version
    arcen uninstall           Uninstall Arcen Agent
    arcen acp                 Run as an ACP server for editor integration
    arcen sessions browse     Interactive session picker with search

    arcen claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: arcen_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``arcen_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``arcen update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``arcen_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, arcen crashes on import and the user can't run
# ``arcen update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import arcen_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'arcen' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``arcen tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``arcen.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("arcen")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"arcen", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"arcen")
        # Windows: the .exe name is already ``arcen.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* arcen_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ARCEN_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".arcen", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``ARCEN_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("ARCEN_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``ARCEN_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ARCEN_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`arcen --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from arcen_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Arcen Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``arcen --version`` before config/logging imports on Termux."""
    if os.environ.get("ARCEN_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to ARCEN_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (arcen tools, arcen setup, arcen model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'arcen {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any arcen module import.
#
# Many modules cache ARCEN_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ARCEN_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.arcen/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ARCEN_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors arcen_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ARCEN_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.arcen/profiles/coder or
    # /opt/data/profiles/coder).  If ARCEN_HOME points to the arcen root
    # instead (e.g. systemd hardcodes ARCEN_HOME=/root/.arcen), we must
    # still read active_profile — the user may have switched profiles via
    # `arcen profile use` and the gateway should honour that choice.
    # See issue #22502.
    arcen_home_env = os.environ.get("ARCEN_HOME", "")
    if profile_name is None and arcen_home_env:
        if Path(arcen_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the arcen root
    if profile_name is None:
        try:
            from arcen_constants import get_default_arcen_root

            active_path = get_default_arcen_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ARCEN_HOME
    if profile_name is not None:
        try:
            from arcen_cli.profiles import resolve_profile_env

            arcen_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent arcen from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ARCEN_HOME"] = arcen_home
...
# Load .env from ~/.arcen/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from arcen_cli.config import get_arcen_home
from arcen_cli.env_loader import load_arcen_dotenv

load_arcen_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ARCEN_REDACT_SECRETS env
# var BEFORE arcen_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_arcen_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "ARCEN_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ARCEN_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `arcen` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from arcen_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from arcen_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if arcen_constants not importable yet
...
from arcen_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_arcen_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ARCEN_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ARCEN_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from arcen_cli.config import get_env_path, get_arcen_home, load_config
    from arcen_cli.auth import get_auth_status

    # Determine whether Arcen itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from arcen_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_arcen_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from arcen_cli.auth import PROVIDER_REGISTRY
    ...
    # Only count these if Arcen has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Arcen to use their tokens.
    if _has_arcen_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from arcen_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, arcen_bin
        cli_args: the original CLI arguments (everything after 'arcen')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    arcen_bin = container_info["arcen_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo arcen {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo arcen {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  arcen --tui --resume {target}")
    if title:
        print(f'  arcen --tui -c "{title}"')
    print()
...
def _workspace_root(dir: Path) -> Path:
    ...
    lockfile/node_modules resolution and ``npm install`` cwd stay
    consistent.
    """
    if (
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @arcen/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@arcen" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/arcen-ink/package.json",
    "packages/arcen-ink/index.js",
    "packages/arcen-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ARCEN_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ARCEN_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ARCEN_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ARCEN_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    arcen_home = os.environ.get("ARCEN_HOME") or str(Path.home() / ".arcen")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ARCEN_HOME": arcen_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(arcen_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(arcen_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if arcen_cli_dir is None:
        arcen_cli_dir = Path(__file__).parent
    bundled = arcen_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ARCEN_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ARCEN_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from arcen_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ARCEN_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ARCEN_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ARCEN_TUI_DIR (e.g. `unset ARCEN_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    #    Termux scopes the install to ui-tui so launch only touches the TUI
    ...
    if (
        ...
        npm = _node_bin("npm")
        if not os.environ.get("ARCEN_QUIET"):
    ...
    if tui_dev:
        # Keep the local @arcen/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @arcen/ink resolves through
        # packages/arcen-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "arcen-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
    ...
    # Dev launches retain the historical "always rebuild" behaviour.
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from arcen_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="arcen-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ARCEN_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ARCEN_PYTHON_SRC_ROOT"] = os.environ.get(
        "ARCEN_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ARCEN_PYTHON", sys.executable)
    env.setdefault("ARCEN_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ARCEN_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ARCEN_MODEL"] = model
        env["ARCEN_INFERENCE_MODEL"] = model
    if provider:
        env["ARCEN_TUI_PROVIDER"] = provider
        env["ARCEN_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ARCEN_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ARCEN_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ARCEN_TUI_SKILLS"] = value
    if query:
        env["ARCEN_TUI_QUERY"] = query
    if image:
        env["ARCEN_TUI_IMAGE"] = image
    if checkpoints:
        env["ARCEN_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ARCEN_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ARCEN_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ARCEN_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ARCEN_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ARCEN_ACCEPT_HOOKS"] = "1"
    ...
    # ARCEN_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `arcen --tui` try to
    ...
    # still set ARCEN_TUI_RESUME themselves.
    env.pop("ARCEN_TUI_RESUME", None)
    if resume_session_id:
        env["ARCEN_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `arcen update` so
    ...
    if code == 42:
        from arcen_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ARCEN_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``arcen kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``arcen kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ARCEN_KANBAN_BOARD"):
        return
    try:
        from arcen_cli.kanban_db import get_current_board

        os.environ["ARCEN_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.arcen/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Arcen — chat and gateway. The skills_sync module is
    ...
    dependency. Arcen still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``ARCEN_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("ARCEN_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'arcen sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from arcen_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from arcen_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'arcen doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Arcen isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  arcen setup")
        print()

        from arcen_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'arcen setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from arcen_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ARCEN_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.arcen/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ARCEN_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ARCEN_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ARCEN_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from arcen_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from arcen_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from arcen_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Arcen?")
        print()
    ...
    # has no creds.json.  Every subsequent `arcen gateway` then paid a 30s
    ...
    session_dir = get_arcen_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: arcen gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `arcen gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  arcen gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Arcen Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  arcen gateway")
        ...
        print()
        print("  Or install as a service: arcen gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'arcen whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from arcen_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from arcen_cli.config import stamp_install_method
    from arcen_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Arcen post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from arcen_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``arcen model``) and the setup wizard
    ...
    from arcen_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ARCEN_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from arcen_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'arcen model' for "
                "available providers, or run 'arcen doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in arcen_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.arcen/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.arcen/.env if the active provider is not 'custom'.
    ...
    """
    from arcen_cli.config import get_env_value, save_env_value, load_config
...
#
# Arcen uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `arcen model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `arcen model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`arcen_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from arcen_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``arcen model`` flow, then route aux tasks to them here.
    """
    from arcen_cli.config import load_config
    from arcen_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from arcen_cli.auth import _prompt_model_selection
    from arcen_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from arcen_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from arcen_constants import OPENROUTER_BASE_URL
    from arcen_cli.auth import (
        ProviderConfig,
    ...
    from arcen_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.arcen/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `arcen setup` from scratch. OpenRouter
    ...
    from arcen_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from arcen_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `arcen auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from arcen_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from arcen_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``arcen model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``arcen model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``arcen auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from arcen_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from arcen_cli.auth import _save_model_choice, deactivate_provider
    from arcen_cli.config import get_env_value, load_config, save_config
    from arcen_cli.secret_prompt import masked_secret_prompt
    ...
    from arcen_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `arcen model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from arcen_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Arcen URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from arcen_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from arcen_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from arcen_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Arcen will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Arcen will install it now (the preflight below "
...
# downstream call sites read `arcen_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `arcen` invocation — including
# fast paths like `arcen --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    from arcen_cli.config import save_env_value, load_config, save_config
    from arcen_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from arcen_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from arcen_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from arcen_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Arcen turns to `copilot --acp`.")
    print("  Arcen currently starts its own ACP subprocess for each request.")
    print("  Arcen uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ARCEN_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``arcen setup`` / ``arcen model``.
    ...
    recover from a malformed paste without editing ``~/.arcen/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from arcen_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from arcen_cli.config import save_env_value
    from arcen_cli.secret_prompt import masked_secret_prompt
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `arcen setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from arcen_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so arcen knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Arcen typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Arcen, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from arcen_cli.auth import AuthError
        from arcen_cli.models import fetch_lmstudio_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from arcen_constants import display_arcen_home as _dhh_fn

            print(
                f"    Arcen will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from arcen_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'arcen model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Arcen CLI with a provider."""
    from arcen_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from arcen_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from arcen_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from arcen_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from arcen_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from arcen_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``arcen slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: arcen slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `arcen slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from arcen_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from arcen_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from arcen_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from arcen_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `arcen security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from arcen_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from arcen_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from arcen_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from arcen_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Arcen home directory to a zip file."""
    if getattr(args, "quick", False):
        from arcen_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from arcen_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore an Arcen backup from a zip file."""
    from arcen_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from arcen_cli.banner import check_for_updates
        from arcen_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Arcen Agent."""
    _require_tty("uninstall")
    from arcen_cli.uninstall import run_uninstall
...
# Critical files that every ``arcen`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``arcen update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "arcen_cli/main.py",
    "arcen_cli/config.py",
    "arcen_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``arcen`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="arcen-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``arcen update --gateway`` so interactive prompts (stash restore,
...
def _update_via_zip(args):
    """Update Arcen Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `arcen update "
            f"--branch {branch}`, or update against main with `arcen update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/arcen-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="arcen-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"arcen-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for arcen-agent itself
        ...
        # GitHub ZIPs extract to arcen-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"arcen-agent-{branch}")
    ...
    from arcen_cli.managed_uv import ensure_uv, update_managed_uv
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "arcen-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Arcen behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes arcen completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Arcen couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Arcen couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Arcen behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Arcen couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Arcen couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `arcen update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/arcen-agent.git",
    "git@github.com:NousResearch/arcen-agent.git",
    "https://github.com/NousResearch/arcen-agent",
    "git@github.com:NousResearch/arcen-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/arcen-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_arcen_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_arcen_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from NousResearch/arcen-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/arcen-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/arcen-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``arcen update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from arcen_constants import get_default_arcen_root

    default_home = get_default_arcen_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``arcen update`` is still progressing even if pip/uv itself is silent.
    """
...
def _arcen_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "arcen.exe",
        scripts_dir / "arcen-gateway.exe",
    ]


def _detect_concurrent_arcen_instances(
    ...
    Other Arcen processes can hold the same executable file during
    ``arcen update``. The quarantine rename then fails with ``[WinError 32]``
    and uv inherits the lock.

    This helper enumerates processes whose ``exe`` matches one of the venv's
    shims (``arcen.exe`` / ``arcen-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``arcen update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``arcen.exe``) is a separate process from the Python
    ...
    for shim in _arcen_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated arcen.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``arcen update``
    ...
    #      arcen.exe sitting *under* a non-Arcen parent must still be flagged,
    #      so we don't blanket-
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "arcen.exe"
    lines = ["✗ Another arcen.exe is running:"]
    ...
    lines.append("")
    lines.append("  Exit any open `arcen` REPLs and")
    lines.append("  stop the gateway (`arcen gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `arcen update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_arcen_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``arcen.exe``.
    ...
    when ``arcen update`` runs, ``arcen.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``arcen.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next arcen invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or another Arcen process (won't recover until
    the user closes it). We mitigate:
    ...
    2. If all retries fail, schedule the .exe for replacement on next
       ...
       the stale image.
    3. Print a clear warning naming the most likely culprit (running gateway /
       REPL) and pointing to ``--force``.
    ...
    for shim in _arcen_exe_shims(scripts_dir):
        ...
        print(
            "    Exit other `arcen` REPLs, stop the gateway, or pause AV "
            "scanning, then re-run `arcen update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_arcen_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``arcen.exe.old.*`` left by prior updates.

    Called early on every arcen invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``arcen update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `arcen update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``arcen.exe`` / ``arcen-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_arcen_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_arcen_exe(scripts_dir)
        try:
    ...
    # partial installs where a newly added base dep silently fails to land on
    # top of a half-stale venv, and the only symptom is a downstream subprocess
    # crashing with ModuleNotFoundError hours later. Reinstall with --reinstall to force resolution if anything is
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``arcen update``, not the venv we just wrote
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `arcen update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `arcen update --force` after closing other arcen processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from arcen_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ARCEN_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from arcen_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # workspaces. This terminal-focused tree only needs the TUI workspaces
    # during `arcen update`.
    ...
    if ws_result.returncode == 0:
        print("  ✓ repo root + ui-tui workspace")
    else:
...
class _UpdateOutputStream:
    """Stream wrapper used during ``arcen update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.arcen/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``arcen update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``arcen update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.arcen/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``arcen update --gateway``) the update is already
    ...
    try:
        ...
        # arcen_cli.config.get_arcen_home to simulate setup failure.
        from arcen_cli.config import get_arcen_home as _get_arcen_home

        logs_dir = _get_arcen_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``arcen update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from arcen_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``arcen update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from arcen_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from arcen_cli.config import recommended_update_command
        from arcen_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``arcen update`` without requiring a reinstall.  The
    ...
    ``arcen`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/arcen.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v arcen'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/arcen, code at /usr/local/lib/arcen-agent).
    fhs_link = Path("/usr/local/bin/arcen")
    ...
    path_comment = (
        "# Arcen Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ARCEN_HOME before running the update.
    ...
    ARCEN_HOME directories.  The ``--backup`` flag on ``arcen update``
    ...
    try:
        from arcen_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_arcen_home so the user sees ~/.arcen/...
    try:
        from arcen_constants import get_arcen_home, display_arcen_home

        home = get_arcen_home()
        try:
            display_path = f"{display_arcen_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  arcen import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``arcen update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Arcen Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Arcen Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `arcen update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Arcen via pip (for PyPI installs)."""
    from arcen_cli import __version__
    from arcen_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "arcen-agent"]
        ...
        cmd = [pipx, "upgrade", "arcen-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "arcen-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "arcen-agent"]
    ...
    print("✓ Update complete! Restart arcen to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # only non-interactive updates (gateway, `--yes`) consult
    ...
    print("⚕ Updating Arcen Agent...")
    print()

    # On Windows, abort early if another arcen.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_arcen_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $ARCEN_HOME, this is cheap
        ...
        try:
            from arcen_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in arcen_cli/config.py bricked
        # every user who ran ``arcen update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``arcen
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``arcen update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_arcen_home added to arcen_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload arcen_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_arcen_home() added since the last release.
        try:
            import importlib
            import arcen_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ARCEN_HOME so
        # it is not affected by sync_skills()'s module-level ARCEN_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ARCEN_HOME env var points at the default or a named profile.
        try:
            from arcen_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'arcen config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                # non-interactive actions cannot prompt for API keys.
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: arcen config migrate")
            else:
                print()
                print("Skipped. Run 'arcen config migrate' later to configure.")
        else:
        ...
        try:
            from arcen_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `arcen curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``arcen update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from arcen_cli.tools_config import install_cua_driver
        ...
        # When running as ``arcen update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_arcen_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all arcen-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. arcen-gateway.service or arcen-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: arcen gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: arcen -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Arcen gateway unit files are still installed.
        # When both arcen.service (from a pre-rename install) and the
        # current arcen-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `arcen update` surfaces the issue until the user migrates.
        try:
            from arcen_cli.gateway import (
                has_legacy_arcen_units,
                _find_legacy_arcen_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_arcen_units():
                print()
                print("⚠ Legacy Arcen gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_arcen_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (arcen.service) fight the current")
                print("  arcen-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    arcen gateway migrate-legacy")
                print()
        ...
        print("  arcen model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``arcen -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from arcen_constants import display_arcen_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `arcen profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_arcen_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → arcen -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.arcen)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  arcen profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     arcen -p {name} chat")
                else:
        ...
        from arcen_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from arcen_constants import get_arcen_home as _hh
        ...
        from arcen_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `arcen profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from arcen_cli.profiles import profile_exists
        ...
        from arcen_cli.profiles import rename_profile
        ...
        from arcen_cli.profiles import export_profile
        ...
        from arcen_cli.profiles import import_profile
        ...
        import tempfile
        from arcen_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="arcen_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  arcen -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      arcen -p {plan.manifest.name} chat")
        ...
        from arcen_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `arcen profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"arcen -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from arcen_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("arcen_requires"):
            print(f"Requires:     Arcen {data['arcen_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from arcen_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.arcen_requires:
        print(f"  Requires: Arcen {mf.arcen_requires}")
...
def cmd_completion(args, parser=None):
    ...
    from arcen_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from arcen_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Arcen log files."""
    from arcen_cli.logs import tail_log, list_logs
...
# so that in ``arcen -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``arcen_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``arcen -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``arcen`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ARCEN_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    try:
        from arcen_cli.plugins import discover_plugins
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            from arcen_cli.mcp_startup import start_background_mcp_discovery
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from arcen_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from arcen_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ARCEN_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ARCEN_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ARCEN_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `arcen --tui` is the hot path on phones. The full parser setup imports
...
def main():
    """Main entry point for arcen CLI."""
    # Cosmetic: make the process show up as 'arcen' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from arcen_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``arcen.exe.old.*`` quarantine files left by previous
    # ``arcen update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_arcen_exe``.
    try:
    ...
    # =========================================================================
    from arcen_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://arcen-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `arcen model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://arcen-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from arcen_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from arcen_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy arcen.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Arcen gateway unit files "
            "(e.g. arcen.service) left over from older installs. Profile "
            "units (arcen-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Arcen Agent with an interactive wizard. "
        "Run a specific section: arcen setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'arcen setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Arcen.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $ARCEN_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Arcen")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from arcen_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Arcen CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: arcen-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Arcen with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set ARCEN_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Arcen Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.arcen/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Arcen profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.arcen/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Arcen profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from arcen_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.arcen/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.arcen/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Arcen Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `arcen "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Arcen "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.arcen/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the Arcen Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Arcen setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Arcen Agent. Use 'arcen debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    arcen debug share              Upload debug report and print URL
    arcen debug share --lines 500  Include more log lines
    arcen debug share --expire 30  Keep paste for 30 days
    arcen debug share --local      Print report locally (no upload)
    arcen debug share --no-redact  Disable upload-time secret redaction
    arcen debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'arcen debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Arcen home directory to a zip file",
        description="Create a zip archive of your entire Arcen configuration, "
        "skills, sessions, and data (excludes the arcen-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/arcen-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.arcen/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo arcen uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from arcen_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore an Arcen backup from a zip file",
        description="Extract a previously created Arcen backup into your "
        "Arcen home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Arcen Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from arcen_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.arcen/skills/.bundled_manifest) "
            "so future 'arcen update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`arcen update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `arcen update`. Pass --sync to re-seed now."
        ),
    )
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from arcen_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from arcen_cli.skills_hub import skills_command
    ...
    from arcen_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/arcen-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `arcen plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from arcen_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``arcen --help``, ``arcen version``, ``arcen logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from arcen_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from arcen_cli.curator import register_cli as _register_curator_cli
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'arcen tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # arcen tools disable <name...> [--platform cli]
    ...
    # arcen tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from arcen_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from arcen_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `arcen computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `arcen tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: arcen computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: arcen computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Arcen as an MCP server",
        description=(
            "Manage MCP server connections and run Arcen as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'arcen mcp add' to connect to a new server, or\n"
            "'arcen mcp serve' to expose Arcen conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Arcen as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `arcen mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `arcen mcp install n8n`)",
    )
    ...
    def cmd_mcp(args):
        from arcen_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_arcen_home() / "sessions"
            ...
            # Launch arcen --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Arcen",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Arcen",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.arcen/ (by default a "
        "single restore-point archive is written to ~/.arcen/backups/ "
        "before apply; restorable with 'arcen import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from arcen_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Arcen Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'arcen config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, arcen will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another arcen.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Arcen Agent",
        description="Remove Arcen Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Arcen Agent as an ACP (Agent Client Protocol) server",
        description="Start Arcen Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Arcen ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Arcen provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.arcen/node/ "
    ...
    def cmd_acp(args):
        """Launch Arcen Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Arcen instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `arcen update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `arcen profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install an Arcen profile distribution. SOURCE can be a git URL "
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Arcen log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    arcen logs                    Show last 50 lines of agent.log
    arcen logs -f                 Follow agent.log in real time
    arcen logs errors             Show last 50 lines of errors.log
    arcen logs gateway -n 100     Show last 100 lines of gateway.log
    arcen logs --level WARNING    Only show WARNING and above
    arcen logs --session abc123   Filter by session ID
    arcen logs --component tools  Only show tool-related lines
    arcen logs --since 1h         Lines from the last hour
    arcen logs --since 30m -f     Follow, starting from 30 min ago
    arcen logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``arcen -c Pokemon Agent Dev`` → ``arcen -c 'Pokemon Agent Dev'``
    ...
    from arcen_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'arcen -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (arcen hooks list, cron
