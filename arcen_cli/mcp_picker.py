"""MCP picker — interactive `arcen mcp picker` (also the default `arcen mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``arcen mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `arcen plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from arcen_cli.cli_output import prompt_yes_no
from arcen_cli.curses_ui import curses_single_select
from arcen_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Arcen session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from arcen_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `arcen mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: arcen mcp install <name>    Picker: arcen mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Arcen is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Arcen — run `arcen update` "
...
def show_catalog() -> None:
    """`arcen mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`arcen mcp picker` (and default `arcen mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`arcen mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `arcen mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
