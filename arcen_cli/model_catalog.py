...
Arcen can read a JSON manifest of curated models for providers an operator
wants to update without shipping a release (currently OpenRouter and
Nous Portal). This module fetches, validates, and caches that manifest
when ``model_catalog.url`` is configured, falling back to the in-repo
hardcoded lists otherwise.

Pipeline
--------
1. ``get_catalog()`` — returns a parsed manifest dict.
   - Checks in-process cache (invalidated by TTL).
   - Reads disk cache at ``~/.arcen/cache/model_catalog.json``.
   - Fetches the configured URL if disk cache is stale or missing.
...
def _cache_path() -> Path:
    ...
    return get_arcen_home() / "cache" / "model_catalog.json"
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``arcen model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
