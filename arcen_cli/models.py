...
Add, remove, or reorder entries here — both `arcen setup` and
`arcen` provider-selection will pick up the change automatically.
"""
...
from arcen_cli import __version__ as _ARCEN_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_ARCEN_USER_AGENT = f"arcen-cli/{_ARCEN_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `arcen model`
    flow without maintaining a separate static list.
    """
    from arcen_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ARCEN_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ARCEN_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Arcen refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Arcen that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Arcen that doesn't ship
...
# this list:  arcen model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `arcen model` interactive picker
# ---------------------------------------------------------------------------
...
#
# Some vendors expose several Arcen provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `arcen model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``arcen model``, the
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``arcen auth add openai-codex`` without
    ``arcen model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``arcen model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    arcen-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from arcen_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — arcen-agent
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``arcen model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from arcen_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from arcen_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Arcen' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``arcen_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Arcen should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``arcen auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.arcen/.env``.
    ...
    """
    try:
        from arcen_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from arcen_cli.auth import read_credential_pool
        from arcen_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without an Arcen release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # an Arcen release). Falls back to the hardcoded catalog if no token
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``arcen model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $ARCEN_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_arcen_home() / "provider_models_cache.json"
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from arcen_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "ArcenAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ARCEN_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Arcen' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ARCEN_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from arcen_constants import get_arcen_home
    return get_arcen_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Arcen will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Arcen cannot verify the model name."
