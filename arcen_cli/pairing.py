...
Usage:
    arcen pairing list              # Show all pending + approved users
    arcen pairing approve <platform> <code>  # Approve a pairing code
    arcen pairing revoke <platform> <user_id> # Revoke user access
    arcen pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle arcen pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: arcen pairing {list|approve|revoke|clear-pending}")
        print("Run 'arcen pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.arcen/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'arcen pairing list' to see pending codes.\n")
