"""``arcen portal`` — the human-readable entry point for Nous Portal.

Running ``arcen portal`` with no subcommand performs the one-shot Portal
...
``arcen auth add nous --type oauth`` (which still works), is identical to
``arcen setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``arcen auth`` or ``arcen tools``. It's the onboarding + discovery
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `arcen model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `arcen portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `arcen auth add nous --type
    oauth`. It reuses the exact wiring behind `arcen setup --portal` (which in
    ...
    """
    from arcen_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `arcen portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `arcen portal` is the
        # human-readable alias for `arcen auth add nous --type oauth` /
        # `arcen setup --portal`.
    ...
    print("Run `arcen portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `arcen portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `arcen portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `arcen auth add "
            "nous --type oauth`, identical to `arcen setup --portal`). "
