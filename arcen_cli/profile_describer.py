...
Used by ``arcen profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``arcen_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Arcen Agent kanban board.
...
Rules:
  ...
  - Never write "Arcen Agent profile" or other meta-narration.
...
def describe_profile(
    ...
    *,
    ...
    try:
        if canon == "default":
            from arcen_constants import get_arcen_home  # type: ignore
            profile_dir = Path(get_arcen_home())
        else:
