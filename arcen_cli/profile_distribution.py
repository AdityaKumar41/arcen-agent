"""Profile distributions — shareable, packaged Arcen profiles via git.

A distribution is an Arcen profile published as a git repository (or
...
* ``arcen profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``arcen skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``arcen profile``, not a parallel tree):

    arcen profile install <source> [--name N] [--alias] [--force] [--yes]
    arcen profile update  <name>  [--force-config] [--yes]
    arcen profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "arcen-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            arcen_requires=str(data.get("arcen_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.arcen_requires:
            out["arcen_requires"] = self.arcen_requires
        if self.author:
...
def check_arcen_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Arcen {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Arcen distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``arcen profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not an Arcen profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not an Arcen distribution."
        )

    # Version check up-front so we fail fast
    check_arcen_requires(manifest.arcen_requires, arcen_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.arcen).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="arcen_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `arcen profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`arcen profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="arcen_dist_update_") as tmp:
        plan = plan_install(
