...
# xAI's public API is OpenAI-compatible for the endpoints Arcen commonly
# uses. The Responses endpoint is included because Arcen' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok vian Arcen-managed OAuth credentials."""

    auth_hint = "arcen auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`arcen auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`arcen auth reset xai-oauth` or re-authenticate with "
                    "`arcen auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `arcen auth add xai-oauth --type oauth`."
            )
