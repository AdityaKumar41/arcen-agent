"""CLI subcommand: ``arcen send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"arcen send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"arcen send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"arcen send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"arcen send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"arcen send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `arcen gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.arcen/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_arcen_env() -> None:
    """Populate ``os.environ`` from ``~/.arcen/.env`` AND bridge top-level
    ...
    startup that ``arcen send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.arcen/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.arcen/config.yaml`` into
       ...
       via ``arcen config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``arcen send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.arcen/.env and ~/.arcen/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "arcen send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  arcen send --to telegram \"hello\"\n"
            "  arcen send --to discord:#ops --file report.md\n"
            "  arcen send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "arcen send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `arcen send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Arcen "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.arcen/.env + ~/.arcen/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  arcen send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | arcen send --to telegram:-1001234567890\n"
            "  arcen send --to discord:#ops --file /tmp/report.md\n"
            "  arcen send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  arcen send --list                  # all platforms\n"
            "  arcen send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `arcen send --list telegram`.",
    )
