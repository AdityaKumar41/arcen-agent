...
the existing module-level functions in arcen_cli.gateway and
arcen_cli.gateway_windows directly. This protocol is a thin facade
...
``arcen gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``arcen gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged arcen
    ...
    detection always fails. Since every Arcen runtime call inside the
    container drops to arcen via ``s6-setuidgid``, that silent failure
...
# in ``arcen_cli.gateway`` (systemd/launchd) and ``arcen_cli.gateway_windows``
...
# active (set via the ``arcen -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in arcen_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from arcen_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from arcen_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from arcen_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from arcen_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in arcen_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from arcen_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from arcen_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from arcen_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from arcen_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``arcen_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from arcen_cli import gateway_windows
        from arcen_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `arcen profile create`
# runs inside the container (Phase 4). Static services (main-arcen, dashboard)
...
# UID/GID of the in-image ``arcen`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the arcen user.
    ...
    (the ``arcen`` user — UID 10000 — running every Arcen runtime
    ...
    is dispatched as the arcen user.
    ...
    with arcen ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           arcen:arcen, 0755 (parent must already exist)
    ``svc_dir/event/``                     arcen:arcen, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 arcen:arcen, 0755
    ``svc_dir/supervise/event/``           arcen:arcen, 03730
    ``svc_dir/supervise/control``          arcen:arcen, 0660    (FIFO)
    ...
    slot's supervise/ was arcen-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _ARCEN_UID, _ARCEN_GID)
        except PermissionError:
            # Running as the arcen user already — directory is arcen-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _ARCEN_UID, _ARCEN_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is arcen-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _ARCEN_UID, _ARCEN_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``arcen -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`arcen profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-arcen, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources ARCEN_HOME (and any extra env) via with-contenv —
             so e.g. ``-e ARCEN_HOME=/data/arcen`` is honored at run
          ...
          4. Drops to the arcen user and exec's
             ``arcen -p <profile> gateway run`` (or just ``arcen
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``arcen gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root ARCEN_HOME profile" (the implicit profile that exists at
        the top of $ARCEN_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``arcen_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$ARCEN_HOME/profiles/default/`` — a
        ...
        ``arcen_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        # guard.
        lines.append("export ARCEN_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            gateway_cmd = "arcen gateway run"
        else:
            gateway_cmd = f"arcen -p {shlex.quote(profile)} gateway run"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid arcen {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${ARCEN_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the ARCEN_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e ARCEN_HOME=/data/arcen``
        gets its logs under /data/arcen/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``arcen logs`` reads and what
        ...
        return (
            ...
            f': "${{ARCEN_HOME:=/opt/data}}"\n'
            f'log_dir="$ARCEN_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R arcen:arcen "$log_dir" 2>/dev/null || true\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid arcen s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with arcen ownership
            ...
            # the arcen user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is arcen-owned (see ``_seed_supervise_skeleton``), and on
