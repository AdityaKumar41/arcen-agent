"""
Interactive setup wizard for Arcen Agent.
...
_DOCS_BASE = "https://arcen-agent.nousresearch.com/docs"
...
# display_arcen_home imported lazily at call sites (stale-module safety during arcen update)
...
from arcen_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from arcen_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Arcen Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Arcen using environment variables or config commands:")
    print_info("  arcen config set model.provider custom")
    print_info("  arcen config set model.base_url http://localhost:8080/v1")
    print_info("  arcen config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'arcen setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from arcen_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'arcen setup')")


def _print_setup_summary(config: dict, arcen_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'arcen setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from arcen_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from arcen_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'arcen setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'arcen setup tts'"))
    else:
    ...
    # Spotify (OAuth via arcen auth spotify — check auth.json, not env vars)
    try:
        from arcen_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'arcen setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {arcen_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('arcen setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('arcen setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('arcen setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('arcen setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('arcen setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('arcen config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('arcen config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('arcen config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('arcen', Colors.GREEN)}              Start chatting")
    print(f"   {color('arcen gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('arcen doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `arcen tools` and `arcen setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``arcen model``)
    ...
    provider added to ``arcen model`` is automatically available here.
    ...
    # Delegate to the shared arcen model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from arcen_cli.main import select_provider_and_model
    try:
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``arcen model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from arcen_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Arcen auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via arcen setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'arcen setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Arcen runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `arcen setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `arcen setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ARCEN_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ARCEN_MAX_ITERATIONS")
    ...
    print_info("  Run `arcen setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ARCEN_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from arcen_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    ...
    try:
        ...
        if "/profiles/" in arcen_home:
            profile_name = arcen_home.rstrip("/").rsplit("/", 1)[-1]
    except Exception:
        pass
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Arcen delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `arcen update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://arcen-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under ARCEN_HOME, and print
    ...
    """
    try:
        from arcen_cli.slack_cli import _build_full_manifest
        from arcen_constants import get_arcen_home

        manifest = _build_full_manifest(
            bot_name="Arcen",
            bot_description="Your Arcen agent on Slack",
        )
        target = Path(get_arcen_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `arcen slack manifest --write` anytime to refresh after "
            "Arcen adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Arcen delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Arcen to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from arcen_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://arcen-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://arcen-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  arcen config edit")
    print_info("   Open config in your editor:  arcen config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from arcen_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Arcen from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'arcen setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     arcen config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_arcen_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo arcen gateway install --system")
                print_info("  Or run in foreground:  arcen gateway")
        else:
            from arcen_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   arcen gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   arcen gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `arcen setup tools` and `arcen tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from arcen_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from arcen_cli.config
    ...
    if section_key == "model":
        ...
        from arcen_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_arcen.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_arcen migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_arcen", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Arcen.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Arcen to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Arcen at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Arcen at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Arcen at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Arcen at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Arcen equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Arcen config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Arcen.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Arcen's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(arcen_home: Path) -> bool:
    ...
    print_info("Arcen can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: arcen claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=arcen_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: arcen claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Arcen configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Arcen config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Arcen (use arcen claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      arcen setup           — full or quick (auto-detected)
      arcen setup model     — just model/provider
      arcen setup tts       — just text-to-speech
      arcen setup terminal  — just terminal backend
      arcen setup gateway   — just messaging platforms
      arcen setup tools     — just tool configuration
      arcen setup agent     — just agent settings
    """
    from arcen_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Arcen Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Arcen Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Arcen Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, arcen_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Arcen configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'arcen setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(arcen_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, arcen_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {arcen_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'arcen config edit'")
    ...
    # Tune later with `arcen setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, arcen_home)
...
def _run_first_time_quick_setup(config: dict, arcen_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'arcen setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    arcen setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  arcen setup gateway")
    print()

    _print_setup_summary(config, arcen_home)


def _run_quick_setup(config: dict, arcen_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'arcen setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Arcen to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'arcen setup gateway'.")
