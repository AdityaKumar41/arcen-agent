...
on Japanese locales, etc.).  Arcen's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``arcen_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Arcen is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ARCEN_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ARCEN_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Arcen-managed Git install directories so
    ...
    # already-running shells don't see the change — which means arcen
    ...
    # install without Arcen-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Arcen's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Arcen's
    ``arcen config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Arcen (User env var in Windows Settings, or
    export in a PowerShell profile) and Arcen picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Arcen-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\arcen\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``arcen`` from right after install) retain their old PATH.

    Any subprocess Arcen spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Arcen-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "arcen", "git", "cmd"),
        os.path.join(local_appdata, "arcen", "git", "bin"),
        os.path.join(local_appdata, "arcen", "git", "usr", "bin"),
        # Arcen venv Scripts directory — host of the arcen.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes arcen, but harmless to include.
        os.path.join(local_appdata, "arcen", "arcen-agent", "venv", "Scripts"),
