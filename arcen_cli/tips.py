...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or arcen -c.",
    ...
    "Ctrl+Z suspends Arcen to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "arcen -c resumes your most recent CLI session. arcen -c \"project name\" resumes by title.",
    "arcen -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "arcen -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "arcen chat -t web,terminal enables only specific toolsets for a focused session.",
    "arcen chat -s github-pr-workflow preloads a skill at launch.",
    "arcen chat -q \"query\" runs a single non-interactive query and exits.",
    "arcen chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "arcen chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "arcen --yolo bypasses all dangerous command approval prompts for the entire session.",
    "arcen chat --source telegram tags the session for filtering in arcen sessions list.",
    "arcen -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "arcen doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "arcen dump outputs a compact setup summary — great for bug reports.",
    "arcen config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "arcen config edit opens config.yaml in your default editor.",
    "arcen config check scans for missing or stale configuration options.",
    "arcen sessions browse opens an interactive session picker with search.",
    "arcen sessions stats shows session counts by platform and database size.",
    "arcen sessions prune --older-than 30 cleans up old sessions.",
    "arcen skills search react --source skills-sh searches the skills.sh public directory.",
    "arcen skills check scans installed hub skills for upstream updates.",
    "arcen skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "arcen skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "arcen mcp add github --command npx adds MCP servers from the command line.",
    "arcen mcp serve runs Arcen itself as an MCP server for other agents.",
    "arcen auth add lets you add multiple API keys for credential pool rotation.",
    "arcen completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "arcen logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "arcen backup creates a zip backup of your entire Arcen home directory.",
    "arcen profile create coder creates an isolated profile that becomes its own command.",
    "arcen profile create work --clone copies your current config and keys to a new profile.",
    "arcen update syncs new bundled skills to ALL profiles automatically.",
    "arcen gateway install sets up Arcen as a system service (systemd/launchd).",
    "arcen memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "arcen webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: arcen tools disables unused tools, arcen skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "arcen models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as arcen -w).",
    ...
    "ARCEN_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Arcen tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'arcen profile create coder' creates the 'coder' command.",
    "arcen profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Arcen prints a resume command with session ID and stats.",
    "arcen sessions export backup.jsonl exports all sessions for backup or analysis.",
    "arcen -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "arcen skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .arcen/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: arcen cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Arcen runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "arcen gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Arcen via a local macOS server.",
    ...
    "Context files (.arcen.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Arcen loads project context from .arcen.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "arcen mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "arcen mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "arcen mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — arcen tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.arcen/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "arcen chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "arcen plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "arcen acp runs Arcen as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ARCEN_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "arcen auth add nous or arcen auth add openai-codex sets up OAuth-based providers.",
    ...
    "arcen status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.arcen/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Arcen your own.",
    ...
    "Before a gateway session reset, Arcen auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.arcen/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at ARCEN_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "ARCEN_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ARCEN_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.arcen/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'arcen profile create ls' would be rejected.",
    "arcen profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "arcen claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ARCEN_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via arcen tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.arcen/.env.",
    "Skills from trusted repos (ArcenPay) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.arcen/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Arcen auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Arcen config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Arcen is working.',
    ...
    '/reload reloads ~/.arcen/.env into the running session — pick up new API keys without restarting.',
    ...
    "ARCEN_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.arcen/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.arcen/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'arcen curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "arcen curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'arcen curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'arcen auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI ---
    'ARCEN_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ARCEN_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'ARCEN_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'ARCEN_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'ARCEN_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ARCEN_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.arcen/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.arcen/skills/ so drop-in skills appear without restarting the session.',
    ...
    'arcen -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'arcen chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'arcen chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'arcen chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "arcen chat --source tool tags programmatic chats so they don't clutter arcen sessions list.",
    'arcen dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'arcen sessions rename <ID> "new title" renames any past session; arcen sessions delete <ID> removes one.',
    'arcen import restores a session export or profile archive produced by sessions export or profile export.',
    'arcen fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'arcen pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'arcen setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'arcen status --deep runs the full health sweep across every component; plain arcen status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'ARCEN_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ARCEN_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.arcen/plugins/ — trust-gated by design.',
    "ARCEN_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ARCEN_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ARCEN_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ARCEN_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ARCEN_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ARCEN_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ARCEN_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'ARCEN_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ARCEN_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ARCEN_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ARCEN_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Arcen block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from arcen sessions list by default — set --source explicitly to see them.',
    ...
    'Plugins can live in ~/.arcen/plugins/ and can register tools, hooks, and CLI commands.',
]
