"""
Arcen Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.arcen/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Arcen PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing arcen-agent or arcen PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Arcen Agent" comment and following line
                if '# Arcen Agent' in line or '# arcen-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('arcen' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing arcen
                if 'arcen' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the arcen wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "arcen",
        Path("/usr/local/bin/arcen"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains arcen_cli reference)
                content = wrapper.read_text()
                if 'arcen_cli' in content or 'arcen-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(arcen_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``arcen`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Arcen home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (arcen_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``arcen gateway run`` processes
    ...
    try:
        from arcen_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from arcen_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ARCEN_HOME`` and ``ARCEN_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\arcen\git\cmd``, ``%LOCALAPPDATA%\arcen\git\bin``,
#      ``%LOCALAPPDATA%\arcen\git\usr\bin``, ``%LOCALAPPDATA%\arcen\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\arcen\git\`` and Node to
#      ``%LOCALAPPDATA%\arcen\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the gateway service path, drops files into
#      ``%LOCALAPPDATA%\arcen\gateway-service\`` and sometimes
...
def _arcen_path_markers(arcen_home: Path) -> list[str]:
    """Path-entry substrings that identify Arcen-owned User-PATH entries."""
    root = str(arcen_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare arcen-agent install dir.
    markers = [root + "\\arcen-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ARCEN_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "arcen".  We don't want to catch
    # unrelated entries like "carcen-foo" or "ephermeral", so we look for
    # backslash-arcen as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(arcen_home: Path) -> list[str]:
    """Strip Arcen-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _arcen_path_markers(arcen_home)
            kept: list[str] = []
...
def remove_arcen_env_vars_windows() -> list[str]:
    """Delete ARCEN_HOME and ARCEN_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ARCEN_HOME", "ARCEN_GIT_BASH_PATH"):
                try:
...
def _is_default_arcen_home(arcen_home: Path) -> bool:
    """Return True when ``arcen_home`` points at the default (non-profile) root."""
    try:
        from arcen_constants import get_default_arcen_root
        return arcen_home.resolve() == get_default_arcen_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ARCEN_HOME directory.

    We shell out to ``arcen -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ARCEN_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m arcen_cli.main` so we don't depend on a `arcen`
    #    wrapper that may be half-removed mid-uninstall.
    arcen_invocation = [_sys.executable, "-m", "arcen_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                arcen_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ARCEN_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.arcen/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.arcen/ for future reinstall
    """
    project_root = get_project_root()
    arcen_home = get_arcen_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ARCEN_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_arcen_home(arcen_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Arcen Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {arcen_home / 'config.yaml'}")
    print(f"  Secrets: {arcen_home / '.env'}")
    print(f"  Data:    {arcen_home / 'cron/'}, {arcen_home / 'sessions/'}, {arcen_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ARCEN_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Arcen data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Arcen code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in arcen_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\arcen\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(arcen_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Arcen-owned PATH entries in User environment")

        log_info("Removing ARCEN_HOME / ARCEN_GIT_BASH_PATH User env vars...")
        removed_env = remove_arcen_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Arcen-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing arcen command...")
    ...
    #     (only when they still point into this Arcen home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Arcen-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(arcen_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Arcen-managed node/npm/npx symlinks found")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.arcen/, just remove the arcen-agent subdir
            if arcen_home in project_root.parents or project_root.parent == arcen_home:
    ...
    #     under ARCEN_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(arcen_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(arcen_home)
    ...
    # 5. Optionally remove ~/.arcen/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ARCEN_HOME dirs live under
        ...
        try:
            if arcen_home.exists():
                shutil.rmtree(arcen_home)
                log_success(f"Removed {arcen_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {arcen_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {arcen_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://arcen-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://arcen-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Arcen Agent! ⚕")
    print()
