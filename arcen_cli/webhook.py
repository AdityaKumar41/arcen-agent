"""arcen webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    arcen webhook subscribe <name> [options]
    arcen webhook list
    arcen webhook remove <name>
    arcen webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.arcen/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _arcen_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: arcen gateway run
"""
...
def webhook_command(args):
    """Entry point for 'arcen webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: arcen webhook {subscribe|list|remove|test}")
        print("Run 'arcen webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (arcen gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: arcen webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from arcen webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (arcen gateway run)")
