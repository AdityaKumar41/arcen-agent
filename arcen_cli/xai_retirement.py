...
Pure logic: walks an Arcen config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `arcen doctor` and a future `arcen migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in an Arcen config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in an Arcen config and return retirement issues.
