"""Centralized logging setup for Arcen Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.arcen/logs/`` (profile-aware via ``get_arcen_home()``).
...
from arcen_constants import get_config_path, get_arcen_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_arcen_session_injector", False):
    ...
    _session_record_factory._arcen_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``arcen logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "arcen_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Arcen logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    arcen_home
        Override for the Arcen home directory.  Falls back to
        ``get_arcen_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_arcen_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._arcen_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the arcen group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from arcen_cli.config import is_managed
