...
#
# Wired into the image as /etc/cont-init.d/01-arcen-setup by the
...
ARCEN_HOME="${ARCEN_HOME:-/opt/data}"
INSTALL_DIR="/opt/arcen"

# Drop to arcen via s6-setuidgid, but skip it when already non-root.
as_arcen() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid arcen "$@"; }
...
# the container starts non-root. The baked image trees (/opt/data, /opt/arcen/
# .venv, ui-tui, node_modules) stay owned by the arcen build UID (10000), so an
...
# into $ARCEN_HOME). See #34837 for the supervision-tree side of this.
#
# The supported way to match host-side ownership is to start as root (the image
# default) and pass ARCEN_UID/ARCEN_GID — or the PUID/PGID aliases — which the
...
# `id -u arcen` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# arcen UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u arcen)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-arcen UID).
...
and the baked image directories are owned by the arcen user (UID $(id -u arcen)),
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e ARCEN_UID=\$(id -u) -e ARCEN_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the arcen user to that UID/GID at boot and chowns the data
...
# --- Bootstrap ARCEN_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid arcen mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ARCEN_HOME paths whose parents only
# root can create (e.g. `ARCEN_HOME=/home/arcen/.arcen` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ARCEN_UID/ARCEN_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ARCEN_UID/
# ARCEN_GID still win when both are set.  See #15290, salvages #25872.
ARCEN_UID="${ARCEN_UID:-${PUID:-}}"
ARCEN_GID="${ARCEN_GID:-${PGID:-}}"

if [ -n "${ARCEN_UID:-}" ] && validate_uid_gid "$ARCEN_UID" && [ "$ARCEN_UID" != "$(id -u arcen)" ]; then
    echo "[stage2] Changing arcen UID to $ARCEN_UID"
    usermod -u "$ARCEN_UID" arcen
fi
if [ -n "${ARCEN_GID:-}" ] && validate_uid_gid "$ARCEN_GID" && [ "$ARCEN_GID" != "$(id -g arcen)" ]; then
    echo "[stage2] Changing arcen GID to $ARCEN_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ARCEN_GID" arcen 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised arcen user (UID 10000)
...
# arcen process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes arcen. Idempotent across container
...
#   - arcen is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G arcen 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] arcen already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" arcen 2>/dev/null; then
        echo "[stage2] Added arcen to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group arcen failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ARCEN_UID is remapped or the top-level $ARCEN_HOME isn't owned by
# the runtime arcen UID, restore ownership to arcen — but ONLY for the
# directories arcen actually writes to. The full $ARCEN_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of arcen-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_arcen_uid=$(id -u arcen)
needs_chown=false
if [ "$(stat -c %u "$ARCEN_HOME" 2>/dev/null)" != "$actual_arcen_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ARCEN_HOME (targeted) to arcen ($actual_arcen_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ARCEN_HOME: chown the directory itself (not its contents)
    # so arcen can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown arcen:arcen "$ARCEN_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $ARCEN_HOME failed (rootless container?) — continuing"
    # Arcen-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by arcen (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$ARCEN_HOME/$sub" ]; then
            chown -R arcen:arcen "$ARCEN_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $ARCEN_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi

# --- Fix ownership of build trees under $INSTALL_DIR ---
# Arcen-owned trees under $INSTALL_DIR must be re-chowned whenever the
# runtime arcen UID no longer owns them — otherwise:
...
#     the source mtime is newer than dist/ or when ARCEN_TUI_FORCE_BUILD
#     is set) and writes to ui-tui/dist/. Without this chown the new
#     arcen UID can't write the build output (#28851).
...
# The set mirrors the build-time `chown -R arcen:arcen` line in the
# Dockerfile — keep them in sync if the Dockerfile chown set changes.
# These are under $INSTALL_DIR (not $ARCEN_HOME), so the bind-mount
# concern doesn't apply — recursive is fine.
#
# This MUST be gated independently of the $ARCEN_HOME ownership check
# above. `usermod -u <new> arcen` re-chowns the arcen home dir
# ($ARCEN_HOME == /opt/data) to the new UID as a side effect, so after a
# ARCEN_UID/PUID remap `stat $ARCEN_HOME` always already matches the new
# UID and `needs_chown` is false — but the build trees under /opt/arcen
# are NOT touched by usermod and remain owned by the build-time UID
# (10000). Gating them on $ARCEN_HOME ownership (as #35027 did) silently
...
# when the venv is not already owned by the runtime arcen UID. Idempotent
...
if [ -n "$venv_owner" ] && [ "$venv_owner" != "$actual_arcen_uid" ]; then
    echo "[stage2] Fixing ownership of build trees under $INSTALL_DIR to arcen ($actual_arcen_uid)"
    chown -R arcen:arcen \
        "$INSTALL_DIR/.venv" \
        "$INSTALL_DIR/ui-tui" \
        "$INSTALL_DIR/gateway" \
...
# Always reset ownership of $ARCEN_HOME/profiles to arcen on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> arcen …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as arcen and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$ARCEN_HOME/profiles" ]; then
    chown -R arcen:arcen "$ARCEN_HOME/profiles" 2>/dev/null || true
fi

# Reset ownership of arcen-owned top-level state files on every boot.
# The targeted data-volume chown above only covers arcen-owned
# *subdirectories*; loose state files living directly under $ARCEN_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> arcen …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged arcen runtime then hits
...
# sweep so host-owned files in a bind-mounted $ARCEN_HOME are never
...
# entries of arcen_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ARCEN_HOME/$f" ]; then
        chown arcen:arcen "$ARCEN_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the arcen runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ARCEN_HOME/config.yaml" ]; then
    chown arcen:arcen "$ARCEN_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$ARCEN_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as arcen user ---
# Run as arcen via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ARCEN_HOME values
...
as_arcen mkdir -p \
    ...
    "$ARCEN_HOME/skins" \
    "$ARCEN_HOME/plans" \
    "$ARCEN_HOME/workspace" \
    "$ARCEN_HOME/home" \
    "$ARCEN_HOME/pairing" \
    "$ARCEN_HOME/platforms/pairing"

# --- Install-method stamp (read by detect_install_method() in arcen status) ---
# Preserved from the tini-era entrypoint (PR #27843). Must be written as
# the arcen user so ownership matches the file's documented owner.
# tee is invoked directly via s6-setuidgid (no `sh -c` wrapper) for the
# same shell-metacharacter safety described above.
printf 'docker\n' | as_arcen tee "$ARCEN_HOME/.install_method" >/dev/null \
    || true
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ARCEN_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_arcen cp "$INSTALL_DIR/$src" "$ARCEN_HOME/$dest"
    fi
}
...
if [ -f "$ARCEN_HOME/.env" ]; then
    chown arcen:arcen "$ARCEN_HOME/.env" 2>/dev/null || true
    chmod 600 "$ARCEN_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $ARCEN_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `arcen update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ARCEN_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ARCEN_HOME/config.yaml" ]; then
    s6-setuidgid arcen "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ARCEN_HOME/auth.json" ] && [ -n "${ARCEN_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$ARCEN_AUTH_JSON_BOOTSTRAP" > "$ARCEN_HOME/auth.json"
    chown arcen:arcen "$ARCEN_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$ARCEN_HOME/auth.json"
fi
...
# can set ARCEN_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ARCEN_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ARCEN_HOME/gateway_state.json" ] && \
        [ "${ARCEN_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$ARCEN_HOME/gateway_state.json"
    chown arcen:arcen "$ARCEN_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$ARCEN_HOME/gateway_state.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_arcen "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/arcen/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Arcen spawns for the
...
# main-wrapper.sh propagates it into the supervised ``arcen`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-arcen, dashboard, etc.).
