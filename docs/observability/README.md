# Arcen Observer Hooks

Arcen observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Arcen' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Arcen catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Arcen only builds
