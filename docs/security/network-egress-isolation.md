...
When running Arcen inside Docker, the default `network_mode: host` gives the
...
The Arcen [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ARCEN_UID=$(id -u) ARCEN_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=arcen,arcen-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://arcen-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Arcen trust model and vulnerability reporting
