...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_arcen_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.arcen/config.yaml (primary user-facing config)
    3. ~/.arcen/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_arcen_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``arcen config set gateway.streaming.*``
