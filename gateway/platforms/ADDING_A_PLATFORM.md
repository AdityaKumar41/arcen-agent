...
There are two ways to add a platform to the Arcen gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.arcen/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Arcen code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `arcen gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `arcen_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Arcen core.
...
And add it to the `arcen-gateway` composite:

```python
"arcen-gateway": {
    "includes": [..., "arcen-your-platform"]
}
```
...
## 12. Status Display (`arcen_cli/status.py`)
...
## 13. Gateway Setup Wizard (`arcen_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ arcen_cli/ toolsets.py \
  --include="*.py" -l | sort -u
