...
# Audio file extensions Arcen recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Arcen' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Arcen-created Telegram private-chat
...
from arcen_constants import get_default_arcen_root, get_arcen_dir, get_arcen_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.arcen/.env manually."
)
...
# Default location: {ARCEN_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_arcen_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; ArcenAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_arcen_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_arcen_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_arcen_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_arcen_dir("cache/screenshots", "browser_screenshots")
_ARCEN_HOME = get_arcen_home()
_ARCEN_ROOT = get_default_arcen_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "ARCEN_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "ARCEN_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "ARCEN_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "ARCEN_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _ARCEN_HOME / "image_cache",
    _ARCEN_HOME / "audio_cache",
    _ARCEN_HOME / "video_cache",
    _ARCEN_HOME / "document_cache",
    _ARCEN_HOME / "browser_screenshots",
    ...
    _ARCEN_HOME / "cache" / "images",
    _ARCEN_HOME / "cache" / "audio",
    _ARCEN_HOME / "cache" / "videos",
    _ARCEN_HOME / "cache" / "documents",
    _ARCEN_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Arcen profile and shared Arcen root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for arcen_root in (_ARCEN_HOME, _ARCEN_ROOT):
        denied.append(arcen_root / ".env")
        denied.append(arcen_root / "auth.json")
        denied.append(arcen_root / "credentials")
        denied.append(arcen_root / "config.yaml")
    return denied
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Arcen
    secrets (``~/.arcen/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``ARCEN_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Arcen-managed cache, under an operator-allowlisted root
    (``ARCEN_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.arcen/.env,
    # ~/.arcen/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?arcen\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+arcen[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("ARCEN_GATEWAY_BUSY_TEXT_MODE", "queue").strip().lower()
            or "queue"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "ARCEN_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "ARCEN_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.arcen/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from arcen_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ARCEN_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ARCEN_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ARCEN_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("ARCEN_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("ARCEN_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("ARCEN_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _arcen_run_generation is set on
