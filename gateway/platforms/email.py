"""
Email platform adapter for the Arcen gateway.

Allows users to interact with Arcen by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "arcen-agent" "version" "{_arcen_version}" '
            '"vendor" "ArcenPay" '
            '"support-email" "noreply@arcenpay.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Arcen Agent")
        ...
        msg_id = f"<arcen-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
