...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Arcen command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Arcen accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Arcen ``/command`` text."""
...
# Uses get_arcen_home() so each profile gets its own Matrix store.
from arcen_constants import get_arcen_dir as _get_arcen_dir

_STORE_DIR = _get_arcen_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("ARCEN_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("ARCEN_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    async def connect(self) -> bool:
        ...
        if self._encryption:
            ...
            try:
                ...
                _acct_id = self._user_id or "arcen"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Arcen Agent" become "Agent".
        """
        if not body:
            return ""
