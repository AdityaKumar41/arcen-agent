...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_arcen_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /arcen prefix). A single regex
            ...
            # manifest (see `arcen slack manifest`). In Socket Mode, Slack
            ...
            from arcen_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/arcen$")

            @self._app.command(_slash_pattern)
            async def handle_arcen_command(ack, command):
            ...
            for _action_id in (
                "arcen_approve_once",
                "arcen_approve_session",
                "arcen_approve_always",
                "arcen_deny",
            ):
            ...
            for _action_id in (
                "arcen_confirm_once",
                "arcen_confirm_always",
                "arcen_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Arcen handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Arcen session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "arcen_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "arcen_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "arcen_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "arcen_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "arcen_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "arcen_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "arcen_confirm_once": "once",
            "arcen_confirm_always": "always",
            "arcen_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "arcen_approve_once": "once",
            "arcen_approve_session": "session",
            "arcen_approve_always": "always",
            "arcen_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/arcen <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/arcen
        ...
        if slash_name in {"arcen", ""}:
            # Legacy /arcen <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from arcen_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/arcen   ``).
        ...
        # "Running /cmd…" ack shown by handle_arcen_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/arcen <question>" must produce public replies so
