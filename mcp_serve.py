"""
Arcen MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Arcen-specific extra)

Usage:
    arcen mcp serve
    arcen mcp serve --verbose
...
logger = logging.getLogger("arcen.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ARCEN_HOME."""
    try:
        from arcen_constants import get_arcen_home
        return get_arcen_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ARCEN_HOME", Path.home() / ".arcen")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from arcen_constants import get_arcen_home
        directory_file = get_arcen_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ARCEN_HOME", Path.home() / ".arcen")
...
class EventBridge:
    ...
    This is the Arcen equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from arcen_constants import get_arcen_home
            db_file = get_arcen_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ARCEN_HOME", Path.home() / ".arcen")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Arcen MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "arcen",
        instructions=(
            "Arcen Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Arcen MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
