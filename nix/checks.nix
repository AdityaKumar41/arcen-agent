...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      arcen-agent = self'.packages.default;
      arcenVenv = arcen-agent.arcenVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "arcen-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${arcenVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "arcen-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "arcen-build-package" { } ''
          echo "PASS: package built at ${arcen-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "arcen-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "arcen-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${arcen-agent}/bin/arcen || (echo "FAIL: arcen binary missing"; exit 1)
          test -x ${arcen-agent}/bin/arcen-agent || (echo "FAIL: arcen-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${arcen-agent}/bin/arcen version 2>&1 | grep -qi "arcen" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "arcen-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in arcen arcen-agent arcen-acp; do
            test -x ${arcen-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "arcen-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking arcen --help ==="
          ${arcen-agent}/bin/arcen --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${arcen-agent}/bin/arcen --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "arcen-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${arcen-agent}/share/arcen-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${arcen-agent}/share/arcen-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ARCEN_BUNDLED_SKILLS" ${arcen-agent}/bin/arcen || \
            (echo "FAIL: ARCEN_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ARCEN_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "arcen-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${arcen-agent}/share/arcen-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${arcen-agent}/share/arcen-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ARCEN_BUNDLED_PLUGINS" ${arcen-agent}/bin/arcen || \
            (echo "FAIL: ARCEN_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ARCEN_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "arcen-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${arcen-agent}/share/arcen-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${arcen-agent}/share/arcen-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${arcen-agent}/share/arcen-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ARCEN_BUNDLED_LOCALES" ${arcen-agent}/bin/arcen || \
            (echo "FAIL: ARCEN_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ARCEN_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (ARCEN_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ARCEN_BUNDLED_LOCALES=${arcen-agent}/share/arcen-agent/locales \
            ${arcenVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ARCEN_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${arcenVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${arcenVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without ARCEN_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "arcen-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${arcen-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${arcen-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ARCEN_TUI_DIR" ${arcen-agent}/bin/arcen || \
            (echo "FAIL: ARCEN_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ARCEN_TUI_DIR set in wrapper"
        ...
        # Verify ARCEN_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        arcen-node = pkgs.runCommand "arcen-node-version" { } ''
          set -e
          echo "=== Checking ARCEN_NODE in wrapper ==="
          grep -q "ARCEN_NODE" ${arcen-agent}/bin/arcen || \
            (echo "FAIL: ARCEN_NODE not set in wrapper"; exit 1)
          echo "PASS: ARCEN_NODE present in wrapper"

          ARCEN_NODE=$(sed -n "s/^export ARCEN_NODE='\(.*\)'/\1/p" ${arcen-agent}/bin/arcen)
          test -x "$ARCEN_NODE" || (echo "FAIL: ARCEN_NODE=$ARCEN_NODE not executable"; exit 1)
          echo "PASS: ARCEN_NODE executable at $ARCEN_NODE"

          NODE_MAJOR=$("$ARCEN_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ARCEN_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ARCEN_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "arcen-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ARCEN_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ARCEN_MANAGED guards ==="
          check_blocked "config set" ${arcen-agent}/bin/arcen config set model foo
          check_blocked "config edit" ${arcen-agent}/bin/arcen config edit
        ...
        extra-python-packages = let
          ...
          arcenWithExtra = arcen-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "arcen-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${arcenWithExtra}/bin/arcen || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${arcenWithExtra}/bin/arcen || \
          ...
          if grep -q "PYTHONPATH" ${arcen-agent}/bin/arcen; then
        ...
        extra-dependency-groups = let
          arcenWithGroups = arcen-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "arcen-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${arcenWithGroups}"
          echo "venv: ${arcenWithGroups.arcenVenv}"
        ...
        messaging-variant = pkgs.runCommand "arcen-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.arcenVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "arcen-config-roundtrip" {
          ...
          merge_and_load() {
            local arcen_home="$1"
            export ARCEN_HOME="$arcen_home"
            ${configMergeScript} ${nixSettings} "$arcen_home/config.yaml"
            ${arcenVenv}/bin/python3 -c '
import json, sys
