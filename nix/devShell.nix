...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      arcenNpmLib = self'.packages.default.passthru.arcenNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. arcen-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        ...
        shellHook = ''
          echo "Arcen Agent dev shell"
          ${combinedNonNpm}
          ${arcenNpmLib.mkNpmDevShellHook npmPackageJsonPaths fixLockfilesExe}
          echo "Ready. Run 'arcen' to start."
        '';
      };
    };
}
