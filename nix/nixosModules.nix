# nix/nixosModules.nix — NixOS module for arcen-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: arcen runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ARCEN_HOME/.env
# and read by arcen at startup — no container recreation needed for env changes.
#
# Tool resolution: the arcen wrapper uses --suffix PATH for nix store tools,
...
#
# Usage:
#   services.arcen-agent = {
#     enable = true;
#     settings.model = "anthropic/claude-sonnet-4";
#     environmentFiles = [ config.sops.secrets."arcen/env".path ];
#   };
#
{ inputs, ... }: {
  ...
  let
    cfg = config.services.arcen-agent;
    effectivePackage =
    ...
    arcen-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-arcen-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "arcen-config-attrs";
      description = "Arcen YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "arcen-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "arcen-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ARCEN_DOC_EOF'\n${value}\nARCEN_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "arcen-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/arcen";
    ...
    # arcen user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "arcen-container-entrypoint" ''
      set -eu

      ARCEN_UID="''${ARCEN_UID:?ARCEN_UID must be set}"
      ARCEN_GID="''${ARCEN_GID:?ARCEN_GID must be set}"

      # ── Group: ensure a group with GID=$ARCEN_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ARCEN_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="arcen"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ARCEN_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ARCEN_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ARCEN_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ARCEN_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="arcen"
        TARGET_HOME="/home/arcen"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ARCEN_UID" -g "$ARCEN_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ARCEN_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ARCEN_UID:$ARCEN_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ARCEN_HOME is owned by the target user.
      ...
      if [ -n "''${ARCEN_HOME:-}" ] && [ -d "$ARCEN_HOME" ]; then
        find "$ARCEN_HOME" \! -user "$ARCEN_UID" -exec chown "$ARCEN_UID:$ARCEN_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/arcen-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/arcen-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/arcen ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/arcen
        chmod 0440 /etc/sudoers.d/arcen
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ARCEN_UID" --regid="$ARCEN_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ARCEN_HOME/.env.
    ...
    # Default: /var/lib/arcen/workspace → /data/workspace.
  ...
  in {
    options.services.arcen-agent = with lib; {
      enable = mkEnableOption "Arcen Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = arcen-agent;
        description = "The arcen-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/arcen";
        description = "State directory. Contains .arcen/ subdir (ARCEN_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Arcen config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ARCEN_HOME/.env at activation time.
          Arcen reads this file on every startup via load_arcen_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ARCEN_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ARCEN_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `arcen gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the arcen user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the arcen plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Arcen discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          arcen_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as arcen (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in arcen-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the arcen CLI to environment.systemPackages and export
          ARCEN_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.arcen symlink to the service
            stateDir. These users are automatically added to the arcen group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.arcen-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the arcen CLI to system PATH and export ARCEN_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.arcen/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ARCEN_HOME = "${cfg.stateDir}/.arcen";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.arcen-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the arcen user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.arcen-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed arcen
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure arcen is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.arcen        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.arcen/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.arcen/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.arcen/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.arcen/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.arcen/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."arcen-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.arcen
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.arcen ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.arcen ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.arcen -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.arcen/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.arcen/$_subdir"
            chmod 2770 "${cfg.stateDir}/.arcen/$_subdir"
            find "${cfg.stateDir}/.arcen/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.arcen/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.arcen/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.arcen/config.yaml
            chmod 0640 ${cfg.stateDir}/.arcen/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.arcen/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.arcen/.managed
          chmod 0644 ${cfg.stateDir}/.arcen/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.arcen/.container-mode <<'ARCEN_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    arcen_bin=${containerDataDir}/current-package/bin/arcen
    ARCEN_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.arcen/.container-mode
            chmod 0644 ${cfg.stateDir}/.arcen/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.arcen/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.arcen";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.arcen" ]; then
                  rm -f "${symlinkPath}"
                  echo "arcen-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.arcen -> stateDir/.arcen for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.arcen";
                target = "${cfg.stateDir}/.arcen";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "arcen-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.arcen/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.arcen/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.arcen/auth.json
              fi
            ''}
          ''}
          ...
          # Arcen reads $ARCEN_HOME/.env at startup via load_arcen_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.arcen/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ARCEN_NIX_ENV_EOF'
    ${envFileContent}
    ARCEN_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.arcen/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.arcen/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.arcen/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.arcen-agent = {
          description = "Arcen Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $ARCEN_HOME/.env by the activation script. load_arcen_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/arcen"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the arcen group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.arcen-agent = {
          description = "Arcen Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ARCEN_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ARCEN_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ARCEN_UID="$ARCEN_UID" \
                --env ARCEN_GID="$ARCEN_GID" \
                --env ARCEN_HOME=${containerDataDir}/.arcen \
                --env ARCEN_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/arcen gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
