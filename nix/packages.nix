# nix/packages.nix — Arcen Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    { pkgs, lib, inputs', ... }:
    let
      arcenAgent = pkgs.callPackage ./arcen-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = arcenAgent.arcenTui;

        fix-lockfiles = arcenAgent.arcenNpmLib.mkFixLockfiles { attr = "tui"; };
      };
    };
}
