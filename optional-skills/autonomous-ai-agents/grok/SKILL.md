...
version: 0.1.0
author: Matt Maximo (MattMaximo), Arcen Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  arcen:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, arcen-agent]
---

# Grok Build CLI — Arcen Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Arcen terminal. Grok
...
1. Prepare stable input files first with Arcen tools (`read_file`,
...
2. **Don't conflate Arcen' xAI auth with the `grok` CLI's auth.** Arcen'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Arcen' xAI auth carries
   over.
