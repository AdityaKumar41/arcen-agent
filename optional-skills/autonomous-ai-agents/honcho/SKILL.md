---
name: honcho
description: Configure and use Honcho memory with Arcen -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Arcen Agent
license: MIT
...
# Honcho Memory for Arcen

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Arcen profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/arcen#running-honcho-locally-with-arcen

### Verify

```bash
arcen honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Arcen profile's AI peer
...
Honcho models conversations as interactions between **peers**. Arcen creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Arcen instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Arcen run |
| `global` | Single session across all directories |

Manual override: `arcen honcho map my-project-name`
...
Each Arcen profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
arcen profile create coder --clone
# creates host block arcen.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `arcen.coder` host block in `honcho.json`
...
```bash
arcen honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Arcen when Honcho memory is active.
...
Config file: `$ARCEN_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `arcen honcho setup`. Ensure `memory.provider: honcho` is in `~/.arcen/config.yaml`.

### Memory not persisting across sessions
Check `arcen honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `arcen profile create <name> --clone`. For existing profiles: `arcen honcho sync`.
...
| Command | Description |
|---------|-------------|
| `arcen honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `arcen honcho status` | Show resolved config, connection test, peer info for active profile |
| `arcen honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `arcen honcho disable` | Disable Honcho for the active profile |
| `arcen honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `arcen honcho peers` | Show peer identities across all profiles |
| `arcen honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `arcen honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `arcen honcho sessions` | List known directory-to-session-name mappings |
| `arcen honcho map <name>` | Map current working directory to a Honcho session name |
| `arcen honcho identity` | Seed AI peer identity or show both peer representations |
| `arcen honcho sync` | Create host blocks for all Arcen profiles that don't have one yet |
| `arcen honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Arcen + Honcho |
| `arcen memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `arcen memory status` | Show active memory provider and config |
| `arcen memory off` | Disable external memory provider |
