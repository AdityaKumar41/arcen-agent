...
version: 0.1.0
author: Tim Koepsel (xzessmedia), Arcen Agent
license: MIT
platforms: [linux, macos]
metadata:
  arcen:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, arcen-agent]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Arcen.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Arcen-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `arcen-agent` (Arcen subagents via `delegate_task`).
