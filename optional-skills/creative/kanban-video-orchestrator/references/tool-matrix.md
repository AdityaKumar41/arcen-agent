...
Maps each role archetype to the Arcen skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public arcen-agent
...
## Arcen skills relevant to video production

### Visual / rendering skills (`arcen-agent/skills/creative/`)
...
### Design / pre-production skills (`arcen-agent/skills/creative/`)
...
### Audio / media skills (`arcen-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`arcen-agent/skills/devops/`)
...
These are **not** Arcen skills but external CLIs / APIs that profiles invoke.
...
## Built-in Arcen tools for media review

These are native Arcen tools — not invoked via terminal but through their own
...
key is present in `~/.arcen/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Arcen profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.arcen/.env` → environment variable.
...
When a new Arcen-public video skill ships:
