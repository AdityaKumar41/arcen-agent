#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `arcen kanban list` and
...
def arcen_available() -> bool:
    return shutil.which("arcen") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed arcen CLI."""
    try:
        out = subprocess.run(
            ["arcen", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `arcen kanban list`
    out = subprocess.run(
        ["arcen", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["arcen", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not arcen_available():
        print("ERROR: 'arcen' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
