...
All three live in `$ARCEN_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$ARCEN_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $ARCEN_HOME/skills/devops/watchers/scripts/watch_rss.py \
  --name hn --url https://news.ycombinator.com/rss --max 5
```

Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.arcen/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $ARCEN_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name arcen-issues --repo AdityaKumar41/arcen-agent --scope issues
```

Poll an arbitrary JSON API:

```bash
python $ARCEN_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$ARCEN_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $ARCEN_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $ARCEN_HOME/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$ARCEN_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
