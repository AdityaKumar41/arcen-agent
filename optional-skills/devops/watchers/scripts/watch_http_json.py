...
Usage (via cron with --no-agent):
    ...
    arcen cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ARCEN_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Arcen-Watcher/1.0"})
