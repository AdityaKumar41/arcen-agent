...
- smoke-test a server with the FastMCP CLI before wiring it into Arcen or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Arcen. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.arcen/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.arcen/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Arcen integration, either:

- configure the server in `~/.arcen/config.yaml` using the `native-mcp` skill, or
...
### Arcen cannot see the deployed server

The server-building part may be correct while the Arcen config is not. Load the `native-mcp` skill and configure the server in `~/.arcen/config.yaml`, then restart Arcen.
