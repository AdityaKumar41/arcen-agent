...
description: Migrate a user's OpenClaw customization footprint into Arcen Agent. Imports Arcen-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Arcen Agent (Nous Research)
license: MIT
platforms: [linux, macos, windows]
metadata:
  arcen:
    tags: [Migration, OpenClaw, Arcen, Memory, Persona, Import]
    related_skills: [arcen-agent]
---

# OpenClaw -> Arcen Migration

Use this skill when a user wants to move their OpenClaw setup into Arcen Agent with minimal manual cleanup.
...
```bash
arcen claw migrate              # Full interactive migration
...
arcen claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `arcen setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_arcen.py` to:

- import `SOUL.md` into the Arcen home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Arcen memory entries
- merge OpenClaw command approval patterns into Arcen `command_allowlist`
- migrate Arcen-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Arcen working-directory configuration
- copy OpenClaw skills into `~/.arcen/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Arcen workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.arcen/tts/`
- archive non-secret docs that do not have a direct Arcen destination
...
- `scripts/openclaw_to_arcen.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.arcen/skills/migration/openclaw-migration/scripts/openclaw_to_arcen.py`

Do not guess a shorter path like `~/.arcen/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.arcen/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Arcen-compatible secrets, currently:
...
Arcen CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Arcen. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Arcen workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Arcen skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.arcen/skills/migration/openclaw-migration/scripts/openclaw_to_arcen.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.arcen/skills/migration/openclaw-migration/scripts/openclaw_to_arcen.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.arcen/skills/migration/openclaw-migration/scripts/openclaw_to_arcen.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.arcen/skills/migration/openclaw-migration/scripts/openclaw_to_arcen.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.arcen/skills/migration/openclaw-migration/scripts/openclaw_to_arcen.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.arcen/skills/migration/openclaw-migration/scripts/openclaw_to_arcen.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Arcen unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Arcen targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Arcen destination. Unsupported auth blobs must still be reported as skipped.
...
- Arcen persona state imported
- Arcen memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.arcen/skills/openclaw-imports/`
