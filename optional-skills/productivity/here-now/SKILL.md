...
- Skill helper paths:
  - `${ARCEN_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ARCEN_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ARCEN_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client arcen
```
...
```bash
PUBLISH="${ARCEN_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client arcen
```
...
```bash
DRIVE="${ARCEN_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `arcen`)    |
