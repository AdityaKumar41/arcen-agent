...
version: 0.1.0
author: Bihruze (Asahi0x), Arcen Agent
license: MIT
...
import upstream classes into Arcen itself.
...
```bash
mkdir -p ~/.arcen/cache/darwinian-evolver && cd ~/.arcen/cache/darwinian-evolver
...
```bash
cd ~/.arcen/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.arcen/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.arcen/skills/research/darwinian-evolver
DE_DIR=~/.arcen/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Arcen core.
   Custom driver scripts under `~/.arcen/skills/...` are user-side and fine.
