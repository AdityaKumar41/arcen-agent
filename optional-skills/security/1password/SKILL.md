...
version: 1.0.0
author: arceus77-7, enhanced by Arcen Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during Arcen terminal calls (desktop app flow only)
...
### Service Account (recommended for Arcen)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.arcen/.env` (the skill will prompt for this on first load).
...
## Arcen Execution Pattern (desktop app flow)

Arcen terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/arcen-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/arcen-op.sock"
