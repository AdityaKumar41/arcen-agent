...
Drive REST and GraphQL diagnosis through Arcen tools — `terminal` for `curl`, `execute_code` for Python `requests`, `web_extract` for vendor docs. Isolate the failing layer before guessing at the fix.
...
- Never hardcode tokens in scripts. Read from env (`os.environ["API_TOKEN"]`) or `~/.arcen/.env`.
