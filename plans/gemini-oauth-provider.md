...
## Client ID
- Need to register a "Desktop app" OAuth client on a ArcenPay GCP project
...
## Token Lifecycle
- Store at `~/.arcen/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `arcen_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `arcen_cli/models.py` — Add Gemini model catalog
4. `arcen_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `arcen_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `arcen_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- ArcenPay GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via ARCEN_GEMINI_CLIENT_ID env var
...
- arcen-agent Copilot OAuth: `arcen_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
