...
Auto-tracks and cleans up ephemeral files created during Arcen Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ARCEN_HOME` and `/tmp/arcen-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ARCEN_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ARCEN_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `ARCEN_HOME` or `/tmp/arcen-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ARCEN_HOME/disk-cleanup/` is itself excluded
- `$ARCEN_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
