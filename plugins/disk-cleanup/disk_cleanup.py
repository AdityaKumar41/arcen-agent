"""disk_cleanup — ephemeral file cleanup for Arcen Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ARCEN_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ARCEN_HOME and /tmp/arcen-*
Never touches: ~/.arcen/logs/ or any system directory.
"""
...
try:
    ...
    def get_arcen_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ARCEN_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".arcen").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ARCEN_HOME/logs/``."""
    return get_arcen_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ARCEN_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ARCEN_HOME or ``/tmp/arcen-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    arcen_home = get_arcen_home()
    try:
        path.resolve().relative_to(arcen_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/arcen-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("arcen-"):
        return True
    return False
...
# Paths under $ARCEN_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so ARCEN_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        arcen_home = get_arcen_home()
        for parent in ("cron", "cronjobs"):
            base = arcen_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ARCEN_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ARCEN_HOME (but leave ARCEN_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "arcen-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(arcen_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == arcen_home:
                continue
            try:
                rel_parts = dirpath.relative_to(arcen_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    arcen_home = get_arcen_home()
    try:
        rel = path.resolve().relative_to(arcen_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "arcen-agent",
        }:
            return None
        ...
        # Path isn't under ARCEN_HOME (e.g. /tmp/arcen-*) — fall through.
        pass
