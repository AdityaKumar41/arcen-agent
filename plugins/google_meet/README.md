...
Let the arcen agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where arcen runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `arcen meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `arcen meet` CLI |
...
| `cli.py` | `arcen meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ARCEN_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `arcen meet node {run,list,approve,remove,status,ping}` |
...
arcen meet install                                      # pip + Chromium
arcen meet setup                                        # preflight
arcen meet auth                                         # optional
arcen meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
arcen meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.arcen/.env
arcen meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
arcen meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
arcen meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.arcen/.env
arcen meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, arcen will **not** switch your system audio input automatically — the
...
arcen plugins enable google_meet
arcen meet node run --display-name my-mac --host 0.0.0.0 --port 18789
