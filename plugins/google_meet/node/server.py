...
Launched by ``arcen meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$ARCEN_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``arcen meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_arcen_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "arcen-meet-node",
    ) -> None:
        self.host = host
        self.port = port
