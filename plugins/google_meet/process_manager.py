...
session-scoped state file under ``$ARCEN_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ARCEN_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_arcen_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this arcen install, leave it first —
    ...
    env = os.environ.copy()
    env["ARCEN_MEET_URL"] = url
    env["ARCEN_MEET_OUT_DIR"] = str(out)
    env["ARCEN_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ARCEN_MEET_HEADED"] = "1"
    if auth_state:
        env["ARCEN_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ARCEN_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ARCEN_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ARCEN_MEET_MODE"] = mode
    if realtime_model:
        env["ARCEN_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ARCEN_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ARCEN_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ARCEN_MEET_REALTIME_KEY"] = realtime_api_key
