...
```bash
arcen memory setup    # select "byterover"
```

Or manually:
```bash
arcen config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.arcen/.env
```
...
Working directory: `$ARCEN_HOME/byterover/` (profile-scoped).
