...
```bash
arcen memory setup    # select "hindsight"
```
...
```bash
arcen config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.arcen/.env
```
...
Arcen spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.arcen/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p arcen ui start
```
...
Config file: `~/.arcen/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `arcen` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `arcen-{profile}` isolates memory per active Arcen profile. Empty placeholders collapse cleanly (e.g. `arcen-{user}` with no user becomes `arcen`). |
...
| `retain_context` | `conversation between Arcen Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.arcen/.env` as `HINDSIGHT_LLM_API_KEY`.
