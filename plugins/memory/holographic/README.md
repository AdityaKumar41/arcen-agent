...
```bash
arcen memory setup    # select "holographic"
```

Or manually:
```bash
arcen config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.arcen-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ARCEN_HOME/memory_store.db` | SQLite database path |
