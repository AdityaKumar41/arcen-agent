...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/arcen>
...
```bash
arcen memory setup honcho   # configure Honcho directly (works on a fresh install)
arcen memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
arcen config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.arcen/.env
```

> `arcen honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `arcen memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ARCEN_HOME/honcho.json` | Profile-local (isolated Arcen instances) |
| 2 | `~/.arcen/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Arcen profile: `arcen` (default) or `arcen_<profile>`.
...
**Deployment shapes** (`arcen memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Arcen to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Arcen session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`arcen-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`arcen`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-arcen-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening arcen in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Arcen session ID (timestamp + hex). Every `arcen` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Arcen profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`arcen`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Arcen profile: `arcen` (default) or `arcen_<profile>` (e.g. `arcen -p coder` -> host key `arcen_coder`). Older `arcen.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ARCEN_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `arcen memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `arcen honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `arcen memory setup`) |
| `arcen honcho status` | Show resolved config for active profile |
| `arcen honcho enable` / `disable` | Toggle Honcho for active profile |
| `arcen honcho mode <mode>` | Change recall or observation mode |
| `arcen honcho peer --user <name>` | Update user peer name |
| `arcen honcho peer --ai <name>` | Update AI peer name |
| `arcen honcho tokens --context <N>` | Set context token budget |
| `arcen honcho tokens --dialectic <N>` | Set dialectic max chars |
| `arcen honcho map <name>` | Map current directory to a session name |
| `arcen honcho sync` | Create host blocks for all Arcen profiles |
