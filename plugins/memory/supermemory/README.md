...
```bash
arcen memory setup    # select "supermemory"
```

Or manually:

```bash
arcen config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.arcen/.env
```

## Config

Config file: `$ARCEN_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `arcen` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `arcen-{identity}` → `arcen-coder`). |
...
All Supermemory API calls send `x-sm-source: arcen`, and document writes stamp
`metadata.sm_source: arcen`. This is a **functional routing key, not telemetry**:
it groups Arcen-written memories into a dedicated "Arcen" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Arcen profile:

```json
{
  "container_tag": "arcen-{identity}"
}
```

For a profile named `coder`, this resolves to `arcen-coder`. The default profile resolves to `arcen-default`. Without `{identity}`, all profiles share the same container.
