...
def _load_supermemory_config(arcen_home: str) -> dict:
    config = _default_config()
    config_path = Path(arcen_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, arcen_home: str) -> None:
    config_path = Path(arcen_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "arcen"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "arcen", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `arcen memory setup`.
        # All other options are documented for $ARCEN_HOME/supermemory.json
    ...
    def save_config(self, values, arcen_home):
        ...
        _save_supermemory_config(sanitized, arcen_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from arcen_constants import get_arcen_home
        self._arcen_home = kwargs.get("arcen_home") or str(get_arcen_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._arcen_home)
