...
Optional Arcen observability plugin that maps Arcen observer hooks to
...
not replace Arcen Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Arcen emit NeMo Relay lifecycle events for
the work Arcen already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Arcen Agent can:

- Preserve Arcen execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
arcen plugins enable observability/nemo_relay
```

The `ARCEN_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `ARCEN_HOME` that the
agent run will use:

```bash
env ARCEN_HOME=/tmp/arcen-nemo-relay-test \
  arcen plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`ARCEN_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`ARCEN_HOME` is the Arcen profile/config home used by both
`arcen plugins enable ...` and the later `arcen chat ...` run. If unset,
Arcen uses the user's default home, usually `~/.arcen`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export ARCEN_HOME=/tmp/arcen-nemo-relay-test
arcen plugins enable observability/nemo_relay
arcen chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `arcen` command you run is built from the
...
uv run arcen plugins enable observability/nemo_relay
uv run arcen chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/arcen_agent-*.whl
...
The plugin can configure exporters directly from `ARCEN_NEMO_RELAY_*`
...
```bash
export ARCEN_NEMO_RELAY_ATOF_ENABLED=1
export ARCEN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export ARCEN_NEMO_RELAY_ATIF_ENABLED=1
export ARCEN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `ARCEN_NEMO_RELAY_ATOF_FILENAME`
- `ARCEN_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `ARCEN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `ARCEN_NEMO_RELAY_ATIF_AGENT_NAME`
- `ARCEN_NEMO_RELAY_ATIF_AGENT_VERSION`
- `ARCEN_NEMO_RELAY_ATIF_MODEL_NAME`
- `ARCEN_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Arcen at it:

```bash
export ARCEN_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Arcen Agent"
agent_version = "local"
```

When `ARCEN_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`ARCEN_NEMO_RELAY_ATOF_*` fallback setup is skipped.
...
export ARCEN_HOME=/tmp/arcen-nemo-relay-docs/arcen-home
mkdir -p "$ARCEN_HOME"

cat > "$ARCEN_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Arcen session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export ARCEN_NEMO_RELAY_ATOF_ENABLED=1
export ARCEN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/arcen-nemo-relay-docs/subagent/atof
export ARCEN_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export ARCEN_NEMO_RELAY_ATOF_MODE=overwrite
export ARCEN_NEMO_RELAY_ATIF_ENABLED=1
export ARCEN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/arcen-nemo-relay-docs/subagent/atif
export ARCEN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export ARCEN_NEMO_RELAY_ATIF_AGENT_NAME='Arcen Agent E2E'
export ARCEN_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export ARCEN_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"arcen.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Arcen Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Arcen dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/arcen-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/arcen-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/arcen-nemo-relay-docs/workdir/beta.txt
cd /tmp/arcen-nemo-relay-docs/workdir

export ARCEN_NEMO_RELAY_ATOF_ENABLED=1
export ARCEN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/arcen-nemo-relay-docs/parallel/atof
export ARCEN_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export ARCEN_NEMO_RELAY_ATOF_MODE=overwrite
export ARCEN_NEMO_RELAY_ATIF_ENABLED=1
export ARCEN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/arcen-nemo-relay-docs/parallel/atif
export ARCEN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export ARCEN_NEMO_RELAY_ATIF_AGENT_NAME='Arcen Agent E2E'
export ARCEN_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Arcen sessions map to `agent` scopes.
- Arcen API request hooks map to `llm` scope start/end events.
- Arcen tool hooks map to `tool` scope start/end events.
