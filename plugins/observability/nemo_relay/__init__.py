"""nemo_relay — optional Arcen plugin for NeMo Relay observability."""
...
class _Settings:
    ...
    atof_filename: str = "arcen-atof.jsonl"
    atof_mode: str = "append"
    atif_enabled: bool = False
    atif_output_directory: str = ""
    atif_filename_template: str = "arcen-atif-{session_id}.json"
    atif_subagent_export_mode: str = "embedded"
    atif_agent_name: str = "Arcen Agent"
...
class _Runtime:
    ...
    def _configure_atof(self) -> None:
        ...
        self.atof_exporter.register("arcen.nemo_relay.atof")
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if self.settings.atif_enabled:
            state.atif_exporter = self.nemo_relay.AtifExporter(
                ...
                extra={"source": "arcen-agent", "plugin": "observability/nemo_relay"},
            )
            state.atif_subscriber_name = f"arcen.nemo_relay.atif.{session_id}"
    ...
    def mark_subagent_stop(self, kwargs: dict[str, Any]) -> None:
        ...
        self.mark("arcen.subagent.stop", kwargs)
...
def on_session_end(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: (runtime.mark("arcen.session.end", kwargs), runtime.export_atif(runtime.ensure_session(kwargs))))
...
def on_pre_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("arcen.turn.start", kwargs))


def on_post_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("arcen.turn.end", kwargs))
...
def on_post_api_request(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("arcen.api.response.unmatched", kwargs)
            return
...
def on_api_request_error(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        span = state.llm_spans.pop(_api_key(kwargs), None)
        if span is None:
            runtime.mark("arcen.api.error", kwargs)
            return
        runtime.nemo_relay.llm.call_end(
            span,
...
def on_post_tool_call(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("arcen.tool.response.unmatched", kwargs)
            return
...
def on_pre_approval_request(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("arcen.approval.request", kwargs))


def on_post_approval_response(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("arcen.approval.response", kwargs))
...
def _load_settings() -> _Settings:
    return _Settings(
        plugins_toml_path=_env("ARCEN_NEMO_RELAY_PLUGINS_TOML"),
        atof_enabled=_env_bool("ARCEN_NEMO_RELAY_ATOF_ENABLED"),
        atof_output_directory=_env("ARCEN_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY"),
        atof_filename=_env("ARCEN_NEMO_RELAY_ATOF_FILENAME") or "arcen-atof.jsonl",
        atof_mode=_env("ARCEN_NEMO_RELAY_ATOF_MODE") or "append",
        atif_enabled=_env_bool("ARCEN_NEMO_RELAY_ATIF_ENABLED"),
        atif_output_directory=_env("ARCEN_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"),
        atif_filename_template=_env("ARCEN_NEMO_RELAY_ATIF_FILENAME_TEMPLATE") or "arcen-atif-{session_id}.json",
        atif_subagent_export_mode=_atif_subagent_export_mode(),
        atif_agent_name=_env("ARCEN_NEMO_RELAY_ATIF_AGENT_NAME") or "Arcen Agent",
        atif_agent_version=_env("ARCEN_NEMO_RELAY_ATIF_AGENT_VERSION") or "unknown",
        atif_model_name=_env("ARCEN_NEMO_RELAY_ATIF_MODEL_NAME") or "unknown",
    )
...
def _atif_subagent_export_mode() -> str:
    mode = _env("ARCEN_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE").lower()
