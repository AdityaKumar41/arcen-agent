...
# ``model_tools`` import time, so ``arcen status``, ``arcen chat``, etc.
...
# ``arcen_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Arcen is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a arcen MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ARCEN_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _arcen_home = _get_arcen_home()
        except (ModuleNotFoundError, ImportError):
            _arcen_home = _Path.home() / ".arcen"
        self._thread_count_store = _ThreadCountStore(
            _arcen_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Arcen is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ARCEN_HOME", str(_Path.home() / ".arcen"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ARCEN_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a arcen MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Arcen is thinking…' marker message.
        ...
        "Arcen is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Arcen is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Arcen is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Arcen is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Arcen is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``arcen setup``.

    The setup wizard at ``arcen_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.arcen/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. arcen-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.arcen/.env")
    print_info("Restart the gateway: arcen gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``arcen cron`` running as a
    separate process from ``arcen gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Arcen plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'arcen-agent[google_chat]'",
        ...
        # the "Arcen is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Arcen is thinking…' marker message "
