"""
LINE Messaging API platform adapter for Arcen Agent.
...
to Arcen Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ARCEN_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            arcen_home = Path.home().joinpath(".arcen").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``arcen status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``arcen status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``arcen setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.arcen/.env`` via ``arcen_cli.config``.
    """
    print()
    ...
    try:
        from arcen_cli.config import get_env_var, set_env_var
    except ImportError:
        print("arcen_cli.config not available; set LINE_* vars manually in ~/.arcen/.env")
        return
