"""ntfy platform adapter (Arcen plugin).
...
``plugins/platforms/ntfy/``. The Arcen plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "arcen-in"              # subscribe topic (incoming)
          publish_topic: "arcen-out"     # optional — defaults to topic
...
_ECHO_TAG = "arcen-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``arcen cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Arcen dependency",
