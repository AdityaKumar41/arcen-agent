"""SimpleX Chat platform adapter (Arcen plugin).
...
This adapter ships as a Arcen platform plugin under
``plugins/platforms/simplex/``. The Arcen plugin loader scans the
...
discoverable and `arcen setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Arcen — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"arcen-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``arcen setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.arcen/.env`` via ``arcen_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("arcen_cli.config not available; set SIMPLEX_* vars manually in ~/.arcen/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `arcen gateway status` without
