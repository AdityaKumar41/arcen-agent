"""
Microsoft Teams platform adapter for Arcen Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Arcen"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        arcen_action = data.get("arcen_action", "")
        session_key = data.get("session_key", "")

        if not arcen_action or not session_key:
        ...
        choice = choice_map.get(arcen_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="arcen_approve",
                    data={**btn_data_base, "arcen_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="arcen_approve",
                    data={**btn_data_base, "arcen_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="arcen_approve",
                    data={**btn_data_base, "arcen_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="arcen_approve",
                    data={**btn_data_base, "arcen_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Arcen\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.arcen/.env")
