...
def _is_tool_gateway_ready() -> bool:
    ...
    Reads ``peek_managed_gateway_token`` and ``resolve_managed_tool_gateway``
    via :mod:`tools.web_tools` rather than direct imports, so unit tests
    that ``patch("tools.web_tools._peek_managed_gateway_token", ...)`` see
    ...
    return _wt.resolve_managed_tool_gateway(
        "firecrawl", token_reader=_wt._peek_managed_gateway_token
    ) is not None
...
def check_firecrawl_api_key() -> bool:
    ...
    existing tests and the ``arcen tools`` setup flow.
    """
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_tool_gateway_enabled():
        return ""
    return (
        ", or use the Managed Tool Gateway via your subscription "
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if _wt.managed_tool_gateway_enabled():
        message += (
            " With your managed gateway you can also use the Tool Gateway. "
            "run `arcen tools` and select Managed gateway as the web provider."
        )
    else:
        message += " " + _wt.managed_tool_gateway_unavailable_message(
...
def _get_firecrawl_client() -> Any:
    ...
    ``_read_managed_gateway_token``, ``Firecrawl``) are also looked up via
    ...
    if direct_config is not None and not _wt.prefers_gateway("web"):
        ...
        managed_gateway = _wt.resolve_managed_tool_gateway(
            "firecrawl", token_reader=_wt._read_managed_gateway_token
        )
        ...
        kwargs = {
            "api_key": managed_gateway.user_token,
            "api_url": managed_gateway.gateway_origin,
        }
        client_config = (
            "tool-gateway",
            kwargs["api_url"],
            managed_gateway.user_token,
        )
