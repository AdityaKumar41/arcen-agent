...
``{title, url, description, position}`` rows every other Arcen web
provider produces.
...
prefers Arcen-managed xAI Grok OAuth (via ``arcen auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.arcen/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``arcen tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `arcen auth` to sign in with "
