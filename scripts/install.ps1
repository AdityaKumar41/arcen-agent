# ============================================================================
# Arcen Agent Installer for Windows
# ============================================================================
...
param(
    ...
    # who need reproducible installs (CI, release
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$ArcenHome = $(if ($env:ARCEN_HOME) { $env:ARCEN_HOME } else { "$env:LOCALAPPDATA\arcen" }),
    [string]$InstallDir = $(if ($env:ARCEN_HOME) { "$env:ARCEN_HOME\arcen-agent" } else { "$env:LOCALAPPDATA\arcen\arcen-agent" }),

    # --- Stage protocol (additive; default invocation behaves as before) ----
    # See the "Stage protocol" section near the bottom of the file for the
    # full contract.  Intended for programmatic drivers (CI, future install.sh
    # parity, etc.).  CLI users
...
if ($IncludeDesktop) {
    throw "-IncludeDesktop is no longer supported in the terminal-focused build."
}
...
$RepoUrlSsh = "git@github.com:AdityaKumar41/arcen-agent.git"
$RepoUrlHttps = "https://github.com/AdityaKumar41/arcen-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Arcen Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by ArcenPay.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ArcenHome)) {
        New-Item -ItemType Directory -Force -Path $ArcenHome | Out-Null
    }
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ArcenHome "node"
...
function Install-Uv {
    # Arcen owns its own uv at $ArcenHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (arcen_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `arcen update` stay in sync.
    $managedUv = Join-Path $ArcenHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $ArcenHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $ArcenHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $ArcenHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $ArcenHome "bin"
...
# (CI step-runners and other automation) invoke each stage
...
function Resolve-UvCmd {
    ...
    # sibling process and ARCEN_HOME wasn't propagated).
...
function Install-Git {
    ...
    which Arcen uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\arcen\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Arcen needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Arcen copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\arcen\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ARCEN_GIT_BASH_PATH`` (User scope) so Arcen can find it in a fresh
    ...
    # Download PortableGit into $ArcenHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ArcenHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Arcen features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Arcen needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ARCEN_GIT_BASH_PATH`` (User env scope) so Arcen can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ArcenHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ArcenHome\git\usr\bin\bash.exe
    $candidates += "$ArcenHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ArcenHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ARCEN_GIT_BASH_PATH", $candidate, "User")
            $env:ARCEN_GIT_BASH_PATH = $candidate
            Write-Info "Set ARCEN_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Arcen may not find Git Bash."
    Write-Info "If needed, set ARCEN_GIT_BASH_PATH manually to your bash.exe path."
}

# The TUI build runs modern Vite tooling, which refuses to start on Node
# outside `^20.19 || >=22.12`. Older Node lacks node:util.styleText, so builds
# crash with a SyntaxError. Returns $true when a `node --version` string clears
# that floor.
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old for the TUI build (need ^20.19 or >=22.12)"
    }

    # Prefer a Arcen-managed Node from a previous run over a too-old system one.
    $managedNode = "$ArcenHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$ArcenHome\node;$env:Path"
        Write-Success "Node.js $version found (Arcen-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Arcen-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ArcenHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ArcenHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\arcen-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ArcenHome\node") { Remove-Item -Recurse -Force "$ArcenHome\node" }
                Move-Item $extractedDir.FullName "$ArcenHome\node"
                ...
                $version = & "$ArcenHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ArcenHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\arcen-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    $stashName = "arcen-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    # both stdin and stdout. Automated update flows and
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Arcen behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running arcen processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/AdityaKumar41/arcen-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/AdityaKumar41/arcen-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/AdityaKumar41/arcen-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\arcen-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\arcen-agent-extract"
    ...
    # next `arcen update` checkout aborts on a "dirty" tree the user never
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `arcen.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install arcen-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and arcen.exe broken with
    ...
    # here, not 30 seconds later when the user runs `arcen`.
    if (-not $NoVenv) {
...
function Set-PathVariable {
    ...
    if ($NoVenv) {
        ...
        $arcenBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so arcen is globally available
    # On Windows, the arcen.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$arcenBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$arcenBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $arcenBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ARCEN_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\arcen instead
    # of the Unix default ~/.arcen
    $currentArcenHome = [Environment]::GetEnvironmentVariable("ARCEN_HOME", "User")
    if (-not $currentArcenHome -or $currentArcenHome -ne $ArcenHome) {
        [Environment]::SetEnvironmentVariable("ARCEN_HOME", $ArcenHome, "User")
        Write-Success "Set ARCEN_HOME=$ArcenHome"
    }
    $env:ARCEN_HOME = $ArcenHome
    
    # Update current session
    $env:Path = "$arcenBin;$env:Path"
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'arcen setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m arcen_cli.main setup
    } else {
        python -m arcen_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $arcenCmd = "$InstallDir\venv\Scripts\arcen.exe"
    if (-not (Test-Path $arcenCmd)) {
        $arcenCmd = "arcen"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ArcenHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'arcen whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: arcen gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ArcenHome\logs\gateway.log"
            Start-Process -FilePath $arcenCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ArcenHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: arcen gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: arcen gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ArcenHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ArcenHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ArcenHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ArcenHome\arcen-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   arcen              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   arcen setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   arcen config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   arcen config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   arcen gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   arcen update       " -NoNewline -ForegroundColor Green
...
# callers (CI, future install.sh, etc.)
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Arcen repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Arcen to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://arcen-cli.arcenpay.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
