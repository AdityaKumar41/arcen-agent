...
# Python-driven tool session (e.g. Arcen terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:AdityaKumar41/arcen-agent.git"
REPO_URL_HTTPS="https://github.com/AdityaKumar41/arcen-agent.git"
ARCEN_HOME="${ARCEN_HOME:-$HOME/.arcen}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ARCEN_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ARCEN_INSTALL_DIR"
...
#   code at /usr/local/lib/arcen-agent, command at /usr/local/bin/arcen,
#   data still at /root/.arcen (ARCEN_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --include-desktop|-IncludeDesktop)
            echo "error: --include-desktop is no longer supported in the terminal-focused build" >&2
            exit 1
            shift
            ;;
        --dir)
        ...
        -h|--help)
            ...
            echo "                   write \$ARCEN_HOME/.no-bundled-skills so future"
            echo "                   'arcen update' runs never inject bundled skills either"
            ...
            echo "                   default (non-root):  ~/.arcen/arcen-agent"
            echo "                   default (root, Linux): /usr/local/lib/arcen-agent"
            echo "  --arcen-home PATH  Data directory (default: ~/.arcen, or \$ARCEN_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Arcen installs the code under"
            echo "  /usr/local/lib/arcen-agent and links the command into"
            echo "  /usr/local/bin/arcen (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ARCEN_HOME"
            echo "  (default /root/.arcen).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ARCEN_HOME/arcen-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs arcen setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `arcen update`
...
emit_manifest() {
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Arcen Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install arcen command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `arcen` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ARCEN_HOME/arcen-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ARCEN_HOME/arcen-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/arcen-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ARCEN_HOME/arcen-agent — then preserve it)
#
# Always no-op when the user set --dir or $ARCEN_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ARCEN_HOME.
    if is_termux; then
        INSTALL_DIR="$ARCEN_HOME/arcen-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ARCEN_HOME/arcen-agent/.git" ]; then
            INSTALL_DIR="$ARCEN_HOME/arcen-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/arcen-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/arcen-agent"
        ...
        # /usr/local/bin/arcen wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/arcen"
        log_info "  Data:    $ARCEN_HOME (unchanged)"
...
get_arcen_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/arcen" ]; then
        echo "$link_dir/arcen"
    else
        echo "arcen"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://arcen-cli.arcenpay.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Arcen owns its own uv at $ARCEN_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (arcen_cli/managed_uv.py) looks in the same
    # place, so install.sh and `arcen update` stay in sync.
    local _managed_uv="$ARCEN_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ARCEN_HOME/bin ..."
    mkdir -p "$ARCEN_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/arcen-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/arcen-uv-installer.$$.sh")"
    ...
    # directly into $ARCEN_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ARCEN_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# The TUI build runs modern Vite tooling, which refuses to start on Node
# outside `^20.19 || >=22.12`. Older Node lacks `node:util.styleText`, so
# builds crash with a SyntaxError. Returns 0 when the given `node --version`
# string clears that floor; anything below it is replaced with the
# Arcen-managed Node $NODE_VERSION LTS.
...
check_node() {
    ...
    # Prefer a Arcen-managed Node from a previous run over a too-old system one.
    if [ -x "$ARCEN_HOME/node/bin/node" ] && node_satisfies_build "$("$ARCEN_HOME/node/bin/node" --version)"; then
        export PATH="$ARCEN_HOME/node/bin:$PATH"
        log_success "Node.js $("$ARCEN_HOME/node/bin/node" --version) found (Arcen-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the TUI build (need ^20.19 or >=22.12) — installing Arcen-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.arcen/node/..."
    ...
    # Place into ~/.arcen/node/ and symlink binaries into the same bin dir
    # the arcen command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ARCEN_HOME/node"
    mkdir -p "$ARCEN_HOME"
    mv "$extracted_dir" "$ARCEN_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$ARCEN_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ARCEN_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ARCEN_HOME/node/bin/npx"  "$node_link_dir/npx"

    export PATH="$ARCEN_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$ARCEN_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.arcen/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Arcen install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Arcen Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="arcen-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Arcen behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up arcen command..."

    if [ "$USE_VENV" = true ]; then
        ARCEN_BIN="$INSTALL_DIR/venv/bin/arcen"
    else
        ARCEN_BIN="$(which arcen 2>/dev/null || echo "")"
        if [ -z "$ARCEN_BIN" ]; then
            log_warn "arcen not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$ARCEN_BIN" ]; then
        log_warn "arcen entry point not found at $ARCEN_BIN"
    ...
    # Create a user-facing shim for the arcen command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ARCEN_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ARCEN_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/arcen"
    cat > "$command_link_dir/arcen" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ARCEN_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/arcen"
    log_success "Installed arcen launcher → $command_link_display_dir/arcen"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "arcen command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v arcen' \
                >/dev/null 2>&1; then
        ...
        log_info "arcen not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Arcen Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Arcen Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Arcen Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so arcen works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.arcen directory structure (config at top level, code in subdir)
    mkdir -p "$ARCEN_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.arcen/.env (top level, easy to find)
    if [ ! -f "$ARCEN_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ARCEN_HOME/.env"
            log_success "Created ~/.arcen/.env from template"
        else
            touch "$ARCEN_HOME/.env"
            log_success "Created ~/.arcen/.env"
        fi
    else
        log_info "~/.arcen/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ARCEN_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.arcen/config.yaml (top level, easy to find)
    if [ ! -f "$ARCEN_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ARCEN_HOME/config.yaml"
            log_success "Created ~/.arcen/config.yaml from template"
        fi
    else
        log_info "~/.arcen/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$ARCEN_HOME/SOUL.md" ]; then
        cat > "$ARCEN_HOME/SOUL.md" << 'SOUL_EOF'
# Arcen Agent Persona
...
-->
SOUL_EOF
        log_success "Created ~/.arcen/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.arcen/"

    # Seed bundled skills into ~/.arcen/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `arcen update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'arcen update'." \
            > "$ARCEN_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ARCEN_HOME/.no-bundled-skills"
        log_info "  Future 'arcen update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.arcen/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.arcen/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ARCEN_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ARCEN_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.arcen/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Arcen Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Arcen will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (arcen --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `arcen update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'arcen setup' after install."
        return 0
    fi
    ...
    # Run arcen setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m arcen_cli.main setup < /dev/tty
    else
        python -m arcen_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ARCEN_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Arcen to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ARCEN_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'arcen whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ARCEN_CMD="$(get_arcen_command_path)"
                $ARCEN_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'arcen whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'arcen gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ARCEN_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ARCEN_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: arcen gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: arcen gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ARCEN_CMD gateway > "$ARCEN_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.arcen/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: arcen gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: arcen gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ARCEN_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ARCEN_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ARCEN_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}arcen${NC}              Start chatting"
    echo -e "   ${GREEN}arcen setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}arcen config${NC}       View/edit configuration"
    echo -e "   ${GREEN}arcen config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}arcen gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}arcen update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'arcen' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'arcen' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'arcen' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ARCEN_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ARCEN_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ARCEN_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$ARCEN_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ARCEN_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Arcen for pip install"
    ...
    ARCEN_CMD="$(command -v arcen 2>/dev/null || echo "")"
    if [ -n "$ARCEN_CMD" ]; then
        log_info "Running arcen setup..."
        "$ARCEN_CMD" setup
    else
        log_warn "arcen command not found on PATH"
        log_info "Try: python -m arcen_cli.main setup"
    fi
}
...
# from another cwd and resolve `.` / the venv against the wrong tree.
require_install_dir() {
...
# Stage protocol. Mirrors the Windows install.ps1 surface closely enough for
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$ARCEN_HOME/.install_method"
            ;;
        *)
