...
#   2. ~/.arcen/node/ from a prior Arcen-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.arcen/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$ARCEN_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   ARCEN_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   ARCEN_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   ARCEN_HOME               (default: $HOME/.arcen)
# ============================================================================

ARCEN_NODE_MIN_VERSION="${ARCEN_NODE_MIN_VERSION:-20}"
ARCEN_NODE_TARGET_MAJOR="${ARCEN_NODE_TARGET_MAJOR:-22}"
ARCEN_HOME="${ARCEN_HOME:-$HOME/.arcen}"
ARCEN_NODE_AVAILABLE=false
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$ARCEN_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $ARCEN_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$ARCEN_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$ARCEN_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $ARCEN_NODE_TARGET_MAJOR..."
    proto install node "$ARCEN_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $ARCEN_NODE_TARGET_MAJOR..."
    nvm install "$ARCEN_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$ARCEN_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${ARCEN_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${ARCEN_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${ARCEN_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${ARCEN_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${ARCEN_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $ARCEN_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $ARCEN_HOME/node/..."
    ...
    mv "$extracted" "$ARCEN_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    mkdir -p "$_link_dir"
    ln -sf "$ARCEN_HOME/node/bin/node" "$_link_dir/node"
    ln -sf "$ARCEN_HOME/node/bin/npm"  "$_link_dir/npm"
    ln -sf "$ARCEN_HOME/node/bin/npx"  "$_link_dir/npx"
    export PATH="$ARCEN_HOME/node/bin:$PATH"

    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $ARCEN_HOME/node/"
    return 0
}
...
ensure_node() {
    ...
    if _nb_have_modern_node; then
        ...
        ARCEN_NODE_AVAILABLE=true
        return 0
    fi

    if [ -x "$ARCEN_HOME/node/bin/node" ]; then
        export PATH="$ARCEN_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Arcen-managed)"
            ARCEN_NODE_AVAILABLE=true
            return 0
        fi
    fi
    ...
    _nb_try_fnm   && { ARCEN_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { ARCEN_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { ARCEN_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { ARCEN_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { ARCEN_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { ARCEN_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $ARCEN_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
