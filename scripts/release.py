...
VERSION_FILE = REPO_ROOT / "arcen_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+arcen-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@arcenpay.com": "shannonsands",
    ...
    "emozilla@arcenpay.com": "emozilla",
    ...
    "arcen@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (ARCEN_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (ARCEN_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: arcen-eval community link)
    ...
    "arcenagent26@gmail.com": "arcenagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+arcenagent26@users.noreply.github.com": "arcenagent26",
    ...
    "clement@arcenpay.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@arcen": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Arcen runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level ARCEN_HOME state files)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"arcen-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "support@arcenpay.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/AdityaKumar41/arcen-agent",
    ...
    lines.append(f"# Arcen Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Arcen Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Arcen Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Arcen Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Arcen Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Arcen Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Arcen Agent v{new_version} ({calver_date})' "
