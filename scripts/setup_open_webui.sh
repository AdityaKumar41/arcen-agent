...
# Bootstrap Open WebUI against Arcen Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.arcen/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-arcen.sh
...
#   ARCEN_API_PORT=8642
#   ARCEN_API_HOST=127.0.0.1
#   ARCEN_API_MODEL_NAME='Arcen Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Arcen Agent WebUI}"
...
ARCEN_ENV_FILE="${ARCEN_ENV_FILE:-$HOME/.arcen/.env}"
ARCEN_API_PORT="${ARCEN_API_PORT:-8642}"
ARCEN_API_HOST="${ARCEN_API_HOST:-127.0.0.1}"
ARCEN_API_CONNECT_HOST="${ARCEN_API_CONNECT_HOST:-127.0.0.1}"
ARCEN_API_MODEL_NAME="${ARCEN_API_MODEL_NAME:-Arcen Agent}"
ARCEN_API_BASE_URL="http://${ARCEN_API_CONNECT_HOST}:${ARCEN_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-arcen.sh"
LOG_DIR="$HOME/.arcen/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ARCEN_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$ARCEN_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.arcen.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.arcen</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.arcen"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.arcen"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-arcen.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Arcen Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-arcen.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.arcen/logs/openwebui.log
StandardError=append:%h/.arcen/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-arcen.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ARCEN_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Arcen API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ARCEN_ENV_FILE"
  upsert_env API_SERVER_HOST "$ARCEN_API_HOST" "$ARCEN_ENV_FILE"
  upsert_env API_SERVER_PORT "$ARCEN_API_PORT" "$ARCEN_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ARCEN_API_MODEL_NAME" "$ARCEN_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ARCEN_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Arcen gateway so API server settings take effect...'
  arcen gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${ARCEN_API_CONNECT_HOST}:${ARCEN_API_PORT}/health" >/dev/null; then
    log 'Arcen API server did not answer on the first check. Trying to start gateway in the background...'
    nohup arcen gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${ARCEN_API_CONNECT_HOST}:${ARCEN_API_PORT}/health" >/dev/null
  ...
  log "Arcen API endpoint: ${ARCEN_API_BASE_URL}"
