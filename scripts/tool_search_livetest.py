#!/usr/bin/env python3
"""Live test harness for Arcen Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any arcen imports.
ORIGINAL_HOME = os.environ.get("ARCEN_HOME")
ORIGINAL_AUTH = Path.home() / ".arcen" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.arcen/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.arcen/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="arcen_ts_live_"))
    arcen_home = home_dir / ".arcen"
    arcen_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, arcen_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".arcen" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, arcen_home / ".env")
        ...
        load_arcen_dotenv(arcen_home=str(Path.home() / ".arcen"))
    ...
    (arcen_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return arcen_home
...
def reset_module_state():
    """Drop cached modules so the new ARCEN_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "arcen_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["ARCEN_HOME"] = str(home)
...
def main():
    ...
    # Restore original ARCEN_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ARCEN_HOME"] = ORIGINAL_HOME
    else:
