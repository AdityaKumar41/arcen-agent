...
# 5. Symlinks the 'arcen' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Arcen Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/arcen-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink arcen into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up arcen command..."

ARCEN_BIN="$SCRIPT_DIR/venv/bin/arcen"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ARCEN_BIN" "$COMMAND_LINK_DIR/arcen"
echo -e "${GREEN}✓${NC} Symlinked arcen → $COMMAND_LINK_DISPLAY_DIR/arcen"
...
# ============================================================================
# Seed bundled skills into ~/.arcen/skills/
# ============================================================================

ARCEN_SKILLS_DIR="${ARCEN_HOME:-$HOME/.arcen}/skills"
mkdir -p "$ARCEN_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.arcen/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ARCEN_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  arcen status        # Check configuration"
if is_termux; then
    echo "  arcen gateway       # Run gateway in foreground"
else
    echo "  arcen gateway install # Install gateway service (messaging + cron)"
fi
echo "  arcen cron list     # View scheduled jobs"
echo "  arcen doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m arcen_cli.main setup
fi
