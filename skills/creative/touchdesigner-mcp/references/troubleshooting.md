...
## 5. Arcen Configuration

### Config location

`$ARCEN_HOME/config.yaml` (defaults to `~/.arcen/config.yaml` when `ARCEN_HOME` is unset)
...
Restart the Arcen session for changes to take effect. The MCP connection is
