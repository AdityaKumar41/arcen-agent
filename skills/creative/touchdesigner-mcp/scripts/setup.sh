...
ARCEN_HOME_DIR="${ARCEN_HOME:-$HOME/.arcen}"
ARCEN_CFG="${ARCEN_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Arcen config has twozero_td MCP entry ──
if [[ ! -f "$ARCEN_CFG" ]]; then
    echo -e " ${FAIL} Arcen config not found at ${ARCEN_CFG}"
    manual_steps+=("Create ${ARCEN_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ARCEN_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Arcen config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Arcen config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ARCEN_CFG} manually"); }
    manual_steps+=("Restart Arcen session to pick up config change")
fi
