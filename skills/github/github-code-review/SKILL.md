...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.arcen/.env ] && grep -q "^GITHUB_TOKEN=" ~/.arcen/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.arcen/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Arcen Agent\",
    \"comments\": [
...
```bash
source "${ARCEN_HOME:-$HOME/.arcen}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Arcen Agent. Code looks clean — good test coverage, no security concerns."
