#!/usr/bin/env python3
"""Google Workspace API CLI for Arcen Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Arcen-facing JSON contract and falls back to the Python client
...
from _arcen_home import get_arcen_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ARCEN_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Arcen Agent")
