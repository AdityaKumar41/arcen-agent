#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Arcen Agent.
...
# Ensure sibling modules (_arcen_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _arcen_home import display_arcen_home, get_arcen_home

ARCEN_HOME = get_arcen_home()
TOKEN_PATH = ARCEN_HOME / "google_token.json"
CLIENT_SECRET_PATH = ARCEN_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ARCEN_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Arcen profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Arcen Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Arcen Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'arcen-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Arcen home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_arcen_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Arcen")
