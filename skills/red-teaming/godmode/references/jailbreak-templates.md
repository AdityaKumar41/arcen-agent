...
## 5. GODMODE FAST — Arcen 4 405B (Zero Refusal)

**Model:** `nousresearch/arcen-4-405b`
**Strategy:** Arcen models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Arcen Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.arcen/config.yaml`:
