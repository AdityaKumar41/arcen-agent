...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ARCEN_HOME", os.path.expanduser("~/.arcen")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ARCEN_HOME", Path.home() / ".arcen")) / "skills" / "red-teaming" / "godmode"
...
ARCEN_HOME = Path(os.getenv("ARCEN_HOME", Path.home() / ".arcen"))
CONFIG_PATH = ARCEN_HOME / "config.yaml"
PREFILL_PATH = ARCEN_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Arcen models — already uncensored, just needs clean prompt
    "arcen": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "arcen" in model_lower or "nous" in model_lower:
        return "arcen"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Arcen config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.arcen/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Arcen for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Arcen for changes to take effect.")
