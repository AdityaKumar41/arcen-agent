"""Tests for ACP Registry metadata shipped with Arcen."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "arcen-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Arcen Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/AdityaKumar41/arcen-agent"
    assert data["website"].startswith("https://arcen-cli.arcenpay.com/")
    assert data["authors"] == ["ArcenPay"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"arcen-agent[acp]=={data['version']}"
    assert uvx["args"] == ["arcen-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"arcen-agent[acp]=={_pyproject_version()}"
    )
