...
engine plugins (e.g. arcen-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``arcen plugins`` picker."""
    from arcen_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "arcen_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "arcen_cli.plugins.get_plugin_context_engine",
