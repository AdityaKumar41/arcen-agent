...
def curator_env(tmp_path, monkeypatch):
    """Isolated ARCEN_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".arcen"
...
def test_dry_run_does_not_advance_state(curator_env, monkeypatch):
    ...
    `arcen curator status`. Fixes #18373.
    """
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
...
def test_cli_unpin_refuses_bundled_skill(curator_env, capsys):
    """arcen curator unpin must refuse bundled/hub skills too (matches pin)."""
    from arcen_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `arcen model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    Three sources of truth, all checked by the shared registry test
    ...
    from arcen_cli.main import _AUX_TASKS
    ...
    # 2. arcen_cli/main.py _AUX_TASKS — CLI picker
