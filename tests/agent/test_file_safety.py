...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_arcen_env(self):
        """Arcen' own .env inside ARCEN_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: arcen internal .env is in ~/.arcen/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.arcen/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.arcen/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Arcen cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        arcen_home = tmp_path / ".arcen"
        cache = arcen_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._arcen_home_path", return_value=arcen_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Arcen cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        arcen_home = tmp_path / ".arcen"
        hub = arcen_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_arcen_home(self, tmp_path):
        """The env basename guard does not depend on ARCEN_HOME resolution."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        arcen_home = tmp_path / ".arcen"
        cache = arcen_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
