...
Regression for https://github.com/NousResearch/arcen-agent/issues/17656 —
...
for the credential stores while leaving arbitrary ``ARCEN_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_arcen_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "arcen_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_arcen_home_path", lambda: home)
    return home
...
def test_arbitrary_arcen_home_file_not_blocked(fake_home):
    """Non-credential files inside ARCEN_HOME stay readable."""
    from agent.file_safety import get_read_block_error
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal Arcen cache file" in err


def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into ARCEN_HOME's
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "arcen_home" / "auth.json"
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at ARCEN_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == ARCEN_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at ARCEN_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in ARCEN_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_arcen_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Arcen-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside ARCEN_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside ARCEN_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside ARCEN_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, ARCEN_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_arcen_home_path", lambda: profile)
