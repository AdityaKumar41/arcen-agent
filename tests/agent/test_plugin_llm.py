...
""",
        ...
        from arcen_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        (arcen_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))
        from arcen_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from arcen_cli.plugins import PluginContext, PluginManifest, PluginManager
