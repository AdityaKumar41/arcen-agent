...
def test_arcen_client_tag_includes_current_version():
    """The client tag must reflect arcen_cli.__version__ verbatim."""
    from arcen_cli import __version__
    from agent.portal_tags import arcen_client_tag

    assert arcen_client_tag() == f"client=arcen-client-v{__version__}"


def test_arcen_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import arcen_client_tag

    tag = arcen_client_tag()
    assert tag.startswith("client=arcen-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import arcen_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=arcen-agent" in tags
    assert arcen_client_tag() in tags
    assert len(tags) == 2
