...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ARCEN_HOME under tmp_path."""
    monkeypatch.setenv("ARCEN_HOME", str(tmp_path / ".arcen"))
    (tmp_path / ".arcen").mkdir()

    # Force the constants/image cache helpers to re-read ARCEN_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("arcen_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_arcen_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ARCEN_HOME — gateway cleanup
