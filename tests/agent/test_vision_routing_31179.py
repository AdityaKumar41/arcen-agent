...
def isolated_home(monkeypatch):
    """Temp ARCEN_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="arcen_test_31179_")
    arcen_home = os.path.join(test_home, ".arcen")
    os.makedirs(arcen_home)
...
def _fresh_modules():
    """Drop cached arcen modules so each test reloads against current env."""
