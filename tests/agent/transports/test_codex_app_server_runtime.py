...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Arcen passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ARCEN_HOME", "/users/alice/.arcen/profiles/backend-worker")
        monkeypatch.setenv("ARCEN_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ARCEN_KANBAN_DB",
            "/users/alice/.arcen/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.arcen/kanban/boards/smoke"]'
            in cmd
        )
