"""Tests for arcen backup and import commands."""
...
def _make_arcen_tree(root: Path) -> None:
    """Create a realistic ~/.arcen directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "arcen_state.db").write_bytes(b"fake-state")
    ...
    # arcen-agent repo (should be EXCLUDED)
    (root / "arcen-agent").mkdir(exist_ok=True)
    (root / "arcen-agent" / "run_agent.py").write_text("# big file\n")
    (root / "arcen-agent" / ".git").mkdir()
    (root / "arcen-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_arcen_agent(self):
        from arcen_cli.backup import _should_exclude
        assert _should_exclude(Path("arcen-agent/run_agent.py"))
        assert _should_exclude(Path("arcen-agent/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        _make_arcen_tree(arcen_home)

        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))
        # get_default_arcen_root needs this
    ...
    def test_excludes_arcen_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include arcen-agent/ directory."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        _make_arcen_tree(arcen_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "arcen-agent" in n]
            assert agent_files == [], f"arcen-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/arcen-backup-*.zip."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        (arcen_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("arcen-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside ARCEN_HOME."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        _make_arcen_tree(arcen_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(arcen_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Arcen backup."""
        from arcen_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only arcen_state.db (old wrong name) is rejected."""
        from arcen_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["arcen_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into arcen home."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (arcen_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (arcen_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (arcen_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (arcen_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_arcen_prefix(self, tmp_path, monkeypatch):
        """Import strips .arcen/ prefix if all entries share it."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".arcen/config.yaml": "model: test\n",
            ".arcen/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (arcen_home / "config.yaml").read_text() == "model: test\n"
        assert (arcen_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_arcen_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a arcen backup."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (arcen_home / "config.yaml").exists()
        # traversal file should NOT exist outside arcen home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (arcen_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (arcen_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (arcen_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (arcen_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".arcen"
        src_home.mkdir(parents=True)
        _make_arcen_tree(src_home)

        monkeypatch.setenv("ARCEN_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from arcen_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".arcen"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("ARCEN_HOME", str(dst_home))
        ...
        # arcen-agent should NOT be present
        assert not (dst_home / "arcen-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from arcen_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without arcen markers fails validation."""
        import io
        from arcen_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_arcen(self):
        """Detects .arcen/ prefix wrapping all entries."""
        import io
        from arcen_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".arcen/config.yaml", "test")
            zf.writestr(".arcen/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".arcen/"
...
class TestBackupEdgeCases:
    def test_nonexistent_arcen_home(self, tmp_path, monkeypatch):
        """Backup exits when arcen home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".arcen"
        monkeypatch.setenv("ARCEN_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("arcen-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_arcen_home(self, tmp_path, monkeypatch):
        """Backup handles empty arcen home (no files to back up)."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        # Only excluded dirs, no actual files
        (arcen_home / "__pycache__").mkdir()
        (arcen_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = arcen_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_arcen(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside arcen root."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        (arcen_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside arcen home
        out_zip = arcen_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        (arcen_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        (arcen_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (arcen_home / "config.yaml").exists()
        assert (arcen_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (arcen_home / "profiles" / "coder" / "config.yaml").exists()
        assert (arcen_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "arcen -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from arcen_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def arcen_home(self, tmp_path):
        """Create a fake ARCEN_HOME with critical state files."""
        home = tmp_path / ".arcen"
        home.mkdir()
    ...
    def test_creates_snapshot(self, arcen_home):
        from arcen_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(arcen_home=arcen_home)
        assert snap_id is not None
        snap_dir = arcen_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, arcen_home):
        from arcen_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", arcen_home=arcen_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, arcen_home):
        from arcen_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(arcen_home=arcen_home)
        db_copy = arcen_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, arcen_home):
        from arcen_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(arcen_home=arcen_home)
        assert (arcen_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, arcen_home):
        from arcen_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(arcen_home=arcen_home)
        with open(arcen_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(arcen_home=empty) is None

    def test_list_snapshots(self, arcen_home):
        from arcen_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", arcen_home=arcen_home)
        id2 = create_quick_snapshot(label="second", arcen_home=arcen_home)

        snaps = list_quick_snapshots(arcen_home=arcen_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, arcen_home):
        from arcen_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", arcen_home=arcen_home)
        snaps = list_quick_snapshots(limit=3, arcen_home=arcen_home)
        assert len(snaps) == 3

    def test_restore_config(self, arcen_home):
        from arcen_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(arcen_home=arcen_home)

        (arcen_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (arcen_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, arcen_home=arcen_home)
        assert result is True
        assert "openrouter" in (arcen_home / "config.yaml").read_text()

    def test_restore_state_db(self, arcen_home):
        from arcen_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(arcen_home=arcen_home)

        conn = sqlite3.connect(str(arcen_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, arcen_home=arcen_home)
    ...
    def test_restore_nonexistent(self, arcen_home):
        from arcen_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", arcen_home=arcen_home) is False

    def test_auto_prune(self, arcen_home):
        from arcen_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", arcen_home=arcen_home)
        snaps = list_quick_snapshots(limit=100, arcen_home=arcen_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, arcen_home):
        from arcen_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", arcen_home=arcen_home)
        deleted = prune_quick_snapshots(keep=3, arcen_home=arcen_home)
        assert deleted == 7
        assert len(list_quick_snapshots(arcen_home=arcen_home)) == 3

    def test_snapshot_includes_pairing_directories(self, arcen_home):
        ...
        (arcen_home / "platforms" / "pairing").mkdir(parents=True)
        (arcen_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (arcen_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (arcen_home / "pairing").mkdir()
        (arcen_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (arcen_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, arcen_home):
        ...
        assert restore_quick_snapshot(snap_id, arcen_home=arcen_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, arcen_home):
...
# ---------------------------------------------------------------------------
# Pre-update backup (arcen update safety net)
# ---------------------------------------------------------------------------

class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``arcen update``
    ...
    def arcen_home(self, tmp_path):
        root = tmp_path / ".arcen"
        root.mkdir()
        _make_arcen_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, arcen_home):
        from arcen_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(arcen_home=arcen_home)
        assert out is not None
        assert out.exists()
        assert out.parent == arcen_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, arcen_home):
        """Pre-update backup should include the same user data that
        ``arcen backup`` would, and should exclude the same directories."""
        from arcen_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(arcen_home=arcen_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # arcen-agent repo excluded
        assert not any(n.startswith("arcen-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, arcen_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from arcen_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(arcen_home=arcen_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(arcen_home=arcen_home)
    ...
    def test_rotation_keeps_only_n(self, arcen_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(arcen_home=arcen_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (arcen_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, arcen_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from arcen_cli.backup import create_pre_update_backup

        (arcen_home / "backups").mkdir(exist_ok=True)
        manual = arcen_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(arcen_home=arcen_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from arcen_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(arcen_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, arcen_home):
        ...
        """
        from arcen_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(arcen_home=arcen_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, arcen_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, arcen_home):
        ...
        first = create_pre_update_backup(arcen_home=arcen_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(arcen_home=arcen_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(arcen_home=arcen_home, keep=0)
    ...
    def test_skips_symlinked_files(self, arcen_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside ARCEN_HOME."""
        from arcen_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def arcen_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".arcen"
        root.mkdir()
        _make_arcen_tree(root)
        # Point ARCEN_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("ARCEN_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for arcen_cli.config + arcen_constants so they pick up ARCEN_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("arcen_cli.config") or mod == "arcen_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, arcen_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from arcen_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "arcen import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((arcen_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, arcen_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not (arcen_home / "backups").exists() or not list(
            (arcen_home / "backups").glob("pre-update-*.zip")
        )

    def test_no_backup_flag_skips(self, arcen_home, capsys):
        from arcen_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, arcen_home, capsys):
        ...
        import yaml
        (arcen_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("arcen_cli.config"):
    ...
    def test_config_disabled_is_silent(self, arcen_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((arcen_home / "backups").glob("pre-update-*.zip")) \
            if (arcen_home / "backups").exists() else True

    def test_cli_flag_overrides_enabled_config(self, arcen_home, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (arcen claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``arcen claw migrate`` runs before mutating ~/.arcen/."""
    ...
    def test_creates_backup_under_backups_dir(self, arcen_home):
        from arcen_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(arcen_home=arcen_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `arcen
        # import` and the update-backup listing both pick them up.
        assert out.parent == arcen_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, arcen_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``arcen backup`` / ``create_pre_update_backup`` — no drift."""
        from arcen_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(arcen_home=arcen_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_arcen_import(self, arcen_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Arcen
        backup — `arcen import` should accept it."""
        from arcen_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(arcen_home=arcen_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, arcen_home):
        from arcen_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(arcen_home=arcen_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(arcen_home=arcen_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, arcen_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(arcen_home=arcen_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((arcen_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_arcen_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.arcen yet — nothing to back up."""
        from arcen_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(arcen_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, arcen_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from arcen_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(arcen_home=arcen_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(arcen_home=arcen_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    """`arcen update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, arcen_home: Path, label="pre-update"):
        from arcen_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, arcen_home=arcen_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from arcen_cli.backup import restore_cron_jobs_if_emptied
        arcen_home = tmp_path / ".arcen"
        jobs_path = arcen_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(arcen_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, arcen_home=arcen_home)
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = arcen_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, arcen_home=arcen_home) is None
        assert restore_cron_jobs_if_emptied("", arcen_home=arcen_home) is None
