...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ARCEN_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ARCEN_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ARCEN_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from arcen_cli.providers import ARCEN_OVERLAYS
        assert "bedrock" in ARCEN_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from arcen_cli.providers import ARCEN_OVERLAYS
        assert ARCEN_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from arcen_cli.providers import ARCEN_OVERLAYS
        assert ARCEN_OVERLAYS["bedrock"].auth_type == "aws_sdk"
