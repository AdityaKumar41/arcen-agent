...
Covers:
 - Normal path (tokens already in Arcen auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Arcen
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `arcen auth openai-codex`. The old
...
@pytest.fixture()
def arcen_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Arcen auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(arcen_auth_only_env):
    """openai-codex appears when tokens are already in Arcen auth store."""
...
def test_codex_picker_uses_live_codex_catalog(arcen_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Arcen
    auth store."""
    arcen_home = tmp_path / ".arcen"
    arcen_home.mkdir()
