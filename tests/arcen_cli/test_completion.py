"""Tests for arcen_cli/completion.py — shell completion script generation."""
...
from arcen_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real arcen structure."""
    p = argparse.ArgumentParser(prog="arcen")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_arcen_completion()" in out
        assert "complete -F _arcen_completion arcen" in out
...
class TestGenerateZsh:
    ...
    def test_registers_compdef_instead_of_invoking_completion_function(self):
        out = generate_zsh(_make_parser())
        assert 'compdef _arcen arcen' in out
        assert '_arcen "$@"' not in out
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_arcen_profiles'" in out
        ...
        try:
            result = subprocess.run(
                [
                    "zsh",
                    "-fc",
                    f"autoload -Uz compinit && compinit -D; source {path}; [[ ${{_comps[arcen]}} == _arcen ]]",
                ],
                capture_output=True,
                text=True,
            )
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c arcen -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from arcen_cli.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_arcen_profiles()" in out
        assert 'profiles_dir="$HOME/.arcen/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'arcen profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _arcen_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_arcen_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _arcen_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_arcen_profiles()" in out
        assert "$HOME/.arcen/profiles" in out
    ...
    def test_fish_has_profiles_helper(self):
        out = generate_fish(_make_parser())
        assert "__arcen_profiles" in out
        assert "$HOME/.arcen/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__arcen_profiles" in out
        count = out.count("(__arcen_profiles)")
