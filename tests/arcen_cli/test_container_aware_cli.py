...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ARCEN_HOME with .container-mode file."""
    arcen_home = tmp_path / ".arcen"
    arcen_home.mkdir()
    monkeypatch.setenv("ARCEN_HOME", str(arcen_home))
    monkeypatch.delenv("ARCEN_DEV", raising=False)

    container_mode = arcen_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=arcen-agent\n"
        "exec_user=arcen\n"
        "arcen_bin=/data/current-package/bin/arcen\n"
    )
    return arcen_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("arcen_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "arcen-agent"
    assert info["exec_user"] == "arcen"
    assert info["arcen_bin"] == "/data/current-package/bin/arcen"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("arcen_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_arcen_dev(container_env, monkeypatch):
    """Returns None when ARCEN_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ARCEN_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_arcen_dev_zero(container_env, monkeypatch):
    """ARCEN_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ARCEN_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        arcen_home = Path(tmpdir) / ".arcen"
        arcen_home.mkdir()
        (arcen_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("arcen_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_arcen_home": lambda: arcen_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ARCEN_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=arcen-custom\n"
        "exec_user=myuser\n"
        "arcen_bin=/opt/arcen/bin/arcen\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "arcen-custom"
    assert info["exec_user"] == "myuser"
    assert info["arcen_bin"] == "/opt/arcen/bin/arcen"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from arcen_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "arcen"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "arcen-agent" in cmd
    assert "/data/current-package/bin/arcen" in cmd
    assert "chat" in cmd
