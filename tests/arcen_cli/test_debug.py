"""Tests for ``arcen debug`` CLI command and debug utilities."""
...
@pytest.fixture
def arcen_home(tmp_path, monkeypatch):
    """Set up an isolated ARCEN_HOME with minimal logs."""
    home = tmp_path / ".arcen"
    home.mkdir()
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from arcen_cli.debug import _upload_paste_rs
        ...
        with patch("arcen_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "arcen_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from arcen_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from arcen_cli.debug import upload_to_pastebin

        with patch("arcen_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("arcen_cli.debug._upload_paste_rs",
             ...
             patch("arcen_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, arcen_home):
        from arcen_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, arcen_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (arcen_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, arcen_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = arcen_home / "logs" / "agent.log"
        from arcen_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, arcen_home):
        ...
        (arcen_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, arcen_home):
    ...
    def test_unknown_log_returns_none(self, arcen_home):
        from arcen_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, arcen_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from arcen_cli.debug import _capture_log_snapshot

        logs_dir = arcen_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, arcen_home):
    ...
    def test_falls_back_when_primary_empty(self, arcen_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of ARCEN_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def arcen_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated ARCEN_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".arcen"
        home.mkdir()
        monkeypatch.setenv("ARCEN_HOME", str(home))
        ...
        monkeypatch.delenv("ARCEN_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, arcen_home_with_secret):
        from arcen_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, arcen_home_with_secret):
        from arcen_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, arcen_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when ARCEN_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("ARCEN_REDACT_SECRETS", "false")

        from arcen_cli.debug import _capture_log_snapshot

        assert os.environ.get("ARCEN_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, arcen_home_with_secret
    ):
        from arcen_cli.debug import _capture_log_snapshot

        log_path = arcen_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, arcen_home_with_secret):
        from arcen_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from arcen_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, arcen_home):
        from arcen_cli.debug import collect_debug_report

        with patch("arcen_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- arcen dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- arcen dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, arcen_home):
        from arcen_cli.debug import collect_debug_report

        with patch("arcen_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, arcen_home):
        from arcen_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, arcen_home):
        from arcen_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, arcen_home, capsys):
        ...
        with patch("arcen_cli.dump.run_dump"), \
             patch("arcen_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("arcen_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, arcen_home, capsys):
        ...
        with patch("arcen_cli.dump.run_dump"), \
             patch(
                 "arcen_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, arcen_home, capsys):
    ...
    def test_share_uploads_three_pastes(self, arcen_home, capsys):
        ...
        with patch("arcen_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- arcen dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- arcen dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- arcen dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, arcen_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from arcen_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("arcen_cli.dump.run_dump"), \
             patch("arcen_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("arcen_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, arcen_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, arcen_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, arcen_home_with_secret, capsys
    ):
        ...
        with patch("arcen_cli.dump.run_dump"), \
             patch("arcen_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("arcen_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "arcen debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, arcen_home):
        from arcen_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from arcen_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from arcen_cli.debug import delete_paste
        ...
        with patch("arcen_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.arcen/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``arcen debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, arcen_home):
        ...
        import ast
        import inspect
        from arcen_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, arcen_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from arcen_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, arcen_home):
    ...
    def test_merges_with_existing_pending(self, arcen_home):
        """Subsequent calls merge into existing pending.json."""
        from arcen_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, arcen_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, arcen_home):
    ...
    def test_sweep_deletes_expired_entries(self, arcen_home):
        ...
        with patch("arcen_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, arcen_home):
        from arcen_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("arcen_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, arcen_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, arcen_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, arcen_home):
        from arcen_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("arcen_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, arcen_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: arcen debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from arcen_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("arcen_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("arcen_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, arcen_home, capsys):
        from arcen_cli.debug import run_debug_share
        ...
        with patch("arcen_cli.dump.run_dump"), \
             ...
             patch("arcen_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, arcen_home, capsys):
        from arcen_cli.debug import run_debug_share
        ...
        with patch("arcen_cli.dump.run_dump"), \
             patch("arcen_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("arcen_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, arcen_home, capsys):
        from arcen_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``arcen debug share`` (CLI) and ``POST /api/ops/debug-share``
    (dashboard). The dashboard renders ``urls`` as real, copyable links, so the
    contract here is the return value, not stdout.
    """

    def test_returns_structured_urls(self, arcen_home):
        from arcen_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("arcen_cli.dump.run_dump"), patch(
            "arcen_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("arcen_cli.debug._schedule_auto_delete"):
            result = build_debug_share(log_lines=50, redact=True)

        assert isinstance(result, DebugShareResult)
        # Seeded logs (agent/gateway) + the summary report.
    ...
    def test_skips_missing_logs_without_failure(self, arcen_home):
        from arcen_cli.debug import build_debug_share

        with patch("arcen_cli.dump.run_dump"), patch(
            "arcen_cli.debug.upload_to_pastebin",
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, arcen_home):
        from arcen_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, arcen_home):
        from arcen_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, arcen_home):
        from arcen_cli.debug import build_debug_share
