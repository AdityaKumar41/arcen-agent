"""Regression tests for #34107 — Docker UID/GID handling in ensure_arcen_home.

When Arcen runs in Docker with ``ARCEN_UID=1000`` / ``ARCEN_GID=911``,
the entrypoint chowns the top-level ``ARCEN_HOME`` once at startup. But
...
The fix is a ``_chown_to_arcen_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_arcen_uid_gid
# ---------------------------------------------------------------------------


class TestResolveArcenUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ARCEN_UID", "1000")
        monkeypatch.setenv("ARCEN_GID", "911")
        from arcen_cli.config import _resolve_arcen_uid_gid
        uid, gid = _resolve_arcen_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("ARCEN_UID", raising=False)
        monkeypatch.delenv("ARCEN_GID", raising=False)
        from arcen_cli.config import _resolve_arcen_uid_gid
        uid, gid = _resolve_arcen_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("ARCEN_UID", "not-a-number")
        monkeypatch.setenv("ARCEN_GID", "911")
        from arcen_cli.config import _resolve_arcen_uid_gid
        uid, gid = _resolve_arcen_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("ARCEN_UID", "")
        monkeypatch.setenv("ARCEN_GID", "")
        from arcen_cli.config import _resolve_arcen_uid_gid
        uid, gid = _resolve_arcen_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToArcenUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_arcen_uid(d)  # must not raise
