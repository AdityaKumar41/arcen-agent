...
class TestEvoLinkModelCatalog:
    ...
    def test_provider_model_ids_prefers_live_models(self, monkeypatch):
        ...
        assert provider_model_ids("evolink") == ["gpt-5.2", "custom-live-model"]
...
class TestEvoLinkProviderResolution:
    def test_provider_overlay_exists(self):
        from arcen_cli.providers import ARCEN_OVERLAYS, get_provider

        assert "evolink" in ARCEN_OVERLAYS
        overlay = ARCEN_OVERLAYS["evolink"]
