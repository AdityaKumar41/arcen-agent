...
Covers:
- Defense 1: gateway stop/restart refuse when _ARCEN_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "arcen gateway restart",
        "arcen gateway stop",
        "arcen gateway start",
        "arcen  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "ARCEN GATEWAY RESTART",           # uppercase
    ])
    def test_arcen_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.arcen.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.arcen.gateway.plist",
        "launchctl stop ai.arcen.gateway",
        "systemctl restart arcen-gateway",
        "systemctl stop arcen-gateway.service",
        "systemctl start arcen-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill arcen gateway process",
        "pkill -f arcen.*gateway",
    ])
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_arcen_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade arcen then run arcen gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.arcen.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify arcen gateway stop/restart refuse when _ARCEN_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_ARCEN_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ARCEN_GATEWAY", raising=False)
