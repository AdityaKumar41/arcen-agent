"""Tests for the top-level `./arcen` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./arcen` should use `arcen_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "arcen"
    called = []

    fake_main_module = types.ModuleType("arcen_cli.main")

    def fake_main():
        called.append("arcen_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "arcen_cli.main", fake_main_module)
