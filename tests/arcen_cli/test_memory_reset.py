"""Tests for the `arcen memory reset` CLI command.
...
- Profile-scoped reset (uses ARCEN_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ARCEN_HOME with memory files."""
    arcen_home = tmp_path / ".arcen"
    memories = arcen_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ARCEN_HOME", str(arcen_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nArcen repo is at ~/.arcen/arcen-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `arcen memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        arcen_home = tmp_path / ".arcen"
        (arcen_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ARCEN_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_arcen_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
