"""Tests for `arcen memory setup [provider]` routing.
...
``arcen memory setup honcho``) without the interactive picker — which matters
because the per-provider ``arcen <provider>`` subcommand is only registered
...
from arcen_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "arcen memory setup" in out
