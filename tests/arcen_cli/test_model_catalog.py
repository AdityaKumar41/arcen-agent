"""Tests for arcen_cli.model_catalog — optional remote manifest fetch + cache."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate ARCEN_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".arcen"
    home.mkdir()
    ...
    import importlib
    from arcen_cli import model_catalog
    importlib.reload(model_catalog)
    monkeypatch.setattr(
        model_catalog,
        "DEFAULT_CATALOG_URL",
        "https://catalog.example/model-catalog.json",
    )
    yield home
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from arcen_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    """``_fetch_manifest_with_fallback`` walks explicitly configured fallbacks."""

    PRIMARY = "https://catalog.example/model-catalog.json"
    FALLBACK = "https://mirror.example/model-catalog.json"
    ...
    def test_no_default_url_skips_network(self, isolated_home, monkeypatch):
        """Production default has no hosted website catalog URL."""
        from arcen_cli import model_catalog
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in arcen_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from arcen_cli import model_catalog
        from arcen_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from arcen_cli import model_catalog
        from arcen_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``arcen model`` CLI — not the live
        ...
        # ``ARCEN_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` ARCEN_HOME directly instead.
        import importlib
        from arcen_cli import model_catalog
        from arcen_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from arcen_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["ARCEN_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("arcen_cli.models.check_nous_free_tier", return_value=False), patch(
                "arcen_cli.models.union_with_portal_free_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ), patch(
                "arcen_cli.models.union_with_portal_paid_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ):
