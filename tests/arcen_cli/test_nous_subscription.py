...
from arcen_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
from arcen_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    monkeypatch.setattr(
        "arcen_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `arcen tools`
# ---------------------------------------------------------------------------
