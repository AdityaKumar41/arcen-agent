"""Tests for `arcen portal` dispatch.

`arcen portal` (no subcommand) is the human-readable alias for the Nous Portal
one-shot onboarding (`arcen auth add nous --type oauth` / `arcen setup
--portal`). The prior status default moved to `arcen portal info`, with
...
from arcen_cli import portal_cli
...
def test_bare_portal_and_login_run_one_shot(monkeypatch, sub):
    """`arcen portal`, `arcen portal login` -> one-shot onboarding."""
    ...
    monkeypatch.setattr(
        "arcen_cli.setup._run_portal_one_shot", fake_one_shot
    )
...
def test_info_and_status_alias_run_status(monkeypatch, sub):
    """`arcen portal info` and the `status` back-compat alias -> status."""
    calls = {"login": 0, "status": 0}

    monkeypatch.setattr(
        "arcen_cli.setup._run_portal_one_shot",
...
def test_login_cancelled_returns_one(monkeypatch):
    ...
    monkeypatch.setattr("arcen_cli.setup._run_portal_one_shot", boom)
    monkeypatch.setattr(portal_cli, "load_config", lambda: {})
...
def test_one_shot_delegates_to_model_flow_nous(monkeypatch):
    ...
    # _model_flow_nous lives in arcen_cli.main and is imported lazily inside
    # _run_portal_one_shot, so patch it at the source module.
    monkeypatch.setattr("arcen_cli.main._model_flow_nous", fake_model_flow)
    ...
    assert calls["model_flow"] == 1, (
        "`arcen portal` must route through _model_flow_nous so the model "
...
def test_one_shot_swallows_cancel_and_systemexit(monkeypatch, exc):
    ...
    monkeypatch.setattr("arcen_cli.main._model_flow_nous", boom)
    monkeypatch.setattr("arcen_cli.config.load_config", lambda: {})
