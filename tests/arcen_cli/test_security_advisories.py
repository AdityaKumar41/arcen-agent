"""Tests for arcen_cli.security_advisories.
...
import arcen_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect ARCEN_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".arcen"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full arcen_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "arcen_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "arcen_cli.config.save_config",
