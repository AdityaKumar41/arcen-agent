"""Tests for IRC gateway configuration via `arcen setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Arcen install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Arcen install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#arcen")
            monkeypatch.setenv("IRC_NICKNAME", "arcen-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `arcen setup gateway` experience with IRC present."""
