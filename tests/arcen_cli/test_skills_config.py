"""Tests for arcen_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestGetDisabledSkills:
    def test_empty_config(self):
        from arcen_cli.skills_config import get_disabled_skills
...
class TestSaveDisabledSkills:
    @patch("arcen_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from arcen_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("arcen_cli.config.load_config")
    ...
    @patch("arcen_cli.config.load_config")
    @patch.dict("os.environ", {"ARCEN_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """ARCEN_SESSION_PLATFORM should be used when ARCEN_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_arcen_platform_takes_precedence(self, tmp_path, monkeypatch):
        """ARCEN_PLATFORM should win over ARCEN_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ARCEN_HOME", str(tmp_path))
        monkeypatch.setenv("ARCEN_PLATFORM", "telegram")
        monkeypatch.setenv("ARCEN_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from arcen_cli.skills_config import _get_categories
        skills = [
