...
``arcen_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``arcen --help``, ``arcen version``,
``arcen logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``arcen --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from arcen_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["arcen", "-w", "--tui"], None),
        (["arcen", "version"], "version"),
        (["arcen", "--tui", "chat"], "chat"),
        (["arcen", "-w", "logs"], "logs"),
        (["arcen", "chat", "hello world"], "chat"),
        (["arcen", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["arcen", "-m", "gpt5", "chat"], "chat"),
        (["arcen", "--model", "gpt5", "chat", "hi"], "chat"),
        (["arcen", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["arcen", "-z", "hello world"], None),
        (["arcen", "-z", "hello", "chat"], "chat"),
        (["arcen", "--model=gpt5", "chat"], "chat"),     # inline form
        (["arcen", "--", "chat"], "chat"),               # -- terminator
        (["arcen", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["arcen", "some-plugin-cmd"], "some-plugin-cmd"),
        (["arcen", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["arcen"],                          # bare → chat
        ["arcen", "--help"],                # top-level help
        ["arcen", "-h"],
        ["arcen", "version"],               # known built-in
        ["arcen", "logs"],
        ["arcen", "gateway", "run"],
        ["arcen", "--tui"],
        ["arcen", "-w", "--tui"],
        ["arcen", "chat", "hi"],
        ["arcen", "help"],                  # accepted built-in-ish
        ["arcen", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["arcen", "meet", "join"],          # potential google_meet plugin
        ["arcen", "honcho", "status"],      # potential memory plugin
        ["arcen", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``arcen help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"arcen_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
