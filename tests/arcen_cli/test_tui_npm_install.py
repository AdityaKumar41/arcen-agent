...
@pytest.fixture
def main_mod():
    import arcen_cli.main as m

    return m


def _touch_ink(root: Path) -> None:
    ink = root / "node_modules" / "@arcen" / "ink" / "package.json"
...
def test_make_tui_argv_scopes_npm_install_on_termux_workspace(
    ...
    assert install_cmd[:7] == [
        ...
        "ui-tui",
        "--workspace",
        "ui-tui/packages/arcen-ink",
        "--include-workspace-root=false",
    ]
    assert calls[0][1]["cwd"] == str(tmp_path)


def test_make_tui_argv_keeps_workspace_install_behaviour(
    tmp_path: Path, main_mod, monkeypatch
) -> None:
    tui_dir = tmp_path / "ui-tui"
    tui_dir.mkdir()
...
def test_make_tui_argv_keeps_always_build_behaviour(
    tmp_path: Path, main_mod, monkeypatch
) -> None:
    _touch_tui_entry(tmp_path)
    monkeypatch.delenv("TERMUX_VERSION", raising=False)
...
def test_no_stray_lockfiles_in_workspace_subdirs(main_mod) -> None:
    ...
    # Also sweep ui-tui/packages/* (arcen-ink etc.)
