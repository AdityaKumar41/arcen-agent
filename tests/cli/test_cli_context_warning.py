...
def _isolate(tmp_path, monkeypatch):
    """Isolate ARCEN_HOME so tests don't touch real config."""
    home = tmp_path / ".arcen"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal ArcenCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ArcenCLI.__new__(ArcenCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["arcen-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Arcen' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Arcen' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Arcen' minimum context length."""
