...
minimal ``ArcenCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def arcen_home(tmp_path, monkeypatch):
    """Isolated ARCEN_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".arcen"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("ARCEN_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves ARCEN_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal ArcenCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, arcen_home):
        ...
        with patch("arcen_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, arcen_home):
        ...
        with patch("arcen_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, arcen_home):
    ...
    def test_no_assistant_message_skipped(self, arcen_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, arcen_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, arcen_home):
        ...
        src = inspect.getsource(ArcenCLI.chat)
