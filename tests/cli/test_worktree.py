...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("arcen/arcen-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned arcen/* and pr-* branches."""

    def test_prunes_orphaned_arcen_branch(self, git_repo):
        """arcen/arcen-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "arcen/arcen-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "arcen/arcen-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "arcen/arcen-deadbeef" in result.stdout
        ...
        assert "arcen/arcen-deadbeef" in orphaned
        ...
        assert "arcen/arcen-deadbeef" not in result.stdout
