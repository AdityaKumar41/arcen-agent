"""Shared fixtures for the arcen-agent test suite.
...
2. **Isolated ARCEN_HOME.** ARCEN_HOME points to a per-test tempdir so
   code reading ``~/.arcen/*`` via ``get_arcen_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".arcen"`` instead of the canonical
   ``get_arcen_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ARCEN_SESSION_* inheritance** — the agent's current gateway
...
# ARCEN_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ARCEN_BEHAVIORAL_VARS = frozenset({
    "ARCEN_YOLO_MODE",
    "ARCEN_INTERACTIVE",
    "ARCEN_QUIET",
    "ARCEN_TOOL_PROGRESS",
    "ARCEN_TOOL_PROGRESS_MODE",
    "ARCEN_MAX_ITERATIONS",
    "ARCEN_SESSION_PLATFORM",
    "ARCEN_SESSION_CHAT_ID",
    "ARCEN_SESSION_CHAT_NAME",
    "ARCEN_SESSION_THREAD_ID",
    "ARCEN_SESSION_SOURCE",
    "ARCEN_SESSION_KEY",
    "ARCEN_GATEWAY_SESSION",
    "ARCEN_CRON_SESSION",
    "_ARCEN_GATEWAY",
    "ARCEN_PLATFORM",
    "ARCEN_MODEL",
    "ARCEN_INFERENCE_MODEL",
    "ARCEN_INFERENCE_PROVIDER",
    "ARCEN_TUI_PROVIDER",
    "ARCEN_MANAGED",
    "ARCEN_DEV",
    "ARCEN_CONTAINER",
    ...
    "ARCEN_TIMEZONE",
    "ARCEN_REDACT_SECRETS",
    "ARCEN_BACKGROUND_NOTIFICATIONS",
    "ARCEN_EXEC_ASK",
    "ARCEN_HOME_MODE",
    "ARCEN_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.arcen/kanban.db instead of the per-test ARCEN_HOME.
    "ARCEN_KANBAN_DB",
    "ARCEN_KANBAN_BOARD",
    "ARCEN_KANBAN_HOME",
    "ARCEN_KANBAN_WORKSPACES_ROOT",
    "ARCEN_KANBAN_LOGS_ROOT",
    "ARCEN_KANBAN_TASK",
    "ARCEN_KANBAN_WORKSPACE",
    "ARCEN_KANBAN_RUN_ID",
    "ARCEN_KANBAN_CLAIM_LOCK",
    "ARCEN_KANBAN_DISPATCH_IN_GATEWAY",
    "ARCEN_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ARCEN_HOME to per-test tempdirs so code that
    reads ``~/.arcen/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ARCEN_* vars that could change test semantics.
    for name in _ARCEN_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ARCEN_HOME to a per-test tempdir. Code that reads
    #    ``~/.arcen/*`` via ``get_arcen_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.arcen/*`` via
    #    ``Path.home() / ".arcen"`` instead of ``get_arcen_home()``
    #    is a bug to fix at the callsite.
    fake_arcen_home = tmp_path / "arcen_test"
    fake_arcen_home.mkdir()
    (fake_arcen_home / "sessions").mkdir()
    (fake_arcen_home / "cron").mkdir()
    (fake_arcen_home / "memories").mkdir()
    (fake_arcen_home / "skills").mkdir()
    monkeypatch.setenv("ARCEN_HOME", str(fake_arcen_home))
    ...
    #    ~/.arcen/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import arcen_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_arcen_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal arcen config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``arcen-gateway`` process
...
# correlated with a ``tests/arcen_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> arcen-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart arcen-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting arcen/python patterns are also blocked.
    """
    ...
    def _matches_arcen_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ARCEN_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_arcen_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch arcen-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m arcen_cli.main".
                if (
                    "arcen" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live arcen-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting arcen/python could hit the live gateway. "
