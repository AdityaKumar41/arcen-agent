...
@pytest.fixture
def arcen_env(tmp_path, monkeypatch):
    """Isolate ARCEN_HOME for each test so jobs/scripts don't leak."""
    home = tmp_path / ".arcen"
    home.mkdir()
    (home / "scripts").mkdir()
    (home / "cron").mkdir()
    ...
    # Reload modules that cache get_arcen_home() at import time.
    import importlib
...
def test_create_job_no_agent_requires_script(arcen_env):
...
def test_create_job_no_agent_stores_field(arcen_env):
    from cron.jobs import create_job

    script_path = arcen_env / "scripts" / "watchdog.sh"
...
def test_create_job_default_is_not_no_agent(arcen_env):
    from cron.jobs import create_job
...
def test_update_job_roundtrips_no_agent_flag(arcen_env):
    from cron.jobs import create_job, update_job, get_job

    script_path = arcen_env / "scripts" / "w.sh"
...
def test_cronjob_tool_create_no_agent_without_script_errors(arcen_env):
...
def test_cronjob_tool_create_no_agent_with_script_succeeds(arcen_env):
    from tools.cronjob_tools import cronjob

    script_path = arcen_env / "scripts" / "alert.sh"
    script_path.write_text("#!/bin/bash\necho alert\n")
...
def test_cronjob_tool_update_toggles_no_agent(arcen_env):
    from tools.cronjob_tools import cronjob
...
def test_cronjob_tool_update_no_agent_without_script_errors(arcen_env):
...
def test_cronjob_tool_create_does_not_require_prompt_when_no_agent(arcen_env):
...
def test_run_job_no_agent_success_returns_script_stdout(arcen_env):
...
def test_run_job_no_agent_wake_gate_is_silent(arcen_env):
    """wakeAgent=false gate in stdout triggers a silent run."""
    from cron.jobs import create_job
    from cron.scheduler import run_job, SILENT_MARKER

    script_path = arcen_env / "scripts" / "gated.sh"
...
def test_run_job_no_agent_script_failure_delivers_error(arcen_env):
    """Non-zero exit → success=False, error alert is the delivered message."""
    from cron.jobs import create_job
    from cron.scheduler import run_job

    script_path = arcen_env / "scripts" / "broken.sh"
    script_path.write_text("#!/bin/bash\necho oops >&2\nexit 3\n")
...
def test_run_job_no_agent_never_invokes_aiagent(arcen_env):
...
def test_run_job_script_shell_script_runs_via_bash(arcen_env):
    """.sh files should execute under /bin/bash even without a shebang line."""
    from cron.scheduler import _run_job_script

    script_path = arcen_env / "scripts" / "shelly.sh"
...
def test_run_job_script_bash_extension_also_runs_via_bash(arcen_env):
    from cron.scheduler import _run_job_script

    script_path = arcen_env / "scripts" / "thing.bash"
...
def test_run_job_script_python_still_runs_via_python(arcen_env):
    """Regression: .py files must keep running via sys.executable."""
    from cron.scheduler import _run_job_script

    script_path = arcen_env / "scripts" / "py.py"
...
def test_run_job_script_path_traversal_still_blocked(arcen_env):
