...
ARCEN_HOME scoping, and tick() serialization for profile jobs.
"""
...
def isolated_cron_profile_home(tmp_path, monkeypatch):
    """Create an isolated Arcen root with a named profile and temp cron store."""
    root = tmp_path / "arcen-root"
...
class TestCronjobToolProfile:
    ...
    def test_schema_advertises_profile(self):
        ...
        assert "arcen profile" in desc_lower
        assert "context-local" in desc_lower
        assert "subprocess" in desc_lower
        assert "temporarily sets arcen_home" not in desc_lower
...
class TestRunJobProfileContext:
    ...
    def _install_agent_stubs(monkeypatch, observed: dict):
        ...
        class FakeAgent:
            def __init__(self, **kwargs):
                ...
                observed["env_home_during_init"] = os.environ.get("ARCEN_HOME")
                observed["profile_env_only_during_init"] = os.environ.get(
                    "ARCEN_PROFILE_TEST_ONLY"
                )
                observed["profile_env_shared_during_init"] = os.environ.get(
                    "ARCEN_PROFILE_TEST_SHARED"
                )
                observed["arcen_home_during_init"] = str(get_arcen_home())
                observed["scheduler_home_during_init"] = str(sched._get_arcen_home())
                observed["skip_context_files"] = kwargs.get("skip_context_files")

            def run_conversation(self, *_a, **_kw):
                from arcen_constants import get_arcen_home

                observed["env_home_during_run"] = os.environ.get("ARCEN_HOME")
                ...
                observed["arcen_home_during_run"] = str(get_arcen_home())
                observed["scheduler_home_during_run"] = str(sched._get_arcen_home())
        ...
        from arcen_cli import runtime_provider as runtime_provider
        ...
        monkeypatch.setattr(sched, "_arcen_home", None)
        monkeypatch.setenv("ARCEN_CRON_TIMEOUT", "0")
    ...
    def test_run_job_sets_and_restores_profile_home(
        ...
        assert observed["arcen_home_during_init"] == str(profile_home.resolve())
        assert observed["arcen_home_during_run"] == str(profile_home.resolve())
        ...
        assert os.environ["ARCEN_HOME"] == str(root)
        assert sched._get_arcen_home() == root
    ...
    def test_profile_dotenv_environment_is_restored(
        ...
        observed: dict = {}
        self._install_agent_stubs(monkeypatch, observed)
        monkeypatch.setenv("ARCEN_PROFILE_TEST_SHARED", "outer")
        monkeypatch.delenv("ARCEN_PROFILE_TEST_ONLY", raising=False)

        def fake_load_dotenv(path, *_a, **_kw):
            observed.setdefault("dotenv_paths", []).append(str(path))
            os.environ["ARCEN_PROFILE_TEST_SHARED"] = "profile-value"
            os.environ["ARCEN_PROFILE_TEST_ONLY"] = "profile-only"
            os.environ["ARCEN_CRON_TIMEOUT"] = "123"
            return True
        ...
        assert os.environ["ARCEN_PROFILE_TEST_SHARED"] == "outer"
        assert "ARCEN_PROFILE_TEST_ONLY" not in os.environ
        assert os.environ["ARCEN_CRON_TIMEOUT"] == "0"
        assert os.environ["ARCEN_HOME"] == str(root)
        assert sched._get_arcen_home() == root
    ...
    def test_no_agent_profile_uses_profile_scripts_dir_and_restores_env(
        ...
        (scripts_dir / "print_home.py").write_text(
            "import os\nprint(os.environ.get('ARCEN_HOME', ''))\n",
            encoding="utf-8",
        )
        monkeypatch.setattr(sched, "_arcen_home", None)
    ...
    def test_run_job_without_profile_leaves_arcen_home_untouched(
        self, isolated_cron_profile_home, monkeypatch
    ):
        import cron.scheduler as sched
        ...
        assert observed["arcen_home_during_init"] == str(root)
        assert os.environ["ARCEN_HOME"] == str(root)
