...
- Path resolution (absolute, relative to ARCEN_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ARCEN_HOME."""
    arcen_home = tmp_path / ".arcen"
    arcen_home.mkdir()
    (arcen_home / "cron").mkdir()
    (arcen_home / "cron" / "output").mkdir()
    (arcen_home / "scripts").mkdir()
    monkeypatch.setenv("ARCEN_HOME", str(arcen_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ARCEN_DIR", arcen_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", arcen_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", arcen_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", arcen_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.arcen/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ARCEN_SESSION_PLATFORM") is None
        assert os.environ.get("ARCEN_SESSION_CHAT_ID") is None
        assert os.environ.get("ARCEN_SESSION_CHAT_NAME") is None
