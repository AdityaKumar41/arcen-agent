...
Override the image with ``ARCEN_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ARCEN_TEST_IMAGE", "arcen-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ARCEN_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ARCEN_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged arcen user
# ---------------------------------------------------------------------------
#
# Background: every Arcen runtime path inside the container drops to UID
# 10000 (the ``arcen`` user) via ``s6-setuidgid arcen``. ``docker exec``
...
# fails (PermissionError swallowed) for arcen, so a test that ran as root
...
# helpers below run every probe under ``-u arcen`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: arcen).
