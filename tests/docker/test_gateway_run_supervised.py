...
Before the s6 migration, ``docker run nousresearch/arcen-agent gateway
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``ARCEN_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``ARCEN_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``arcen`` per the conftest module
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``arcen gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "ARCEN_GATEWAY_NO_SUPERVISE=1",
    ...
    if inspect.stdout.strip() == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "ARCEN_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The ARCEN_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``arcen gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``arcen gateway
    ...
    # Count python processes running `arcen gateway run`. If the
    ...
    r = _sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*arcen.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `arcen gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-arcen`
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${ARCEN_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Arcen Gateway Starting…"
    ...
    assert "⚕" in combined or "Arcen Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Arcen Gateway Starting" in file_contents, (
