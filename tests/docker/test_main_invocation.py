...
def test_no_args_starts_arcen(built_image: str) -> None:
    """``docker run <image>`` should start arcen cleanly.
...
def test_chat_subcommand_passthrough(built_image: str) -> None:
    """``docker run <image> chat --help`` should exec ``arcen chat --help``.
...
def test_bare_executable_passthrough(built_image: str) -> None:
    ...
    arcen wrapper. Useful for long-lived sandbox mode and for testing.
    """
    r = subprocess.run(
...
def test_container_exit_code_matches_inner_exit(built_image: str) -> None:
    ...
    Critical for CI: ``docker run <image> arcen batch ...`` returns a
