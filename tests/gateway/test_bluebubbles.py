...
class TestBlueBubblesHelpers:
    ...
    def test_default_mention_patterns_match_arcen_variants(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter.require_mention is True
        assert adapter._message_matches_mention_patterns("Arcen, summarize this")
        assert adapter._message_matches_mention_patterns("@Arcen agent help")
        assert not adapter._message_matches_mention_patterns("casual family chatter")
        assert not adapter._message_matches_mention_patterns("antiarcen should not match")
    ...
    def test_custom_mention_patterns_override_defaults(self, monkeypatch):
        ...
        assert not adapter._message_matches_mention_patterns("Arcen what is next?")

    def test_clean_mention_text_strips_leading_wake_word(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter._clean_mention_text("Arcen, summarize this") == "summarize this"
        assert adapter._clean_mention_text("Arcen agent: summarize this") == "summarize this"
        assert adapter._clean_mention_text("please ask Arcen about this") == "please ask Arcen about this"
