...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Arcen"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^arcen"]}
        )
        assert adapter._message_matches_mention_patterns("arcen please help") is True
        assert adapter._message_matches_mention_patterns("please arcen help") is False

    def test_pattern_is_case_insensitive(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^arcen"]}
        )
        assert adapter._message_matches_mention_patterns("ARCEN help") is True
...
class TestShouldProcessMessage:
    ...
    def test_group_accepted_when_text_matches_wake_word(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch,
            extra={"require_mention": True, "mention_patterns": ["^arcen"]},
        )
        msg = MagicMock(is_in_at_list=False)
        assert adapter._should_process_message(msg, "arcen help", is_group=True, chat_id="grp1") is True
