...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "arcen_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_arcen_home() / config.yaml`` — point it here.
    monkeypatch.setattr("arcen_constants.get_arcen_home", lambda: arcen_home)
    monkeypatch.setattr("arcen_cli.config.get_arcen_home", lambda: arcen_home)
    return cfg_path
