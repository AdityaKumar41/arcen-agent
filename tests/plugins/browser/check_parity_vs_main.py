...
Run from the PR worktree:
    ...
    cd ~/.arcen/arcen-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.arcen/arcen-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.arcen/arcen-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a arcen-agent checkout"
)
...
# Isolated ARCEN_HOME for the config write.
