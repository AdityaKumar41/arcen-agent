...
Verifies that:
 ...
 2. User plugins at $ARCEN_HOME/plugins/model-providers/<name>/ override bundled
...
def _clear_provider_caches():
    ...
    for mod in list(sys.modules.keys()):
        if (
            ...
            or mod.startswith("_arcen_user_provider")
        ):
            del sys.modules[mod]
...
def test_all_profiles_register():
    ...
    for required in (
        ...
        "minimax-oauth", "gmi", "evolink", "xiaomi", "alibaba-coding-plan",
    ):
...
def test_general_plugin_manager_skips_model_provider_kind(tmp_path, monkeypatch):
    ...
    from arcen_cli import plugins as plugin_mod
    ...
    user_plugin = arcen_home / "plugins" / "test-model-provider"
