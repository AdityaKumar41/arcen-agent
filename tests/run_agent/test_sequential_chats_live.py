...
AlexKucera reported on Discord (2026-04-16) that after ``arcen update`` pulled
...
Opt-in — not part of default CI:
    ARCEN_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.arcen/.env).
"""
...
LIVE = os.environ.get("ARCEN_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set ARCEN_LIVE_TESTS=1"),
