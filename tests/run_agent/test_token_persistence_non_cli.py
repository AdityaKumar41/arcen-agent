...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    arcen_state = ModuleType("arcen_state")
    arcen_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "arcen_state", arcen_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Arcen"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Arcen"
