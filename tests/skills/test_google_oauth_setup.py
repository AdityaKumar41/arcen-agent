...
class TestArcenConstantsFallback:
    """Tests for _arcen_home.py fallback when arcen_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_arcen_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _arcen_home.py with arcen_constants blocked."""
        monkeypatch.setitem(sys.modules, "arcen_constants", None)
        spec = importlib.util.spec_from_file_location("_arcen_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_arcen_home_env_var(self, monkeypatch, tmp_path):
        """When arcen_constants is missing, ARCEN_HOME comes from env var."""
        monkeypatch.setenv("ARCEN_HOME", str(tmp_path / "custom-arcen"))
        module = self._load_helper(monkeypatch)
        assert module.get_arcen_home() == tmp_path / "custom-arcen"

    def test_fallback_defaults_to_dot_arcen(self, monkeypatch):
        """When arcen_constants is missing and ARCEN_HOME unset, default to ~/.arcen."""
        monkeypatch.delenv("ARCEN_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_arcen_home() == Path.home() / ".arcen"

    def test_fallback_ignores_empty_arcen_home(self, monkeypatch):
        """Empty/whitespace ARCEN_HOME is treated as unset."""
        monkeypatch.setenv("ARCEN_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_arcen_home() == Path.home() / ".arcen"

    def test_fallback_display_arcen_home_shortens_path(self, monkeypatch):
        """Fallback display_arcen_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("ARCEN_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_arcen_home() == "~/.arcen"

    def test_fallback_display_arcen_home_profile_path(self, monkeypatch):
        """Fallback display_arcen_home() handles profile paths under ~/."""
        monkeypatch.setenv("ARCEN_HOME", str(Path.home() / ".arcen/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_arcen_home() == "~/.arcen/profiles/coder"

    def test_fallback_display_arcen_home_custom_path(self, monkeypatch):
        """Fallback display_arcen_home() returns full path for non-home locations."""
        monkeypatch.setenv("ARCEN_HOME", "/opt/arcen-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_arcen_home() == "/opt/arcen-custom"

    def test_delegates_to_arcen_constants_when_available(self):
        """When arcen_constants IS importable, _arcen_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_arcen_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import arcen_constants
        assert module.get_arcen_home is arcen_constants.get_arcen_home
        assert module.display_arcen_home is arcen_constants.display_arcen_home
...
class TestInstallDeps:
    ...
    Regression coverage for the Arcen Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "arcen-agent[google]" in out
