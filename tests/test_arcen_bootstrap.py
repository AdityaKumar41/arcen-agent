"""Tests for arcen_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Arcen entry point
(arcen, arcen-agent, arcen-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Arcen entry point imports arcen_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported arcen_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("arcen_bootstrap", None)
    import arcen_bootstrap  # noqa: WPS433
    return arcen_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Arcen CLI
...
class TestEntryPointsImportBootstrap:
    """Every Arcen entry point must import arcen_bootstrap as its
    ...
    # Entry points that invoke Arcen as a process.  Each one must
    # import arcen_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "arcen_cli/main.py",   # arcen CLI (console_script)
        "run_agent.py",          # arcen-agent (console_script)
        "acp_adapter/entry.py",  # arcen-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import arcen_bootstrap' and that
        ...
        half-finished ``arcen update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``arcen_bootstrap`` as a top-level module) leaves arcen
        ...
        """
        # Resolve relative to the arcen-agent repo root.  Tests live
        # at tests/test_arcen_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # arcen start even when arcen_bootstrap hasn't been
        ...
        assert first_import_name == "arcen_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'arcen_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import arcen_bootstrap' line to be the first import."
        )
