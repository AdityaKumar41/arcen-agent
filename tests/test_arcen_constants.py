"""Tests for arcen_constants module."""
...
class TestGetDefaultArcenRoot:
    """Tests for get_default_arcen_root() — Docker/custom deployment awareness."""

    def test_no_arcen_home_returns_native(self, tmp_path, monkeypatch):
        """When ARCEN_HOME is not set, returns ~/.arcen."""
        monkeypatch.delenv("ARCEN_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_arcen_root() == tmp_path / ".arcen"

    def test_arcen_home_is_native(self, tmp_path, monkeypatch):
        """When ARCEN_HOME = ~/.arcen, returns ~/.arcen."""
        native = tmp_path / ".arcen"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARCEN_HOME", str(native))
        assert get_default_arcen_root() == native

    def test_arcen_home_is_profile(self, tmp_path, monkeypatch):
        """When ARCEN_HOME is a profile under ~/.arcen, returns ~/.arcen."""
        native = tmp_path / ".arcen"
    ...
    def test_arcen_home_is_docker(self, tmp_path, monkeypatch):
        """When ARCEN_HOME points outside ~/.arcen (Docker), returns ARCEN_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARCEN_HOME", str(docker_home))
        assert get_default_arcen_root() == docker_home

    def test_arcen_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ARCEN_HOME outside ~/.arcen is treated as the root."""
        custom = tmp_path / "my-arcen-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARCEN_HOME", str(custom))
        assert get_default_arcen_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ARCEN_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARCEN_HOME", str(profile))
        assert get_default_arcen_root() == docker_root

    def test_no_arcen_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\arcen, not ~/.arcen."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ARCEN_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(arcen_constants.sys, "platform", "win32")

        assert get_default_arcen_root() == local_appdata / "arcen"

    def test_no_arcen_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/arcen without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("ARCEN_HOME", raising=False)
        ...
        assert get_default_arcen_root() == home / "AppData" / "Local" / "arcen"


class TestGetArcenHome:
    """Tests for get_arcen_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When ARCEN_HOME is unset on Windows, use %LOCALAPPDATA%\\arcen."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ARCEN_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(arcen_constants.sys, "platform", "win32")
        monkeypatch.setattr(arcen_constants, "_profile_fallback_warned", False)

        assert get_arcen_home() == local_appdata / "arcen"
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(arcen_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(arcen_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".arcen"
