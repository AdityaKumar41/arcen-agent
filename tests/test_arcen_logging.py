"""Tests for arcen_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    arcen_logging._logging_initialized = False
    ...
    arcen_logging._install_session_record_factory()
    yield
    ...
    arcen_logging._logging_initialized = False
    arcen_logging.clear_session_context()


@pytest.fixture
def arcen_home(tmp_path, monkeypatch):
    """Provide an isolated ARCEN_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_arcen_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["ARCEN_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, arcen_home):
        log_dir = arcen_logging.setup_logging(arcen_home=arcen_home)
        assert log_dir == arcen_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, arcen_home):
        arcen_logging.setup_logging(arcen_home=arcen_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, arcen_home):
        arcen_logging.setup_logging(arcen_home=arcen_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, arcen_home):
        arcen_logging.setup_logging(arcen_home=arcen_home)
        arcen_logging.setup_logging(arcen_home=arcen_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, arcen_home):
        arcen_logging.setup_logging(arcen_home=arcen_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        arcen_logging.setup_logging(arcen_home=arcen_home, force=True)
    ...
    def test_custom_log_level(self, arcen_home):
        arcen_logging.setup_logging(arcen_home=arcen_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, arcen_home):
        arcen_logging.setup_logging(
            arcen_home=arcen_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, arcen_home):
        arcen_logging.setup_logging(arcen_home=arcen_home)
    ...
    def test_writes_to_agent_log(self, arcen_home):
        arcen_logging.setup_logging(arcen_home=arcen_home)

        test_logger = logging.getLogger("test_arcen_logging.write_test")
        ...
        agent_log = arcen_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, arcen_home):
        arcen_logging.setup_logging(arcen_home=arcen_home)

        test_logger = logging.getLogger("test_arcen_logging.warning_test")
        ...
        agent_log = arcen_home / "logs" / "agent.log"
        errors_log = arcen_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, arcen_home):
        arcen_logging.setup_logging(arcen_home=arcen_home)

        test_logger = logging.getLogger("test_arcen_logging.info_test")
    ...
    def test_explicit_params_override_config(self, arcen_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (arcen_home / "config.yaml").write_text(yaml.dump(config))

        arcen_logging.setup_logging(arcen_home=arcen_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, arcen_home):
        """The custom record factory injects session_tag on all records."""
        arcen_logging.setup_logging(arcen_home=arcen_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_arcen_session_injector", False), (
            "Record factory should have _arcen_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, arcen_home):
        arcen_logging.setup_logging(arcen_home=arcen_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, arcen_home):
        arcen_logging.setup_logging(arcen_home=arcen_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, arcen_home):
        ...
        gw_log = arcen_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, arcen_home):
    ...
    def test_gateway_log_receives_gateway_records(self, arcen_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, arcen_home):
    ...
    def test_agent_log_still_receives_all(self, arcen_home):
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, arcen_home):
        """When session context is set, log lines include [session_id]."""
        arcen_logging.setup_logging(arcen_home=arcen_home)
        arcen_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, arcen_home):
    ...
    def test_clear_session_context(self, arcen_home):
        """After clearing, session tag disappears."""
        arcen_logging.setup_logging(arcen_home=arcen_home)
        arcen_logging.set_session_context("xyz789")
        arcen_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, arcen_home):
        ...
        def thread_a():
            arcen_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            arcen_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        arcen_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        arcen_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = arcen_logging._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = arcen_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in arcen_logging.COMPONENT_PREFIXES
        # The gateway component captures both core gateway logs and the
        # arcen_plugins facility (plugin-installed gateway adapters log
        # under that prefix).
        assert ("gateway", "arcen_plugins") == arcen_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = arcen_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == arcen_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = arcen_logging.COMPONENT_PREFIXES["cli"]
        assert "arcen_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == arcen_logging.COMPONENT_PREFIXES["cron"]


class TestSetupVerboseLogging:
    """setup_verbose_logging() adds a DEBUG-level console handler."""

    def test_adds_stream_handler(self, arcen_home):
        arcen_logging.setup_logging(arcen_home=arcen_home)
        arcen_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_arcen_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, arcen_home):
        arcen_logging.setup_logging(arcen_home=arcen_home)
        arcen_logging.setup_verbose_logging()
        arcen_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        arcen_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = arcen_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        arcen_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("arcen_cli.config.is_managed", return_value=True):
                arcen_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, arcen_home):
        level, max_size, backup = arcen_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, arcen_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, arcen_home):
        import yaml
        config = {"model": "test"}
        (arcen_home / "config.yaml").write_text(yaml.dump(config))
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> arcen_logging._ManagedRotatingFileHandler:
        handler = arcen_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that arcen_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        handler.flush()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        arcen_logging.setup_logging(arcen_home=arcen_home, mode="gateway")
        gw_path = arcen_home / "logs" / "gateway.log"
        rotated = arcen_home / "logs" / "gateway.log.1"
