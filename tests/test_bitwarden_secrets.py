...
binary download are exercised manually by `arcen secrets bitwarden
...
def arcen_home(tmp_path, monkeypatch):
    """Point Arcen at an isolated home directory."""
    home = tmp_path / ".arcen"
    home.mkdir()
    monkeypatch.setenv("ARCEN_HOME", str(home))
    # Some modules cache get_arcen_home; clear if needed.
    import arcen_constants
    if hasattr(arcen_constants, "_ARCEN_HOME_CACHE"):
        arcen_constants._ARCEN_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(arcen_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(arcen_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(arcen_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from arcen_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under arcen_home/cache/."""
    home = tmp_path / ".arcen"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
