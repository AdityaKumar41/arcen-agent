"""Tests for the secret-source tracking in ``arcen_cli.env_loader``.

These cover the small public surface that lets `arcen model` / `arcen setup`
...
from arcen_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_arcen_dotenv()`` is called at module-import time from several
    hot modules (cli.py, arcen_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per ARCEN_HOME per process.
    """
    ...
    # cli.py, arcen_cli/main.py, run_agent.py, trajectory_compressor.py,
