"""
Tests for mcp_serve — Arcen MCP server.
...
def _isolate_arcen_home(tmp_path, monkeypatch):
    """Redirect ARCEN_HOME to a temp directory."""
    monkeypatch.setenv("ARCEN_HOME", str(tmp_path))
    try:
        import arcen_constants
        monkeypatch.setattr(arcen_constants, "get_arcen_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking arcen_state schema."""
