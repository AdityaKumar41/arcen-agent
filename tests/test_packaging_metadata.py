...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``arcen_cli``) for the package
    itself and ``arcen_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``arcen_cli`` without ``arcen_cli.*``,
    so the wheel shipped ``arcen_cli/*.py`` but dropped subpackages such as
    ``proxy``. This kind of package-data drift should fail in tests rather than
    in a user's install.
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (arcen_cli/plugins.py) registers each bundled plugin by
