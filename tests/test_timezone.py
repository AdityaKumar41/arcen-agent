"""
Tests for timezone support (arcen_time module + integration points).
...
def _reset_arcen_time_cache():
    """Reset the arcen_time module cache (replacement for removed reset_cache)."""
    arcen_time._cached_tz = None
    arcen_time._cached_tz_name = None
    arcen_time._cache_resolved = False


# =========================================================================
# arcen_time.now() — core helper
# =========================================================================

class TestArcenTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_arcen_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["ARCEN_TIMEZONE"] = "Asia/Kolkata"
        result = arcen_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["ARCEN_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="arcen_time"):
            result = arcen_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["ARCEN_TIMEZONE"] = "Europe/London"
        tz = arcen_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["ARCEN_TIMEZONE"] = "Not/A/Timezone"
        tz = arcen_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When ARCEN_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) ARCEN_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("ARCEN_TIMEZONE=" + os.environ.get("ARCEN_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "ARCEN_TIMEZONE=NOT_SET" in result["output"], (
            "ARCEN_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When ARCEN_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("ARCEN_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=arcen_tz) which shifted
        absolute time when system-local tz != Arcen tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_arcen_tz(self):
        """Already-aware datetimes should be normalized to Arcen tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Arcen tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Arcen tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Arcen is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Arcen tz
        ...
        # Use an Arcen timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _arcen_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["ARCEN_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_arcen_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Arcen timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["ARCEN_TIMEZONE"] = "US/Eastern"
        _reset_arcen_time_cache()
