...
class TestToolsetConsistency:
    ...
    def test_arcen_platforms_share_core_tools(self):
        """All arcen-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on arcen-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["arcen-cli", "arcen-telegram", "arcen-discord", "arcen-whatsapp", "arcen-slack", "arcen-signal", "arcen-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_arcen_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("arcen-whatsapp")

    def test_arcen_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("arcen-api-server")
