...
def test_session_context_uses_session_cwd(monkeypatch, tmp_path):
    ...
    Falling back to os.getcwd() makes agents answer from the launcher folder
    even when the session cwd is a real project.
    """
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["ARCEN_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("ARCEN_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("ARCEN_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("ARCEN_VOICE", "1")
    monkeypatch.delenv("ARCEN_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("ARCEN_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("ARCEN_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "arcen_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("ARCEN_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("ARCEN_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "arcen_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    import arcen_cli.config as config_mod
    ...
    # whose tools live in arcen-cli's universe (see toolsets.py).
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("ARCEN_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "arcen_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("ARCEN_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("ARCEN_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("ARCEN_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("ARCEN_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "arcen_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown ARCEN_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_history_to_messages_renders_multimodal_content():
    # bb/gui preserves image URLs in the resume payload so clients
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    monkeypatch.delenv("ARCEN_MODEL", raising=False)
    monkeypatch.setenv("ARCEN_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("ARCEN_MODEL", raising=False)
    monkeypatch.delenv("ARCEN_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/arcen-test "}}
    )

    assert server._resolve_model() == "nous/arcen-test"


def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("ARCEN_MODEL", "nous/arcen-test")
    monkeypatch.setenv("ARCEN_TUI_PROVIDER", "nous")
    monkeypatch.delenv("ARCEN_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nous/arcen-test", "nous")


def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("ARCEN_MODEL", "nous/arcen-test")
    monkeypatch.delenv("ARCEN_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("ARCEN_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
        "arcen_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("nous/arcen-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("ARCEN_MODEL", "sonnet")
    monkeypatch.delenv("ARCEN_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("ARCEN_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "arcen_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "arcen_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "arcen_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_ws_orphan_reap_closes_worker_when_session_stays_detached(monkeypatch):
    ...
    Regression for #38591 fallout: every reconnect spawned a fresh
...
def test_session_create_does_not_persist_empty_row(monkeypatch):
    ...
    Every TUI launch opens a session here just to paint the composer;
...
def test_config_set_fast_updates_live_agent_and_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "arcen_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("ARCEN_YOLO_MODE", "false")
    ...
    assert os.environ.get("ARCEN_YOLO_MODE") == "1"
...
def test_config_set_statusbar_survives_non_dict_display(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_arcen_home", tmp_path)
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("ARCEN_EXEC_ASK", raising=False)
    monkeypatch.delenv("ARCEN_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("ARCEN_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["ARCEN_GATEWAY_SESSION"] == "1"
    assert server.os.environ["ARCEN_EXEC_ASK"] == "1"
    assert server.os.environ["ARCEN_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("arcen_cli.main._has_any_provider_configured", lambda: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("arcen_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
        "arcen_cli.runtime_provider.resolve_runtime_provider",
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("arcen_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    monkeypatch.setattr("arcen_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_config_set_model_syncs_inference_provider_env(monkeypatch):
    """After an explicit provider switch, ARCEN_INFERENCE_PROVIDER must
    ...
    server._sessions["sid"] = _session(agent=_Agent())
    monkeypatch.setenv("ARCEN_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "arcen_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    assert os.environ["ARCEN_INFERENCE_PROVIDER"] == "anthropic"


def test_config_set_model_syncs_tui_provider_unconditionally(monkeypatch):
    """Regression for #16857: /model must set ARCEN_TUI_PROVIDER even when
    ...
    # Both env vars must reflect the user's choice. ARCEN_TUI_PROVIDER is
    # the canonical explicit-this-process carrier consumed by
    # _resolve_startup_runtime() on /new.
    assert os.environ["ARCEN_TUI_PROVIDER"] == "custom:xuanji"
    assert os.environ["ARCEN_INFERENCE_PROVIDER"] == "custom:xuanji"
...
def test_config_set_model_syncs_tui_provider_env(monkeypatch):
    ...
    agent = Agent()
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setenv("ARCEN_TUI_PROVIDER", "openai-codex")
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("arcen_cli.model_switch.switch_model", fake_switch_model)
    ...
    try:
        ...
        assert resp["result"]["value"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["ARCEN_TUI_PROVIDER"] == "anthropic"
        assert os.environ["ARCEN_MODEL"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["ARCEN_INFERENCE_MODEL"] == "anthropic/claude-sonnet-4.6"
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors ArcenCLI._manual_compress's
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "Arcen TUI Status" in out
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("arcen_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "arcen_cli.commands.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("arcen_state")
    ...
    monkeypatch.setitem(sys.modules, "arcen_state", fake_mod)
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_arcen_home
    # fixture pins ARCEN_HOME to a temp dir; the handler should append
    # /sessions to it.
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `arcen model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `arcen model` and the classic CLI /model picker.
    ...
    with patch(
        "arcen_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("arcen_cli.models.provider_model_ids") as live_fetch:
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"arcen_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "arcen_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
                "arcen_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
            patch(
                "arcen_cli.browser_connect.manual_chrome_debug_command",
                return_value=None,
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "arcen_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_arcen_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.arcen/.env
    ...
    with patch.dict(sys.modules, {"arcen_cli.config": fake}):
...
def test_session_save_writes_under_arcen_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Arcen profile
    home — not the project/workspace CWD — and include the system prompt,
    mirroring the classic CLI /save export.

    Regression: the gateway handler wrote ``arcen_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Arcen.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("arcen_conversation_*.json"))
    ...
    assert payload["model"] == "arcen-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are Arcen."
