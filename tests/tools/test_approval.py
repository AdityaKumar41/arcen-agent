...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("arcen_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("arcen_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"ARCEN_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_arcen_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.arcen/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_arcen_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ARCEN_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_arcen_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$ARCEN_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestArcenConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.arcen/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.arcen/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.arcen/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.arcen/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.arcen/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.arcen/config.yaml")
        assert dangerous is True
        assert "arcen config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.arcen/config.yaml")
        assert dangerous is True

    def test_custom_arcen_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ARCEN_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.arcen/config.yaml"
        )
        assert dangerous is True
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.arcen/config.yaml"
        )
        assert dangerous is True

    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.arcen/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.arcen/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.arcen/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.arcen/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.arcen/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Arcen config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_arcen_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $ARCEN_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.arcen/arcen-agent && source venv/bin/activate && python -m arcen_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m arcen_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m arcen_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "arcen_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart arcen-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkill_arcen_detected(self):
        """pkill targeting arcen/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_arcen_detected(self):
        cmd = "killall arcen"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep arcen) bypasses the pkill/killall name-matching
    ...
    def test_pkill_arcen_still_detected(self):
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("ARCEN_GATEWAY_SESSION", "ARCEN_CRON_SESSION",
                      "ARCEN_YOLO_MODE",
                      "ARCEN_SESSION_KEY", "ARCEN_INTERACTIVE")
        }
        os.environ.pop("ARCEN_YOLO_MODE", None)
        os.environ.pop("ARCEN_INTERACTIVE", None)
        # ARCEN_CRON_SESSION takes priority over ARCEN_GATEWAY_SESSION in
        ...
        os.environ.pop("ARCEN_CRON_SESSION", None)
        os.environ["ARCEN_GATEWAY_SESSION"] = "1"
        os.environ["ARCEN_SESSION_KEY"] = self.SESSION_KEY
