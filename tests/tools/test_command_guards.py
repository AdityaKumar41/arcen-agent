...
def _clean_state():
    ...
    saved = {}
    for k in ("ARCEN_INTERACTIVE", "ARCEN_GATEWAY_SESSION", "ARCEN_EXEC_ASK", "ARCEN_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ARCEN_INTERACTIVE"] = "1"
        session_key = os.getenv("ARCEN_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ARCEN_INTERACTIVE or ARCEN_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
