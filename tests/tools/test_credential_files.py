...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        arcen_home.mkdir()
        (arcen_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(arcen_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.arcen/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        (arcen_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        (arcen_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        (arcen_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.arcen/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        arcen_home = tmp_path / ".arcen"
        (arcen_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ARCEN_HOME": str(arcen_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.arcen")

        assert mounts[0]["container_path"] == "/home/user/.arcen/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.arcen/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.arcen/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ARCEN_HOME."""
        monkeypatch.setenv("ARCEN_HOME", str(tmp_path / ".arcen"))
        (tmp_path / ".arcen").mkdir()

        # Create a sensitive file one level above arcen_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside arcen_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ARCEN_HOME must still be registered."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))
        (arcen_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_arcen_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ARCEN_HOME must be allowed."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        subdir = arcen_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ARCEN_HOME pointing outside must be rejected."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))

        # Create a sensitive file outside arcen_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside arcen_home pointing outside
        symlink = arcen_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ARCEN_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, arcen_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ARCEN_HOME."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))
        ...
        sensitive.write_text("{}")
        self._write_config(arcen_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(arcen_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ARCEN_HOME via config must still mount."""
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))

        (arcen_home / "oauth.json").write_text("{}")
        self._write_config(arcen_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        (arcen_home / "cache" / "documents").mkdir(parents=True)
        (arcen_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.arcen/cache/documents" in paths
        assert "/root/.arcen/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.arcen/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        arcen_home = tmp_path / ".arcen"
        arcen_home.mkdir()
        # Use legacy dir name — get_arcen_dir prefers old if it exists
        (arcen_home / "document_cache").mkdir()
        (arcen_home / "image_cache").mkdir()
        monkeypatch.setenv("ARCEN_HOME", str(arcen_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(arcen_home / "document_cache") in host_paths
        assert str(arcen_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.arcen/cache/documents" in container_paths
        assert "/root/.arcen/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        arcen_home = tmp_path / ".arcen"
        doc_dir = arcen_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        arcen_home = tmp_path / ".arcen"
        ss_dir = arcen_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.arcen/cache/screenshots/session_abc/screen1.png"
