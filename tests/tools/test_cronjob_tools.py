...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `arcen-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.arcen/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.arcen/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because arcen uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("ARCEN_INTERACTIVE", raising=False)
        monkeypatch.delenv("ARCEN_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("ARCEN_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("ARCEN_INTERACTIVE", false_like_value)
        monkeypatch.delenv("ARCEN_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("ARCEN_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["ARCEN_INTERACTIVE", "ARCEN_GATEWAY_SESSION", "ARCEN_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("ARCEN_INTERACTIVE", "ARCEN_GATEWAY_SESSION", "ARCEN_EXEC_ASK"):
