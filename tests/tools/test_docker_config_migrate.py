...
def _run_migration(arcen_home: Path, **env_overrides: str) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env.update(
        {
...
def test_docker_config_migrate_does_not_rewrite_invalid_yaml(tmp_path: Path) -> None:
    ...
    assert "arcen config:" in proc.stderr
...
def test_docker_config_migrate_skip_env_leaves_config_unchanged(tmp_path: Path) -> None:
    ...
    proc = _run_migration(tmp_path, ARCEN_SKIP_CONFIG_MIGRATION="1")
