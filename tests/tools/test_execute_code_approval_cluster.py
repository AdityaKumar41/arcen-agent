...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ARCEN_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ARCEN_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("ARCEN_GATEWAY_SESSION", "1")
    monkeypatch.delenv("ARCEN_INTERACTIVE", raising=False)
    monkeypatch.delenv("ARCEN_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("ARCEN_CRON_SESSION", "1")
    monkeypatch.delenv("ARCEN_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_arcen_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ARCEN_HOME": "/h", "ARCEN_PROFILE": "p",
        "ARCEN_CONFIG": "/c.yaml", "ARCEN_ENV": "/e",
        # other ARCEN_* → dropped (broad prefix removed)
        "ARCEN_BASE_URL": "https://x", "ARCEN_INTERACTIVE": "1",
        "ARCEN_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ARCEN_HOME", "ARCEN_PROFILE", "ARCEN_CONFIG", "ARCEN_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ARCEN_BASE_URL", "ARCEN_INTERACTIVE", "ARCEN_KANBAN_DB",
...
def test_env_scrub_logs_dropped_arcen_vars(caplog):
    """Dropping a non-allowlisted, non-secret ARCEN_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ARCEN_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ARCEN_HOME": "/h",          # allowlisted → kept, not logged
        "ARCEN_BASE_URL": "https://x",   # dropped → logged
        "ARCEN_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ARCEN_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ARCEN_HOME" in out and "PATH" in out
    assert "ARCEN_BASE_URL" not in out and "ARCEN_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ARCEN_BASE_URL" in msgs and "ARCEN_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ARCEN_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ARCEN_* vars."""
    import logging
