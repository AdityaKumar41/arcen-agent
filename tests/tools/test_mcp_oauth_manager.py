...
def test_arcen_provider_subclass_exists():
    """ArcenMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ARCEN_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ARCEN_PROVIDER_CLS is not None
    assert issubclass(_ARCEN_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_arcen_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ArcenMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ARCEN_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ARCEN_HOME", str(tmp_path))
    ...
    assert _ARCEN_PROVIDER_CLS is not None
    assert isinstance(provider, _ARCEN_PROVIDER_CLS)
    assert provider._arcen_server_name == "srv"
