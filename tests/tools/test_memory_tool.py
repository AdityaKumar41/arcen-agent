...
class TestScanMemoryContent:
    ...
    def test_arcen_config_mod_blocked(self):
        result = _scan_memory_content("edit .arcen/config.yaml to change settings")
        assert "Blocked" in result
        assert "arcen_config_mod" in result
        result = _scan_memory_content("update .arcen/SOUL.md with new personality")
        assert "Blocked" in result
        assert "arcen_config_mod" in result
    ...
    def test_arcen_config_mod_no_false_positives(self):
        """Merely mentioning arcen config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .arcen/config.yaml for settings") is None
        assert _scan_memory_content("Read .arcen/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .arcen/config.yaml file contains runtime options") is None
