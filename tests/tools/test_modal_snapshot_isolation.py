...
def _restore_tool_modules():
    original_arcen_home = os.environ.get("ARCEN_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_arcen_home is None:
            os.environ.pop("ARCEN_HOME", None)
        else:
            os.environ["ARCEN_HOME"] = original_arcen_home
        _reset_modules(("tools", "arcen_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    arcen_cli = types.ModuleType("arcen_cli")
    arcen_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["arcen_cli"] = arcen_cli
    arcen_home = tmp_path / "arcen-home"
    os.environ["ARCEN_HOME"] = str(arcen_home)
    sys.modules["arcen_cli.config"] = types.SimpleNamespace(
        get_arcen_home=lambda: arcen_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="arcen-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": arcen_home / "modal_snapshots.json",
