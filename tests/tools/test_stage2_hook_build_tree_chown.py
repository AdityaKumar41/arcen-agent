...
$INSTALL_DIR (/opt/arcen/.venv, ui-tui, node_modules) to the runtime arcen
UID whenever they are not already arcen-owned — INDEPENDENTLY of whether
$ARCEN_HOME ownership already matches.

Regression guard for the ARCEN_UID/PUID remap path broken by #35027.

`usermod -u <new> arcen` re-chowns the arcen home dir ($ARCEN_HOME ==
/opt/data) to the new UID as a side effect. #35027 gated the build-tree chown
behind `stat $ARCEN_HOME != arcen_uid`, so after any remap that stat is
...
The fix probes the build trees directly (stat .venv) rather than $ARCEN_HOME.
...
def test_build_tree_chown_not_gated_on_arcen_home(stage2_text: str) -> None:
    """The build-tree chown must NOT live inside the `if [ "$needs_chown" = true ]`
    block keyed on $ARCEN_HOME ownership — that is exactly the #35027 bug."""
    block = _build_tree_block(stage2_text)
    # The block probes the venv owner, not $ARCEN_HOME.
...
def _run_build_tree_block(
    text: str, *, venv_owner: int, arcen_uid: int
) -> bool:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        #   id -u arcen       -> handled via actual_arcen_uid var below
        #   chown ...          -> record that it fired
        script = (
            "set -eu\n"
            f'INSTALL_DIR="/opt/arcen"\n'
            f'actual_arcen_uid={arcen_uid}\n'
...
def test_chown_fires_when_venv_owner_differs(stage2_text: str) -> None:
    """The #35027 regression scenario: after a remap $ARCEN_HOME already
    matches the new UID, but the venv is still owned by the build-time UID
    (10000). The build-tree chown MUST still fire."""
    fired = _run_build_tree_block(stage2_text, venv_owner=10000, arcen_uid=4242)
    assert fired, (
        "build-tree chown must fire when the venv is not owned by the runtime "
        "arcen UID, regardless of $ARCEN_HOME ownership (#35027 regression)"
    )


def test_chown_skipped_when_venv_already_owned(stage2_text: str) -> None:
    """Idempotency: once the venv is arcen-owned, the recursive chown is
    skipped on subsequent boots."""
    fired = _run_build_tree_block(stage2_text, venv_owner=4242, arcen_uid=4242)
    assert not fired, (
        "build-tree chown must be skipped when the venv already matches the "
        "runtime arcen UID (avoid expensive recursive chown on every restart)"
    )


def test_chown_skipped_for_default_uid(stage2_text: str) -> None:
    """No remap: venv owned by the default build UID (10000) and arcen is
    still 10000 — nothing to do."""
    fired = _run_build_tree_block(stage2_text, venv_owner=10000, arcen_uid=10000)
    assert not fired
