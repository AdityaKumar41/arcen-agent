...
When ARCEN_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the arcen user's home directory ($ARCEN_HOME == /opt/data).
Runtime-writable trees under ``/opt/arcen`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/arcen/gateway`` is one such
