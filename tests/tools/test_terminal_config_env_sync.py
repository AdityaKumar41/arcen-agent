...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. arcen_cli/config.py:save_config_value
                       -> ``_config_to_env_sync`` dict (one-shot when the
                          user runs ``arcen config set …``)
...
dicts) keeps the test independent of the user's ~/.arcen/config.yaml and
mirrors the pattern used in tests/arcen_cli/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``arcen config set foo bar``."""
    from arcen_cli import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    """``arcen config set terminal.X true`` must propagate to .env for
    ...
    assert not missing, (
        f"`arcen config set terminal.X` doesn't sync these load-bearing "
        f"keys to .env: {sorted(missing)}.  Add them to _config_to_env_sync "
        f"in arcen_cli/config.py:set_config_value."
    )
...
def test_docker_persist_across_processes_is_bridged_everywhere():
    ...
    container on Arcen exit or just stops it (keeping it for the next
...
def test_docker_orphan_reaper_is_bridged_everywhere():
    ...
    ``terminal.docker_orphan_reaper`` controls whether Arcen sweeps stale
...
def test_docker_volumes_is_bridged_everywhere():
    ...
    ``arcen config set terminal.docker_volumes '["/host:/workspace"]'`` wrote
...
def test_docker_forward_env_is_bridged_everywhere():
    ...
    ``arcen config set terminal.docker_forward_env '["GITHUB_TOKEN"]'`` had no
