...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._arcen_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(arcen_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(arcen_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_arcen_bin(self):
        """After _INSTALL_FAILED, manual install in ARCEN_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        arcen_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(arcen_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(arcen_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._arcen_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == arcen_bin
            assert _tirith_mod._resolved_path == arcen_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# ARCEN_HOME isolation
# ---------------------------------------------------------------------------

class TestArcenHomeIsolation:
    def test_arcen_bin_dir_respects_arcen_home(self):
        """_arcen_bin_dir must use ARCEN_HOME, not hardcoded ~/.arcen."""
        from tools.tirith_security import _arcen_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ARCEN_HOME": tmpdir}):
            result = _arcen_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_arcen_home(self):
        """_failure_marker_path must use ARCEN_HOME, not hardcoded ~/.arcen."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ARCEN_HOME": "/custom/arcen"}):
            result = _failure_marker_path()
        assert result == "/custom/arcen/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ARCEN_HOME; verify it's active."""
        arcen_home = os.getenv("ARCEN_HOME")
        assert arcen_home is not None, "ARCEN_HOME should be set by conftest"
        assert "arcen_test" in arcen_home, "Should point to test temp dir"

    def test_get_arcen_home_fallback(self):
        """Without ARCEN_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_arcen_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ARCEN_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".arcen")
            result = _get_arcen_home()
