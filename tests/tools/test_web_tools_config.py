...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_arcen_home_override(self, tmp_path):
        """Auth lookup should read from ARCEN_HOME/auth.json, not ~/.arcen/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".arcen").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``arcen tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "arcen_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
