...
def test_check_website_access_uses_dynamic_arcen_home(monkeypatch, tmp_path):
    ...
    # Invalidate the module-level cache so the new ARCEN_HOME is picked up.
    # A prior test may have cached a default policy (enabled=False) under the
    # old ARCEN_HOME set by the autouse _isolate_arcen_home fixture.
...
def test_check_website_access_fails_open_on_malformed_config(tmp_path, monkeypatch):
    ...
    # Simulate default path by pointing ARCEN_HOME to tmp_path
