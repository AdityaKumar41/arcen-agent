...
class TestWriteDenyExactPaths:
    ...
    def test_arcen_env(self):
        # ``.env`` under the active ARCEN_HOME (profile-aware, not just
        # ``~/.arcen``) must be write-denied. The hermetic test conftest
        # points ARCEN_HOME at a tempdir — resolve via get_arcen_home()
        # to match the denylist.
        from arcen_constants import get_arcen_home
        path = str(get_arcen_home() / ".env")
        assert _is_write_denied(path) is True

    def test_arcen_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.arcen/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "arcen_root"
        ...
        # Sanity check: ARCEN_HOME does point to the profile dir, not the root.
        from arcen_constants import get_arcen_home, get_default_arcen_root
        assert get_arcen_home() == profile_home
        assert get_default_arcen_root() == root
...
class TestWriteAllowed:
    ...
    def test_arcen_config_not_env(self):
        path = os.path.join(str(Path.home()), ".arcen", "config.yaml")
