...
def server():
    with patch.dict(
        "sys.modules",
        {
            "arcen_constants": MagicMock(
                get_arcen_home=MagicMock(return_value="/tmp/arcen_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'arcen-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'arcen-release' patched"
    }
