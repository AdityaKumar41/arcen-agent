...
One ``CDPSupervisor`` runs per Arcen ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "arcen-dialog-bridge.invalid"
...
(() => {
  if (window.__arcenDialogBridgeInstalled) return;
  window.__arcenDialogBridgeInstalled = true;
  const ENDPOINT = "http://arcen-dialog-bridge.invalid/";
