...
selectable via ``arcen tools`` → Image Generation; the active model is
...
def _get_managed_fal_client(managed_gateway):
    ...
    with _managed_fal_client_lock:
        ...
        _managed_fal_client = _ManagedFalSyncClient(
            fal_client,
            key=managed_gateway.user_token,
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `arcen tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_tool_gateway_enabled():
        lines.append(
            "  2. Sign in to a managed gateway account that has the managed FAL "
            "gateway enabled (`arcen setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `arcen tools` "
        "→ Image Generation (run `arcen plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping arcen with a FAL key configured
