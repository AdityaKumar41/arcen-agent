#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Arcen Skills Hub.
...
Used by arcen_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            arcen_meta = metadata.get("arcen", {})
            if isinstance(arcen_meta, dict):
                tags = arcen_meta.get("tags", [])
        if not tags:
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                arcen_meta = meta_block.get("arcen", {})
                if isinstance(arcen_meta, dict):
                    tags = arcen_meta.get("tags", [])
...
def _load_arcen_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ARCEN_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ARCEN_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ARCEN_INDEX_CACHE_FILE.stat().st_mtime
            if age < ARCEN_INDEX_TTL:
                return json.loads(ARCEN_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ARCEN_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Arcen index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Arcen index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ARCEN_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ARCEN_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ArcenIndexSource(SkillSource):
    """Skill source backed by the centralized Arcen Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "arcen-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "arcen-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ArcenIndexSource(auth=auth), # Centralized index (search + resolved install paths)
