...
it is automatically downloaded from GitHub releases to $ARCEN_HOME/bin/tirith.
...
def _get_arcen_home() -> str:
    """Return the Arcen home directory, respecting ARCEN_HOME env var."""
    return str(get_arcen_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_arcen_home(), ".tirith-install-failed")
...
def _arcen_bin_dir() -> str:
    """Return $ARCEN_HOME/bin, creating it if needed."""
    d = os.path.join(_get_arcen_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ARCEN_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_arcen_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ARCEN_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ARCEN_HOME/bin/tirith
    ...
    arcen_bin = os.path.join(_arcen_bin_dir(), "tirith")
    if os.path.isfile(arcen_bin) and os.access(arcen_bin, os.X_OK):
        _resolved_path = arcen_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return arcen_bin
