...
def managed_tool_gateway_enabled(*, force_fresh: bool = False) -> bool:
    """Compatibility gate for the removed first-party managed tool gateway."""
    return False
...
def managed_tool_gateway_unavailable_message(
    ...
    *,
    ...
    """Return guidance for unavailable managed tool gateway paths."""
    return (
        f"{capability} is unavailable in this Arcen Agent build. Configure "
        "the direct provider API key for this tool instead."
    )
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    if managed_enabled is None:
        managed_enabled = managed_tool_gateway_enabled()
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.arcen/.env`` (via
    ``arcen_cli.config.get_env_value`` when available) so tool-side
