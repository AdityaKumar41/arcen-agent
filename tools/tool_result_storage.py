...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/arcen-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/arcen-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/arcen-results"
HEREDOC_MARKER = "ARCEN_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/arcen-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"ARCEN_PERSIST_{uuid.uuid4().hex[:8]}"
