...
provider** — turn on a backend by enabling a plugin (``arcen plugins
enable video_gen/<name>``) and selecting it in ``arcen tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/arcen-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `arcen tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `arcen plugins list` to see "
            f"installed video gen backends, or `arcen tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `arcen tools` → "
...
# `arcen tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`arcen tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `arcen tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
