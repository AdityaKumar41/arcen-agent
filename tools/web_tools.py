...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.arcen/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``arcen tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `arcen tools` repaint.
        try:
