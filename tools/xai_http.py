...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``arcen tools`` repaint, tool-registration scans,
    ...
    2. ``~/.arcen/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_arcen_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.arcen/.env`` first, then ``os.environ``.

    Wraps :func:`arcen_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from arcen_cli.config import get_env_value as _arcen_get_env_value

        value = _arcen_get_env_value(name)
...
def arcen_xai_user_agent() -> str:
    """Return a stable Arcen-specific User-Agent for xAI HTTP calls."""
    try:
        from arcen_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Arcen-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Arcen-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``arcen_cli.config.get_env_value`` so keys
    stored in ``~/.arcen/.env`` (the standard Arcen location) are honored —
