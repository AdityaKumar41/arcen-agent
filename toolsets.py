...
_ARCEN_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (ARCEN_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `arcen tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ARCEN_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Arcen toolsets (CLI + messaging platforms)
    #
    ...
    "arcen-cli": {
        ...
        "tools": _ARCEN_CORE_TOOLS,
        "includes": []
    },

    "arcen-cron": {
        # Mirrors arcen-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `arcen tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as arcen-cli; gated by `arcen tools`",
        "tools": _ARCEN_CORE_TOOLS,
        "includes": []
    },
    ...
    "arcen-email": {
        "description": "Email bot toolset - interact with Arcen via email (IMAP/SMTP)",
        "tools": _ARCEN_CORE_TOOLS,
        "includes": []
    },
    ...
    "arcen-sms": {
        "description": "SMS bot toolset - interact with Arcen via SMS (Twilio)",
        "tools": _ARCEN_CORE_TOOLS,
        "includes": []
    },
    ...
    "arcen-webhook": {
        ...
        "tools": _ARCEN_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "arcen-gateway": {
        ...
        "tools": [],
        "includes": ["arcen-telegram", "arcen-discord", "arcen-whatsapp", "arcen-slack", "arcen-signal", "arcen-bluebubbles", "arcen-homeassistant", "arcen-email", "arcen-sms", "arcen-mattermost", "arcen-matrix", "arcen-dingtalk", "arcen-feishu", "arcen-wecom", "arcen-wecom-callback", "arcen-weixin", "arcen-qqbot", "arcen-webhook", "arcen-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (arcen-<name>).
        # Gives them _ARCEN_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("arcen-"):
            platform_name = name[len("arcen-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ARCEN_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
