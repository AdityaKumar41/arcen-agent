...
# arcen modules.  arcen_cli sets ARCEN_PYTHON_SRC_ROOT before spawning this
# subprocess; inserting it first ensures the installed packages win.
_src_root = os.environ.get("ARCEN_PYTHON_SRC_ROOT", "")
...
# tested; override via ``ARCEN_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("ARCEN_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``ARCEN_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``arcen --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def main():
    ...
    # server stalled the whole shell on "summoning arcen…" for the full
