"""Persistent slash-command worker — one ArcenCLI per TUI session.
...
def _run(cli: ArcenCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ARCEN_SESSION_KEY"] = args.session_key
    os.environ["ARCEN_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ArcenCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
