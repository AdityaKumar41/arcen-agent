...
_DISABLE_FLUSH = (os.environ.get("ARCEN_TUI_GATEWAY_NO_FLUSH", "") or "").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
class TeeTransport:
    ...
    secondaries swallow failures so a secondary sink never stalls the
    main IO path.
    """
    ...
    def write(self, obj: dict) -> bool:
        # Primary first so a slow secondary never delays Ink/stdio.
