...
React + Ink terminal UI for Arcen. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ARCEN_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.arcen/.arcen_history` or under `ARCEN_HOME`.
...
```text
ui-tui/
  packages/arcen-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      arcen-ink.d.ts    type declarations for @arcen/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ArcenCLI subprocess for slash commands
```
