...
await build({
  ...
  // Skip the prebuilt @arcen/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@arcen/ink': resolve(root, 'packages/arcen-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The arcen_cli launcher
