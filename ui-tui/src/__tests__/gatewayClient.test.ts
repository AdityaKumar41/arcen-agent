...
describe('GatewayClient websocket attach mode', () => {
  ...
  beforeEach(() => {
    originalGatewayUrl = process.env.ARCEN_TUI_GATEWAY_URL
    FakeWebSocket.reset()
  ...
  afterEach(() => {
    if (originalGatewayUrl === undefined) {
      delete process.env.ARCEN_TUI_GATEWAY_URL
    } else {
      process.env.ARCEN_TUI_GATEWAY_URL = originalGatewayUrl
    }
  ...
  it('waits for websocket open and resolves RPC requests', async () => {
    process.env.ARCEN_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=abc'
  ...
  it('reattaches when ARCEN_TUI_GATEWAY_URL rotates between requests', async () => {
    process.env.ARCEN_TUI_GATEWAY_URL = 'ws://gateway-old.test/api/ws?token=abc'
    const gw = new GatewayClient()
    ...
    process.env.ARCEN_TUI_GATEWAY_URL = 'ws://gateway-new.test/api/ws?token=xyz'
  ...
  it('redacts query string secrets in attach failure logs and events', () => {
    process.env.ARCEN_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=hunter2&channel=secret'
    delete (globalThis as { WebSocket?: unknown }).WebSocket
  ...
  it('redacts attach URL secrets when the WebSocket constructor throws', () => {
    ...
    process.env.ARCEN_TUI_GATEWAY_URL = secretUrl
  ...
  it('redacts user-info credentials even on URLs the WHATWG parser rejects', () => {
    ...
    process.env.ARCEN_TUI_GATEWAY_URL = fixture
    delete (globalThis as { WebSocket?: unknown }).WebSocket
