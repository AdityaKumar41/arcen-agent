import type { RunExternalProcess } from '@arcen/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`arcen ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching arcen: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`arcen ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
