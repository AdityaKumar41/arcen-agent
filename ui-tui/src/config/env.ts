import type { MouseTrackingMode } from '@arcen/ink'
...
export const STARTUP_RESUME_ID = (process.env.ARCEN_TUI_RESUME ?? '').trim()
export const STARTUP_QUERY = (process.env.ARCEN_TUI_QUERY ?? '').trim()
export const STARTUP_IMAGE = (process.env.ARCEN_TUI_IMAGE ?? '').trim()
...
//
// - ARCEN_TUI_MOUSE_TRACKING (truthy/falsy) explicitly overrides everything.
//   This is the "force a value" knob and intentionally beats the legacy
//   kill-switch and the Termux default.
// - ARCEN_TUI_DISABLE_MOUSE=1 forces mouse off — the legacy kill switch.
...
//   behavior.
const mouseTrackingOverride = parseToggle(process.env.ARCEN_TUI_MOUSE_TRACKING)
const mouseTrackingDisabledLegacy = truthy(process.env.ARCEN_TUI_DISABLE_MOUSE)
...
export const NO_CONFIRM_DESTRUCTIVE = truthy(process.env.ARCEN_TUI_NO_CONFIRM)

const inlineOverride = parseToggle(process.env.ARCEN_TUI_INLINE)
...
// less fragile. Override explicitly with ARCEN_TUI_INLINE=0/1.
...
// synthetic timer).
export const SHOW_FPS = truthy(process.env.ARCEN_TUI_FPS)
