...
const STARTUP_TIMEOUT_MS = Math.max(5000, parseInt(process.env.ARCEN_TUI_STARTUP_TIMEOUT_MS ?? '15000', 10) || 15000)
const REQUEST_TIMEOUT_MS = Math.max(30000, parseInt(process.env.ARCEN_TUI_RPC_TIMEOUT_MS ?? '120000', 10) || 120000)
...
const resolveGatewayAttachUrl = () => {
  const raw = process.env.ARCEN_TUI_GATEWAY_URL?.trim()

  return raw ? raw : null
}

const resolvePython = (root: string) => {
  const configured = process.env.ARCEN_PYTHON?.trim() || process.env.PYTHON?.trim()
...
// Connection URLs often carry bearer tokens in the query
...
export class GatewayClient extends EventEmitter {
  ...
  private startSpawnedGateway(root: string) {
    ...
    const cwd = process.env.ARCEN_CWD || root
  ...
  start() {
    const root = process.env.ARCEN_PYTHON_SRC_ROOT ?? resolve(import.meta.dirname, '../../')
