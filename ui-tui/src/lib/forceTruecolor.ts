...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ARCEN_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Arcen on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ARCEN_TUI_TRUECOLOR=1.
