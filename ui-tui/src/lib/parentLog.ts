...
const logDir = join(process.env.ARCEN_HOME?.trim() || join(homedir(), '.arcen'), 'logs')
...
// a real ~/.arcen (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('arcen-tui: parent lifecycle log unavailable\n')
    }
  }
}
