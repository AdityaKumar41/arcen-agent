...
/**
 * Return true when Arcen should enable Termux-focused TUI defaults.
 *
 * Defaults to on in Termux, with an explicit opt-out for debugging:
 *   ARCEN_TUI_TERMUX_MODE=0
 */
export const isTermuxTuiMode = (env: NodeJS.ProcessEnv = process.env): boolean => {
  ...
  const override = String(env.ARCEN_TUI_TERMUX_MODE ?? '').trim().toLowerCase()
