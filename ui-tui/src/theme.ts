...
// Mirrors @arcen/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @arcen/ink is bundled separately from packages/.
...
// Terminal. Explicit ARCEN_TUI_THEME / COLORFGBG signals above still win,
...
// `ARCEN_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `ARCEN_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `ARCEN_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `ARCEN_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Arcen palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.ARCEN_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.ARCEN_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.ARCEN_TUI_BACKGROUND ?? '')
